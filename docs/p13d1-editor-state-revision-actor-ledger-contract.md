...
- 真实 failure-first：**16 failed / 0 passed**；Grok 首轮专项 **16 passed**，test-only 反假绿返修后 **17 passed**。
- Codex 独立串行通过专项+精确 schema **18 passed**、PRAGMA 顺序回归 **2 passed**、融合 apply/consume、检查点恢复、修订恢复、本地票据回调代表性真实路径 **5 passed**。
- 19 个生产文件冻结哈希逐项一致；`py_compile`、`git diff --check`、精确暂存白名单均通过。未运行后端全量、Playwright、前端 lint/build 或整仓 E2E。
- 顺序污染的真实根因是 P13-C 测试在 `PRAGMA=1` 后调用 `Session.commit()` 归还连接，随后可能在另一池连接执行恢复；已改为同一显式连接完成开启、写入、提交、关闭与精确校验，未修改生产代码或删除结束守卫。
- Codex 以 `功能：完成P13D1修订操作者可信账本` 提交并推送协作分支，实现提交=`a8982e3`。P13-D2 可在工作区与远端一致后单独冻结。
