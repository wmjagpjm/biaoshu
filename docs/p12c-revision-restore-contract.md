...
> **验收**：Grok 首版专项 **23 passed**；Codex 先把“迁移失败安全”假证据改成真实 DROP 前故障注入，得到 **1 failed / 22 passed** 与四文件 **1 failed / 120 passed**，证明 SQLite 回滚后临时表残留；生产返修用零行 DML 触发真实事务后，Codex 独立通过专项 **23**、四文件 **121**、后端串行全量 **800 passed**，前端无改动沿用单 worker、零重试 **263 passed**。
...
Grok 只允许修改以下 11 个文件，其中前 8 个为原实现边界，后 3 个仅允许更新被 C2 合法改变的阶段守卫/结构证据：
...
9. `backend/tests/test_p12c_revision_history_read.py`：只把“restore 路由不存在”阶段守卫收敛为“除精确 C2 POST 外仍无其他写路由”，不得弱化 C1 只读/零写/SQL/损坏断言；
10. `backend/tests/test_p12c_checkpoint_restore_revisions.py`：只把“restore 函数内直接 recorder”AST 更新为“checkpoint 编排以字面量 `checkpoint_restore` 调共享原语，且共享原语唯一调用 recorder”；不得改 D3 行为断言；
11. `backend/tests/test_editor_state_revisions.py`：只把来源精确集合从八类更新为含 `revision_restore` 的九类，并继续证明伪造来源被 DB CHECK 拒绝。

禁止修改 C1 history read service、其他模型/路由/既有测试、认证、前端、依赖、配置或文档。`main.py` 已挂载 revision router，不得改动。三个旧测试的变化必须是上述精确阶段演进，不能删除用例、放宽状态码/计数/作用域/零写或以 C2 专项取代旧回归。Grok 不得 `git add/commit/push`。
...
扩大回归若命中 C1“无 restore”、P12C-A“精确八来源”或 D3“函数内直接 recorder”三个过时结构守卫，必须只按 §6 后三项更新为新阶段的更强等价证据；它们不是生产回退，也不得通过删断言、接受宽泛状态码或把 `revision_restore` 伪装成旧来源解决。
...
Grok 初版完成后，Codex 审查发现原“故障注入”测试没有实际调用迁移，路由守卫仍允许 POST 404/405/500，共享原语测试也没有真实拒绝伪造来源。第一轮 test-only 返修补成 DROP 旧表前抛异常、旧 DDL/八列逐值/索引/FK/旧 CHECK/临时表完整核对，并收紧路由必须 200 与伪造来源固定错误；该测试真实暴露 SQLAlchemy SQLite legacy transaction control 下 `CREATE TABLE` 未进入物理事务，失败后临时表残留。

第二轮受限返修仅在迁移创建临时表前执行 `UPDATE editor_state_revisions SET id = id WHERE 0`，由零行 DML 触发真实 DBAPI 事务，事务所有权仍属于 `ensure_schema_columns` 外层 `begin`。同时关闭双配额场景 `>=1`、写回漂移/recorder/两类裁剪/commit 失败只认裸 500 或可选错误体等假绿。最终精确 11 文件白名单、暂存区空、`py_compile` 与 `git diff --check` 均通过；Grok 最终回执=`msg_b0389af3509b4e4d93076334060141d8`，Codex 确认=`msg_1dc76dd27beb48ba8c367da1432ea0c6`。
