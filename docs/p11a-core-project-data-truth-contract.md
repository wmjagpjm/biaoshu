...
P11A 已按十文件白名单交付。`projectStore` 不再读取、写入或合并本地项目与演示项目，列表/详情/创建只走既有 `/api/projects*`；技术标、商务标列表和三条创建路径均使用真实空态、在途禁用与固定中文错误；商务标详情不再由 `mockBusinessProjects` 复活；查重/废标选择器失败为空且无未处理拒绝。

Grok 首版后，Codex 发现 E2E 只验证旧 `biaoshu.projects.v1` 单键值，无法阻断改写到 v2/cache 别名的假绿，遂下发仅限新 E2E 的返修。最终测试锁定项目元数据键族仅保留旧 v1 且原值不变，失败创建前后完整 local/session/cookie 快照全等，成功 pending 只含真实 projectId 与页面文件名；技术标/商务标 editor-state 既有本地键继续作为明确非目标精确放行。

Codex 独立通过：lint、build、P11A E2E 10 passed、认证/RBAC 11 passed、解析策略 6 passed、模板复用 1 passed、Chromium headless 单 worker 串行全量 E2E 155 passed、`git diff --check`。本包无后端改动，后端基线继续为 487 passed。
