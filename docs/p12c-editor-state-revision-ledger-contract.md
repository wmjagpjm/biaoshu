...
P12C-B-A 已证明浏览器 PUT 可记录 `browser_put`，B-B1 已证明九类任务可记录 `task`，B-B2 已证明五类商务 revise 可记录 `revise`，B-C1/C2 已分别证明个人 callback 与 P8C 票据 callback 可记录 `callback/local_parser`，B-D1 已证明融合 apply 可记录 `content_fuse_apply`。P12C-B 剩余包仍须分别审计 consume 与 checkpoint restore 的真实事务；不得把 D1 或可选来源参数视为二者已自动接入。P12C-C 只有在 B 全量闭环后才能冻结最小元数据列表、按需详情和是否复用 P12B-D restore；权限、正文出域、保留配额和恢复竞态必须重新审查。
