...
> **状态**：P12C-A 账本基础与 P12C-B-A 浏览器 PUT 接入均已实现、独立验收并推送。
> **拆包**：P12C-A（`daa8c43`/`226e1c1`）→ P12C-B-A 浏览器 PUT（`fbf93c0`/`acf3139`，已完成）→ P12C-B 其余写入事务逐包接入 → P12C-C 受限浏览/恢复另行冻结。
...
P12C-B-A 已证明浏览器 PUT 可同锁同事务记录 `browser_put`。P12C-B 剩余包仍须逐一审计任务/revise、两类 callback、content-fuse 与 checkpoint restore 的真实事务，证明每个成功写入与 transition 同成同败；不得把 A 包的可选来源参数视为其他调用者已自动接入。P12C-C 只有在 B 全量闭环后才能冻结最小元数据列表、按需详情和是否复用 P12B-D restore；权限、正文出域、保留配额和恢复竞态必须重新审查。
