...
Codex 独立验收：专项 **14 passed**、P12C-A/全状态/矩阵/基础 PUT 受影响回归 **107 passed**、后端串行全量 **680 passed**；只有 1 条既有 Starlette/httpx 弃用警告。`py_compile`、精确三文件白名单、工作树与暂存区 diff 检查全部通过。本包当时未接任务/revise/callback/content-fuse/checkpoint restore；后续 B-B1 已另包接入九类任务。revision 列表、详情、恢复或前端仍未实现。
