...
| 操作 | `finance` | `owner` / `bid_writer` | `hr` / `bidder` | 未登录 required | disabled |
| --- | --- | --- | --- | --- | --- |
| 查看财务报价列表、明细 | 允许 | 拒绝（`role_forbidden`） | 拒绝（`role_forbidden`） | 全局会话中间件拒绝（`401 auth_required`） | 拒绝（`role_forbidden`） |
| 修改报价或项目 | 拒绝 | 保持现有业务标权限 | 拒绝 | 无会话 | 保持现有兼容逻辑 |
| 访问通用项目、编辑器、文件、设置 | 拒绝（P10A 既有规则） | 保持既有规则 | 拒绝（P10A 既有规则） | 保持既有规则 | 保持既有兼容逻辑 |
...
4. 严格财务依赖只在 required 模式接受 `finance`；禁用模式固定 `role_forbidden`，无会话保持 P10A 全局中间件优先返回 `401 auth_required`，不污染个人兼容分支；
...
- 所有已登录非财务角色调用新接口返回 `403 role_forbidden`；required 未登录用户保持全局 `401 auth_required`；
