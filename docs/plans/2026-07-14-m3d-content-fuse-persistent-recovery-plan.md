...
不得修改共享 API/认证、router/AppShell、`useTechnicalPlanEditors`、`contentFuse.ts`、M3-A worker、Playwright 配置、依赖、后端、其他工作台或角色文件。若现有 `reloadFromApi` 接口不足，必须先发 `question`，不得擅自扩白名单。
