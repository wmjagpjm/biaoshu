...
> **状态**：已完成、独立验收并推送；冻结=`52bbabf`、实现=`82cc82e`。
...
Grok 在生产修改前运行新专项，结果 **7 failed / 3 passed**：7 项精确由缺少 `local_parser` recorder 触发，stale、null 与个人 callback 隔离三项自然通过。实现严格限定服务与新专项两文件，专项转为 **10 passed**；相关回归唯一失败来自 C1 阶段守卫仍断言 P8C 零 `local_parser`。

Codex 限定第一次返修只能修改新专项和 C1 个人 callback 专项：把阶段守卫更新为 P8C 精确 `n0+1`、唯一 `local_parser` after 与零 `callback`；把 recorder `added_count` 从宽松 `>=1` 收紧为精确 1；增加固定 401 helper 覆盖无效、缺失、过期和三类重放。生产文件哈希在返修前后保持 `a13a95dffbc5e2a841dc389ef742d4323daa5058`。Grok 返修后通过 **20/147 passed**。

Codex 独立通过三文件编译、专项 **20**、扩大受影响回归 **272**、后端串行全量 **721 passed**，只有 1 条既有弃用警告；精确三文件白名单、diff 与安全审查通过。实现已由 Codex 以 `82cc82e` 中文提交并推送，Grok 未提交或推送。
