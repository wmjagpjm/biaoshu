...
**补充结论：** 以上公开资料均不足以同时证明“当前可用的读取端点、允许的调用频率/重试、增量游标、凭据归属、允许存储字段和保留期”。因此 P9B 仍没有默认来源，且唯一允许的产品行为仍是本机 CSV/JSON 原子导入。

**恢复条件不变：** 用户确认一个来源，并提供该来源的正式读取 API/RSS/导出契约或平台书面接入资料，以及许可证/授权说明。若来源需要账号或 `userKey`，仅在用户自行持有、以环境变量注入且不入库/日志的前提下评估；若只有网站检索页、验证码、发布端接口或未公开页面请求，则直接排除。
...
- P9B 国内官方来源补充审计已完成：全国公共资源交易平台、中国政府采购网、天津/北京开放数据的公开资料均未形成可直接接入的完整读取契约；结论仅用于排除无授权实现，不构成对任一来源的接入或背书。
