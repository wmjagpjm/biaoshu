...
> **实施提示：** 本文仅冻结规划与验收边界。P9A 已按第 3 节的文字版式契约实现；P9B 的来源和受控读取边界已在 `docs/plans/2026-07-13-p9b-chnenergy-watch-plan.md` 单独冻结，仍须遵守该计划的任务拆分；P9C 未获得本文件第 3 节的产品决策前，禁止开始其代码实现、提交或推送。
...
上述文字规则已满足 P9A 开工闸门；P9B 已在 2026-07-13 由用户指定单一来源并冻结独立计划，P9C 的开工闸门仍未满足。
...
未确认前，现有本地 256 维哈希回退和可选 API 仅作为基线，不启动模型替换或批量索引重建。2026-07-14 的当前代码审计和待确认决策已单独记录在 `docs/plans/2026-07-14-p9c-semantic-retrieval-decision-gate.md`。
...
- P9C 已满足“P9B 先闭环”的顺序前提，但仍处于“等待产品决策”状态；当前基线审计见 `docs/plans/2026-07-14-p9c-semantic-retrieval-decision-gate.md`。离线/API、数据出域、模型版本、成本、降级、迁移/回滚和评测阈值未确认前不得写代码。
