...
**完成状态（2026-07-14）**：计划=`12e067f`、后端=`3dc600a`、前端=`254f8c7` 已由 Codex 审查、独立验收并推送。后端全量 364 项、前端 `lint` / `build`、P10F 定向 E2E 4 项和前端全量 E2E 均通过；后续只能在新契约中扩展人员业绩、证件、附件、审批、导出或 Word 写入。
...
3. 严格 `require_hr` 在 required 下精确匹配角色，并拒绝 disabled 与所有者隐式绕过；HR 写接口继续使用该依赖。标书制作者展示投影需要新增同样拒绝 disabled、且只依据精确 `member.role=bid_writer` 的严格读取依赖（`is_owner` 不可替代角色，但具有该角色的所有者可按角色通过），不能把既有个人版 `get_workspace_id` 语义误用为跨角色授权，也不能反向开放 `/api/hr/*`。
