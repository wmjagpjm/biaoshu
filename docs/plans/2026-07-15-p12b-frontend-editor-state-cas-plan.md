...
契约/计划由 Codex 先中文提交并推送；Grok 核心七文件实现经审查后，又受限更新三份受影响既有 E2E，由 Codex 独立验收、单独提交前端并更新路线图、联调和主交接。P12B-B 后只能进入 P12B-C 延迟写入围栏，禁止跳到检查点恢复；长期目标保持 active。

## 5. 实施与审查结果

- 首版收口了技术/商务全状态版本、两个项目保存链、guidance 单一写入者、固定冲突 UX 和矩阵三键 PUT，但 Grok 自报全量为 191 passed、4 failed、3 did not run，且实现文件早于新增测试，不满足 failure-first；Codex 未予验收。
- 第一次定点返修把两份真实矩阵 E2E 改为“陈旧 expected 优先命中全状态冲突”，给 HR 推荐桩补合法版本，并修复无矩阵明细的普通 409 伪造空矩阵冲突；同时把技术/商务串行断言加强为“第二请求 expected 精确等于第一响应版本”。
- 第二次机械返修删除两份矩阵 E2E 的 `.or(...)` 备用选择器和宽泛 `response.ok()` 断言，改用精确语义选择器与 HTTP 状态。
- Codex 独立通过 lint、build、技术 truth 28、商务 truth 18、矩阵 8、HR 推荐 4、融合确认 6、持久恢复 5 和单 worker 全量 201。技术 truth 首轮有 1 项在应用纯白页、首个 GET 为零时失败；该精确用例 1/1 与整文件 28/28 随后均通过，确认不是业务断言失败。
- 最终实现仍没有修改后端、共享 API/类型、矩阵组件算法、M3-D 或配置，也没有新增 restore。下一包必须先独立审计 P12B-C 的任务、callback、P8C 票据与 M3-D 延迟写入者。
