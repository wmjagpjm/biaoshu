...
> **状态：** 2026-07-19 冻结=`2f03b8c`；首轮实现与定向测试完成。后端全量仅剩旧删除测试未识别新列，已冻结最小测试边界修订，等待 Grok 单行修复与全量复跑。
...
首轮后端全量为 **2 failed / 1160 passed**：历史测试的裸 INSERT 因 ORM 缺少服务端默认失败，Grok 已在原生产白名单内补上 `server_default=0` 并定向通过；剩余唯一失败是 `test_p12f_revision_delete.py::test_delete_ast_and_source_guards` 的旧 ORM 字段精确清单。该文件冻结 SHA-256 为 `E1CE8CBA925022EC6202146879557DC570DE87FB73ADE78A68705BAC7CD1529E`，只授权在 `display_name` 后增加 `is_pinned` 一项，禁止任何其它变化。
