...
> **执行者：Grok**：初始严格十文件，第一次范围修订后十六文件，Codex 独立回归暴露精确表列基线后最终严格十七文件；先只写后端专项和 P12F-H history E2E 形成真实业务红测，再实现后端与技术/商务共用前端，并机械同步六份既有后端元数据合同测试及一份真实 SQLite 列集合基线；只自测并通过消息箱请求审查，不暂存、不提交、不推送。
>
> **状态：** 2026-07-18 已完成并推送。初始冻结=`0660145`，第一次范围修订=`0db935b`，第二次范围修订=`aca68b6`，实现=`b4338ba`；最终严格十七文件，Grok 未暂存、未提交、未推送。
...
1. 核对分支、远端、工作区、初始冻结/两次范围修订提交和十七文件哈希；完整阅读 H 契约及 P12F-A/F/G、history/restore/数据库迁移契约。
...
按契约第 9 节逐条串行运行后端专项、受影响回归、后端全量、P12F-H、history、checkpoint、技术/商务 truth、lint、build；Grok 完成后发送 `review_request`，报告红测、精确十七文件、结果、事务/迁移/六键/PATCH/迟到/零旁路证据、哈希、风险和未做项。

Codex 进行受限差异审查，必要时仅在十七文件内下发最小返修；独立重跑全部门和前端全量，通过后才中文提交、推送并更新契约、路线图、交接和联调文档。
...
1. failure-first：后端 **30 failed / 0 passed / 0 errors**；前端 **3 failed / 1 passed**，均为真实业务缺失而非收集、服务或语法错误。
2. Grok 完成实现后经历四轮受限返修；最终只改十七文件，关闭精确错误、rowcount、事务回滚、旧库迁移、严格六键、真实 CSRF、失败保值、Cookie/外网泄漏、A/B 迟到和 SQLite 精确列基线。
3. Codex 后端独立结果：**30/240/132/1140 passed**。第二次范围修订依据的首轮回归失败按事实保留为 **1 failed / 131 passed**。
4. Codex 前端独立结果：**5/52/51/28/18/315 passed**，lint/build/py_compile/diff/白名单/哈希/静态禁区均通过。首轮全量的单个既有 P8B 瞬时导航失败已独立通过并在无代码变更的完整第二轮 **315/315** 中闭合。
5. 实现提交 `b4338ba` 已推送；本计划后续只作完成记录，不再作为 Grok 待执行任务。
