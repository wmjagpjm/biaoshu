...
**下一步**：P13-A 与 P12F-D/E-A/E-B/F-A/F-B/G-A/G-B/H 均已完成。只读审计选择 **P12F-I 修订名称与可见内容联合搜索**：冻结基线=`060191e`，严格四文件，在完整验证 20 条候选后把已校验展示名称与既有可见快照字段做联合字面匹配，并准确更新共用前端文案。固定/置顶仍须另立裁剪保护与配额契约，不得混入。
...
**P12F-I 已审计并冻结**：契约=`docs/p12f-revision-display-name-search-contract.md`，计划=`docs/plans/2026-07-18-p12f-revision-display-name-search-plan.md`，冻结基线=`060191e`。严格四文件复用既有 POST/search SQL 七列与六键响应；后端只在完整验证全部候选后，以同一 NFKC+casefold 规则联合匹配非空名称和既有可见字段，同一行只返回一次且保持倒序。前端只修正“名称或内容搜索”入口/活动/失败/空态文案并补技术/商务 E2E；路由、Schema、API、数据库、索引、候选 20 与所有写能力冻结。
