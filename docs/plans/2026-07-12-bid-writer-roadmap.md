...
- 轻量解析和 MinerU Markdown 回传已经可用；包 8 MVP 可插拔调度已推送（`6db1586`：默认 `lightweight` + 测试 fake），P8B 已让 `parseStrategy` 驱动技术标/商务标的 `light/local/ask` 动作，P8C 已补 required 模式 10 分钟单项目单次回传票据；P8D 已提供离线调用本机既有 `mineru.exe` 并受控回传的标准库助手。**MinerU CLI/模型仍需人工准备，Docling 未接**。
