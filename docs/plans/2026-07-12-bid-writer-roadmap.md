...
**P12D-B 技术/商务共用前端对比入口已冻结**：只修改既有 API 封装、共享修订面板和修订历史 E2E。保留“查看摘要”，新增按需“与当前对比”；严格解析四键响应、13 键有序无重复子集和两侧六项摘要，只显示固定中文字段标签。摘要/比较/恢复互斥，项目、修订、折叠、刷新和恢复均以请求代次隔离迟到；新增且仅新增 3 个 E2E，failure-first 预期 **3 failed / 21 passed**，最终目标历史专项 **24 passed**。完整契约见 `docs/p12d-revision-comparison-frontend-contract.md`，实施计划见 `docs/plans/2026-07-17-p12d-revision-comparison-frontend-plan.md`。

**下一步**：提交并推送 P12D-B 冻结文档，通过本地消息箱交 Grok 三文件 failure-first 实现；Grok 只实现和自测，Codex 负责 parser/状态机/竞态反假绿审查、检查点与双真值受影响回归、lint/build、前端单 worker 零重试全量、中文提交和文档闭环。正文 diff、任意历史两两比较、删除、搜索、分页和多人协作继续不进入 B 包。
