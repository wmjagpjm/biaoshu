...
**P12B 安全恢复审计已拆为四道门**：P12B-A 服务端规范全状态版本与可选 CAS；P12B-B 技术/商务前端 expected 版本与整态冲突 UX；P12B-C 任务、个人 callback、P8C 票据和 M3-D 的迟到写入围栏；P12B-D 恢复前安全检查点与原子恢复。P12B-A 已完成并推送：计划/契约=`0b55c30`、实现=`780cc82`、闭环=`bf3e86a`；Codex 独立通过专项 19、定向 12、原回归 104、后端串行全量 537 passed。P12B-B 也已完成并推送：契约/计划=`0636302`、实现=`473e823`；技术主 hook、商务 hook、guidance 与矩阵合并已收口为两个项目串行队列，所有浏览器 editor-state PUT 使用最新 expected，固定全状态冲突保留本地并只允许显式全量重载。两轮返修关闭全量旧测试、普通 409 伪矩阵冲突和 E2E 宽泛断言；Codex 独立通过技术 28、商务 18、矩阵 8、HR 推荐 4、融合 6+5、前端全量 201。P12B-A 缺 expected 的非浏览器旧写入和 P12B-C 延迟写入者仍未收口，任何阶段都不得提前增加 restore 端点或按钮。Word `structure` 因缺少容器/跨页视觉决策继续不接线；外部来源和真实语义调优不得合包。
