...
**P12B 安全恢复审计已拆为四道门**：P12B-A 服务端规范全状态版本与可选 CAS；P12B-B 技术/商务前端 expected 版本与整态冲突 UX；P12B-C 任务、个人 callback、P8C 票据和 M3-D 的迟到写入围栏；P12B-D 恢复前安全检查点与原子恢复。P12B-A 已完成并推送：计划/契约=`0b55c30`、实现=`780cc82`；共享 P12A 精确 13 键算法，GET/PUT 返回 `stateVersion`，可选 `expectedStateVersion` 在项目锁后比较，全状态冲突优先于矩阵冲突。两轮返修消除锁后重复读取、提交后假失败、`updatedAt` 重读漂移和既有非有限报价兼容断裂；Codex 独立通过专项 19、定向 12、原回归 104、后端串行全量 537 passed。下一包必须先冻结并实现 P12B-B 前端 CAS；P12B-A 缺 expected 的旧写入仍可覆盖，任何阶段都不得提前增加 restore 端点或按钮。Word `structure` 因缺少容器/跨页视觉决策继续不接线；外部来源和真实语义调优不得合包。
