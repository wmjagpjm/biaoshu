...
#### V1-J：lightweight 无有效正文质量门（已完成）

冻结=`1e33f1b`、测试=`3917c3a`、实现=`f9562a5`。全空 PDF/DOCX/TXT/MD 统一 failed，零 editor-state/project step 写入；混合 PDF 保留空页提示并继续 success。知识库失败显示固定中文、物理清除旧 chunk、保留文档与文件。严格两生产+两测试文件，启动诊断、OCR、依赖、前端和数据库均未混入。

Failure-first 为新专项 **6 failed / 4 passed**、V1-D **2 failed / 8 passed**；Codex 最终独立 V1-J/V1-D/代表回归 **10/10/38 passed**，编译、diff-check、严格四文件和冻结 blob 通过。后续只读审计已选定并冻结 V1-K；复杂跨页版式、OCR 自动安装、在线热备与 V2/V3 继续后置。

#### V1-K：静默启动诚实诊断（已完成）

A/B 审计与双确认一致：五入口统一到单一 UTF-8 BOM PowerShell 真源；默认仍 Hidden、无浏览器、无 pause；端口必须确认本仓归属并通过固定回环就绪探测；结果只写有限脱敏状态侧车，显式诊断只读。Stop、业务代码、依赖、真实端口和数据均未触碰。冻结=`8f0c137`，测试=`8f0c366`/`1fdea4c`/`997b57c`，实现=`cd4102f`。
...
A/B 审计与 Q1 九项决策均独立 YES。唯一拓扑为“内网浏览器 → 显式 RFC1918 IPv4 上的 Vite 5173 → 回环 FastAPI 8000”；默认仍 loopback，LAN 必须显式 profile/host、强制 required 并先证明后端 `authRequired=true` 且 `bootstrapped=true`。前端保持同源 `/api`，不扩 CORS、不改 Strict Cookie、不暴露 OpenAPI/backend，不使用通配 bind/Host。

V1-K 七键状态和单次原子替换保持；防火墙只文档化 Private/LocalSubnet/5173 的管理员手工步骤。IPv6、自动防火墙、WAL/PostgreSQL、HTTPS/公网、Docker 与 V2/V3 后置。契约=`docs/v1l-trusted-lan-access-contract.md`，计划=`docs/plans/2026-07-22-v1l-trusted-lan-access-plan.md`；测试冻结=`ea01c48`，夹具修正=`7c9266e`/`b0f197e`，实现=`10b5f3e`。
