...
阶段 0/1/2、阶段 3 M3-A 至 M3-D、阶段 4 **包 5** 至 **包 8/P8B/P8C/P8D/P8E**、P9A/P9B/P9C/P9D、阶段 5 P10A 至 P10K、**P11A/P11B/P11C 三个真实数据收口包**，以及 **P12A/P12B-A/P12B-B/P12B-C** 均保持已交付。P8E 完整契约见 `docs/p8e-docling-local-helper-contract.md`，实施与独立验收记录见 `docs/plans/2026-07-15-p8e-docling-local-helper-plan.md`。
...
**P12B 安全恢复审计拆为四道门**：P12B-A 服务端规范全状态版本与可选 CAS；P12B-B 技术/商务前端 expected 版本与整态冲突 UX；P12B-C 任务、个人 callback、P8C 票据和 M3-D 的迟到写入围栏；P12B-D 恢复前安全检查点与原子恢复。A 已完成：计划/契约=`0b55c30`、实现=`780cc82`、闭环=`bf3e86a`，后端全量 537。B 已完成：契约/计划=`0636302`、实现=`473e823`，前端全量 201。C 也已完成：冻结=`b5a9d90`、C1=`0c8fc77`、C2=`f3c05ae`、C3=`59fcd50`；三批依次封住任务/revise、callback/P8C、M3-D，Codex 最终独立通过 C3 后端专项 62、后端全量 570、相关 E2E 48、前端全量 212。C3 两轮返修修复成功重读后下一编辑丢失，并把四种不确定响应的本地保留、零重试、零 PUT 与零异常改为逐轮证据。下一主线只能先只读审计并冻结 P12B-D；当前仍不得增加 restore 端点/按钮，也不得合入历史浏览、删除、自动检查点或强制覆盖。Word `structure` 因缺少容器/跨页视觉决策继续不接线；外部来源和真实语义调优不得合包。
