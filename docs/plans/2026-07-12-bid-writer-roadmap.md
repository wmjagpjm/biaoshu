...
**P12B 安全恢复审计已拆为四道门**：P12B-A 服务端规范全状态版本与可选 CAS；P12B-B 技术/商务前端 expected 版本与整态冲突 UX；P12B-C 任务、个人 callback、P8C 票据和 M3-D 的迟到写入围栏；P12B-D 恢复前安全检查点与原子恢复。P12B-A 已完成并推送：计划/契约=`0b55c30`、实现=`780cc82`、闭环=`bf3e86a`；Codex 独立通过专项 19、定向 12、原回归 104、后端串行全量 537 passed。P12B-B 已完成只读审计并冻结契约=`docs/p12b-frontend-editor-state-cas-contract.md`、计划=`docs/plans/2026-07-15-p12b-frontend-editor-state-cas-plan.md`：实际浏览器写入者是技术主 hook、商务 hook 和独立 guidance hook，且矩阵合并绕过普通队列；七文件前端包必须消除 guidance 旁路，让全部 PUT 携带最新 expected、同项目串行，全状态 409 阻断自动保存并只允许显式全量重载。P12B-A 缺 expected 的旧写入和 P12B-C 延迟写入者仍未收口，任何阶段都不得提前增加 restore 端点或按钮。Word `structure` 因缺少容器/跨页视觉决策继续不接线；外部来源和真实语义调优不得合包。
