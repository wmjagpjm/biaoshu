...
- P11A 审计确认项目元数据生产入口仍会把 mock/localStorage 当成服务端项目的成功回退；本包先把技术标/商务标列表、详情、创建与查重/废标选择器收口为 `/api/projects*` 单一真值。它不改 editor-state 本地备份、知识库降级、未挂载首页、后端或角色权限。
...
阶段 0/1/2、阶段 3 M3-A 至 M3-D、阶段 4 **包 5** 至 **包 8/P8B/P8C**、P9A/P9B/P9C/P9D 与阶段 5 P10A 至 P10K 均保持已交付。新一轮只读审计已选择 **P11A 核心项目真实数据收口**：服务端 `/api/projects*` 是技术标/商务标项目列表、详情与创建的唯一权威，真实空数组保持空，失败显式且不得由 mock、`biaoshu.projects.v1` 或本地假 ID 复活。完整契约见 `docs/p11a-core-project-data-truth-contract.md`，纯前端十文件白名单与串行验收见 `docs/plans/2026-07-14-p11a-core-project-data-truth-plan.md`。

下一动作是先提交并推送 P11A 契约/计划，再按 `docs/HANDOFF-next.md` §3.1 后台静默派发 Grok；Grok 只实现和自测，Codex 审查、独立验收、中文提交与文档闭环。P11A 不得搭车修改 editor-state 本地备份、知识库 localStorage、后端、角色权限、通用版本库、MinerU/Docling、模型门、Word 布局、外部标讯来源、完整财务审计、附件核验或公网生产部署。
