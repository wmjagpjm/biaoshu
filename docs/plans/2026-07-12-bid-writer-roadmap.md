...
**P12D-A 修订与当前状态差异摘要 API 已冻结**：只读组合服务端当前权威 13 键与 P12C-C1 已校验目标修订，逐字段用共享规范 JSON 比较；只返回变更字段名与两侧六项有界计数摘要，不返回正文、字段值、ID 或版本。严格四文件白名单，前端入口、正文 diff、任意历史两两比较、删除、搜索、分页和多人协作均不在本包。完整契约见 `docs/p12d-revision-current-diff-summary-contract.md`，实施计划见 `docs/plans/2026-07-16-p12d-revision-current-diff-summary-plan.md`。

**下一步**：把 P12D-A 四文件 failure-first 任务通过本地消息箱交给 Grok；Grok 只实现和自测，Codex 负责反假绿审查、P12C 受影响回归、后端全量、中文提交和文档闭环。P12D-A 验收后再独立冻结 P12D-B 技术/商务共用前端对比入口。
