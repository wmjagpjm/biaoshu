...
> **状态**：阶段 0/1/2 已完成；阶段 3 **已完成并推送**（M3-A=`5d37dba`，M3-B=`e2e5d04`）；阶段 4 **包 5** 已推送（`460097a`）；**包 6** 已推送（`1289c92` 实现响应矩阵源分页调用）；**包 7** 已推送（`2c7b3e0` 实现响应矩阵字段级三方合并）；**包 8** 调度 MVP 已推送（`6db1586`），其后 **P8B 解析策略接线**已完成（计划=`f662674`、后端=`0994cc8`、前端=`80d2579`；MinerU 仅外置 callback、Docling 未接）；**包 9A 已实现并完成完整独立验收**（`c1ff160`，含 WPS 技术标/商务标实际渲染抽检）。阶段 5 已完成 P10A 身份/RBAC、P10B/P10C strict 财务、P10D strict 人力人员资质素材卡、P10F strict 人力项目团队推荐快照（计划=`12e067f`，后端=`3dc600a`，前端=`254f8c7`）、P10E strict 投标人匿名合规预览（计划=`26f7e40`，后端=`1b6ccf3`，前端=`37cf835`）、P10G strict 投标人项目级合规统计（计划=`26b43ea`，后端=`c3cf8b4`，前端=`d5656cc`）与 **P10H strict 人员业绩素材卡**（计划=`7694843`，后端=`6c76d80`，前端=`4eb8a14`）。
...
财务、人力、投标人以及账号登录、角色权限、协作审批属于平台演进阶段。当前已受限交付 P10A 身份底座、P10B/P10C 财务、P10D 人力资质卡、P10F 人力团队推荐快照、P10H 人员业绩卡、P10E 投标人匿名汇总和 P10G 投标人单项目统计；任何新增角色能力仍须独立冻结数据、权限和审计边界，禁止绕过现有单 workspace 约束。
...
- 轻量解析和 MinerU Markdown 回传已经可用；包 8 MVP 可插拔调度已推送（`6db1586`：默认 `lightweight` + 测试 fake），后续 P8B 已让 `parseStrategy` 驱动技术标/商务标的 `light/local/ask` 动作（计划=`f662674`、后端=`0994cc8`、前端=`80d2579`）；**真实 MinerU 仍仅外置 callback，Docling 未接**。
...
| 人力 | P10D 已交付当前空间人员资质素材卡；P10F 已交付由 strict `hr` 维护的技术标团队推荐快照，strict `bid_writer` 仅可按需读取最小投影；P10H 已交付独立人员业绩素材卡 | 不看完整标书和定价细节；不收集证件号、联系方式、附件或外链；不以所有者身份绕过 strict `hr` 或 strict `bid_writer` |
...
阶段 0/1/2/3 已完成并推送（M3-A=`5d37dba`，M3-B=`e2e5d04`）。阶段 4 **包 5** 已推送（`460097a`）；**包 6** 已推送（`1289c92`）；**包 7** 已推送（`2c7b3e0`）；**包 8** 调度 MVP=`6db1586`，后续 **P8B** 解析策略接线已完成（计划=`f662674`、后端=`0994cc8`、前端=`80d2579`；MinerU 仍仅外置 callback、Docling 未接）。阶段 5 的 P10A 身份/RBAC、P10B 财务报价只读投影、P10C 财务成本草案/毛利快照、P10D 人员资质素材卡、P10F 团队推荐快照、P10E 匿名合规汇总、P10G 单项目合规统计与 P10H 人员业绩素材卡均已完成。下一包仍须从剩余主线中独立选择并先冻结契约；可选方向包括人力附件/证件校验、投标人矩阵明细/版本/结果跟踪、财务税务/审批/导出，或真实解析器生产部署。禁止沿用 P10C/P10D/P10E/P10F/P10G/P10H 路径顺带扩权。
