...
阶段 0/1/2、阶段 3 M3-A 至 M3-D、阶段 4 **包 5** 至 **包 8/P8B/P8C**、P9A/P9B/P9C/P9D、阶段 5 P10A 至 P10K 与 **P11A 核心项目真实数据收口**均保持已交付。P11A 以服务端 `/api/projects*` 作为技术标/商务标项目列表、详情与创建的唯一权威，真实空数组保持空，失败显式且不得由 mock、`biaoshu.projects.v1` 或本地假 ID 复活；计划=`70a2dc7`、前端=`b0a86e4`，Codex 独立通过前端全量 155。完整契约与验收见 `docs/p11a-core-project-data-truth-contract.md`，实施记录见 `docs/plans/2026-07-14-p11a-core-project-data-truth-plan.md`。

下一动作是再次只读审计未实现主线，在真实 MinerU/Docling 外置部署治理、P9C 固定模型运行时门、Word `structure`/整章布局、合法外部标讯来源、editor-state 本地备份收口或其他明确用户价值中只冻结一个最小包。不得以 P11A 名义搭车修改后端、角色权限、通用版本库、完整财务审计、附件核验或公网生产部署。
