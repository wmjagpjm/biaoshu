...
财务、人力、投标人以及账号登录、角色权限、协作审批属于平台演进阶段。当前已受限交付 P10A 身份底座、P10B/P10C 财务、P10D 人力资质卡、P10F 人力团队推荐快照、P10H 人员业绩卡、P10I 人员资质到期提示、P10E 投标人匿名汇总和 P10G 投标人单项目统计；任何新增角色能力仍须独立冻结数据、权限和审计边界，禁止绕过现有单 workspace 约束。
...
**验收**：真实本机技术标/商务标导出 E2E、非法结果结构收敛、下载不阻断、后端项目图片定向回归、前端 lint/build 和单 worker 全量 E2E。
...
阶段 0/1/2/3 已完成，M3-A=`5d37dba`、M3-B=`e2e5d04`、M3-C 计划=`c63310f`/实现=`b8ff605` 均已推送。阶段 4 **包 5** 至 **包 8/P8B** 均保持已交付，阶段 5 的 P10A 至 P10I 均已完成。下一包已冻结为 P9D：只在技术标/商务标导出页展示既有后端 `imageWarnings`，且不阻断下载；等待 Grok 受限实现和 Codex 独立验收。后续人力附件/真实证件核验、投标人明细、财务扩展或真实解析器部署仍须各自另立契约。
