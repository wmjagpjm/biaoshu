...
- 当前已具备中标内容模板资产化、多模板/卡片融合、差异确认、最近 10 条有限修订与受限恢复、统一卡片库和受限角色数据域。真实剩余缺口集中在多人协作与更完整版本治理、真实 MinerU/Docling 生产部署、P9C 固定模型运行时门与真实语义调优、Word `structure`/整章布局、更多合法外部标讯来源及完整生产部署治理。导出版式模板与中标内容模板是两个不同概念，后续不得混用术语。
...
**P9C-R1 固定离线模型运行时门已冻结**：只允许固定 `BAAI/bge-small-zh-v1.5`、提交 `26478543676740eb665f803ca07f3f7f478857c8`、10 个必需文件与 safetensors SHA-256。显式准备 CLI 是唯一联网路径；生产加载、搜索、状态、启动、上传、重索引和预检全部严格本地。当前真实状态仍是依赖缺失、缓存为空、预检 `model_unavailable`/退出码 2；不得在真实验收前改写为已就绪。完整契约见 `docs/p9c-fixed-model-runtime-gate-contract.md`，实施计划见 `docs/plans/2026-07-16-p9c-fixed-model-runtime-gate-plan.md`。

**下一步**：把 P9C-R1 六文件 failure-first 任务通过本地消息箱交给 Grok；Grok 只实现和做无网络自动化自测，Codex 独立审查后才安装固定依赖、显式下载约 91.91 MiB 制品并运行真实预检。不得合入真实用户语料调优、其他模型/GPU/在线 embedding、MinerU/Docling、Word `structure`、外部标讯、版本治理或多人协作。
