...
**下一步**：先对剩余主线做一次全局只读盘点并冻结单一最小包，不从 C3 直接扩展。盘点至少比较：P9C 固定模型运行时门与真实语义验收、MinerU/Docling 生产部署、Word `structure`/整章布局、其他合法外部标讯来源、多人协作/完整版本治理和生产部署治理；按用户价值、外部依赖、可独立验收性和风险排序后再选一项。删除、diff、搜索、跨项目历史、超出最近 10 条的完整历史/保留策略不得借 C3 顺手合入。
