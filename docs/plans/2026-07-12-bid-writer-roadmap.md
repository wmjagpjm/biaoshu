...
**下一步**：先做新一轮全局只读审计，在真实 MinerU/Docling 生产部署、Word `structure`/整章布局、合法外部标讯来源和有限版本治理扩展中选择一个依赖清楚、可 failure-first、可独立验收的最小包。P9C 真实用户语料评测涉及用户数据，其他模型/GPU/在线 embedding 涉及新的供应链与运行时边界，均不得默认并入下一包。
