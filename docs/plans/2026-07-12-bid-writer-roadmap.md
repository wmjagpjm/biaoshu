...
**下一步**：P13-A 与 P12F-D 均已完成；只读审计已选择 P12F-E-A 修订时间范围筛选后端基础。它只在既有页 GET 增加严格 UTC 包含下界/排除上界，并以 `esrc3` 绑定时间范围、可选来源和分页位置；Grok 严格三文件 failure-first，Codex 独立审查和后端全量验收。前端日期控件留给 E-B，正文搜索、来源多选、命名/固定/删除、跨项目历史、多人协作和 SSE 扩展不得合包。
...
**P12F-E-A 已审计冻结，待实现**：契约=`docs/p12f-revision-time-range-filter-contract.md`，计划=`docs/plans/2026-07-18-p12f-revision-time-range-filter-plan.md`。A 包只改后端路由、history service 和新专项测试：严格 `createdFrom` 包含下界、`createdBefore` 排除上界，任一边界存在时以 `esrc3 {b,f,i,s,t}` 绑定范围、可选来源和末条位置；无范围的 V1/V2 必须完全兼容。前端日期控件、时区交互、正文搜索、来源多选及任何写能力均留后续独立包。
