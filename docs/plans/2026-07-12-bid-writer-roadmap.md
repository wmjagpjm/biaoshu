...
> **状态**：阶段 0/1/2 已完成；阶段 3 **已完成并推送**（M3-A=`5d37dba`，M3-B=`e2e5d04`）；阶段 4 **包 5** 已推送（`460097a`）；**包 6** 已推送（`1289c92` 实现响应矩阵源分页调用）；**包 7** 已推送（`2c7b3e0` 实现响应矩阵字段级三方合并）；**包 8** MVP 可插拔解析调度**本批实现待审查**（MinerU 仅外置 callback、Docling 未接、`parseStrategy` 未接线）；**包 9** 未开始。
...
- 轻量解析和 MinerU Markdown 回传已经可用；包 8 MVP 增加可插拔调度（默认 `lightweight` + 测试 fake），**真实 MinerU 仅外置 callback，Docling 未接，`parseStrategy` 未接线**。
...
**未做（包 9）**：交付增强（Word 精细版式/外部标讯/embedding）——须独立 task。

#### 功能包 8：可插拔解析调度（MVP）

**状态**：**本批实现待 Codex 审查**（未 commit/push）。计划见 `docs/plans/2026-07-13-pluggable-parse-plan.md`。
...
阶段 0/1/2/3 已完成并推送（M3-A=`5d37dba`，M3-B=`e2e5d04`）。阶段 4 **包 5** 已推送（`460097a`）；**包 6** 已推送（`1289c92` 实现响应矩阵源分页调用）；**包 7** 已推送（`2c7b3e0` 实现响应矩阵字段级三方合并）。阶段 4 **包 8** MVP 可插拔解析调度**本批实现待审查**（MinerU 仅外置 callback、Docling 未接、`parseStrategy` 未接线）；**包 9**（交付增强）仍未开始。多角色（阶段 5）仍不开始。
