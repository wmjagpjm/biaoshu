...
> **状态（2026-07-13）**：**已实现并完成完整独立验收**；实现提交 `c1ff160`（实现P9A最小标题左栏）。自动化检查与 WPS 实际渲染抽检均已通过。
...
| 项 | 冻结规则 |
|---|---|
| 作用对象 | 实际大纲树中没有下级节点的叶节点，以及技术标章节 Markdown、商务标 Markdown 标题序列中没有更深级标题的叶标题；固定区块和章节标题不参与 |
...
“最小标题”是大纲节点或 Markdown 标题序列中的末级标题，而不是固定的第六级标题：只有三级标题的文档可作用于三级叶子标题；出现四级标题时该分支的四级标题才作用。标题后无正文也可视作叶子标题。项目概述、分析项、解析摘录、目录/正文容器、章节标题、全局事实和响应矩阵等固定区块不参与此判定。实现必须在写入 Word 前完成层级判定，禁止因流式写入把所有标题都误加左栏。
...
3. 在既有标题段落属性顺序规则内写入 `w:pBdr/w:left`，将叶子标题的 `w:space` 调整为 6；关闭任一开关时不得写入该左栏属性。
...
| 只有一级大纲节点或 Markdown 标题 | 一级叶子标题可有左栏；既有分页开关仍按原规则生效 |
...
## 7. 实施与验收证据（2026-07-13）

- `min_heading_left_enabled` 已兼容 snake_case/camelCase；仅与 `heading_border.enabled` 同时开启时生效。
- 实际大纲叶节点与 Markdown 叶标题复用原 `w:pBdr` 的 `w:left`，升为 `w:sz=18`、`w:space=6`；父标题保持原 `w:sz=8`、`w:space=4`。
- 项目概述、分析项、解析摘录、目录/正文容器、章节标题、全局事实和响应矩阵均不视为叶子标题；未接线 `structure`，未写入 `w:pgBorders`。
- 前端表单已提供“最小标题左栏”开关，预览只给示例叶子标题显示同语义强调线。
- 独立验证：`backend .venv\Scripts\python.exe -m pytest -q tests/test_export_heading_border.py` 为 **13 passed**；后端全量 `pytest -q` 为 **175 passed**（1 条既有 Starlette/httpx 弃用警告）；`frontend npm run lint`、`npm run build`、`git diff --check` 均通过。
- Word 回归测试会解压 `.docx` 并检查 `word/*.xml`，确认无 `w:pgBorders`；另覆盖技术标、商务标、大纲父/叶节点和 Markdown 父/叶标题。
- 实际编辑器抽检：通过真实 API 导出技术标和商务标 `.docx`，在本机 WPS `12.1.0.26895` 中打开检查。技术标大纲父节点“总体设计”和 Markdown 父标题“部署拓扑”保持普通边框，叶子标题“部署架构”“机房节点”“售后保障”显示加粗左边线；商务标文档标题保持普通边框，叶子小节“二、资格响应”显示加粗左边线；两份文档均无整章页框，中文内容与分页可正常打开。
