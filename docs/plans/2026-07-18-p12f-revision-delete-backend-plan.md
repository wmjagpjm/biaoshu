...
> **执行者：Grok**：原冻结为严格四文件，先只新增专项形成真实业务红测，再实现独立删除服务与 DELETE 路由；回归阶段由 Codex 识别旧 history 守卫冲突并受限修正为五文件。Codex 负责独立规划、受限审查、独立验收、中文文档闭环和协作分支推送。
>
> **状态：** 2026-07-18 已完成并推送。冻结=`c176cb5`，实现=`d2555d4`；原四文件边界因旧 history 写路由守卫与新 DELETE 必然冲突，受限修正为五文件且仅允许修改该守卫单一函数。
...
按契约第 7 节从专项、受影响回归、auth 到后端全量逐组串行执行，禁止 xdist；再运行 `py_compile`、`git diff --check`、精确五文件、空暂存区、实体结构与禁区扫描。第五文件只能修改旧 history 写路由守卫的单一函数。

Grok 通过消息箱发 `review_request`，必须包含真实 failure-first、精确文件清单、每组结果、事务/SQL/权限/零副作用证据、最终哈希、风险和未做项；不得暂存、提交或推送。Codex 仅允许冻结范围内最小返修，独立验收后才可中文提交和推送。

实际执行中，首轮并行跑 restore/auth 的结果已废弃；Codex 识别并修正冻结漏项后，将返修限制为路由/实体字节冻结、删除服务、新专项，以及旧 history 文件的单一守卫函数。两轮静态补充进一步关闭 search 子集、Project SELECT 额外参数与未包裹 `Session.query` 三项假证据。

最终 Grok 与 Codex 分别串行通过专项/history+cursor+search/restore+retention/auth/全量 **14/71/93/39/1110 passed**；Codex 全量耗时 1620.30 秒，仅 1 条既有弃用告警。`py_compile`、`git diff --check`、精确五文件、空暂存区及五个 SHA-256 均通过，实现提交=`d2555d4`。详细消息链、哈希与冻结修正见契约第 8 节。

## 6. 未做

不做前端确认/重载、多选/批量/范围删除、软删除/回收站/撤销、自动清理、命名/固定/标签、检查点删除、审计报表、导出/分享、跨项目历史、多人协作、SSE/WebSocket、表/迁移/依赖/配置变更。P12F-G-B 现在可以只读审计，但必须另立契约、计划和白名单后再交 Grok。
