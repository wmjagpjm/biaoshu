...
> **交接日期**：2026-07-20（P13-C 快速第一版已完成；P13-D1 已实现并完成 test-only 返修，正等待 Codex 最终验收，代码尚未提交）
...
> **本地状态**：只允许分支 `collab/grok-code-codex-review`；P13-D1 冻结基线=`3132684`。当前存在未暂存的 19 个生产文件、1 个既有测试修改和 1 个未跟踪专项测试，禁止 pull/reset/checkout/stash。操作级交接见 `docs/HANDOFF-p13d1-in-progress.md`。
...
当前执行包：P13-D1 editor-state 修订操作者可信账本；契约=`docs/p13d1-editor-state-revision-actor-ledger-contract.md`，计划=`docs/plans/2026-07-20-p13d1-editor-state-revision-actor-ledger-plan.md`，在途操作手册=`docs/HANDOFF-p13d1-in-progress.md`。Grok 已实现并完成 test-only 返修，最终 review=`msg_de747706fcb64a188eef50d77e29d451`；专项 17 passed、精确 schema 1 passed，但 Codex 尚未最终验收，代码未暂存/提交/推送。
下一步：先审查返修后的专项与 19 个生产文件冻结哈希；最小修复既有 `test_no_commit_rollback_refresh_project_lock` 未恢复 SQLite PRAGMA 的顺序污染，再由 Codex 串行验收迁移、真实 worker、九路传播、无泄漏和原子回滚。D1 验收推送后立即冻结 P13-D2 当前操作者用户名展示。presence/SSE/WebSocket、按 actor 搜索、Word 整章版式、真实解析模型/语料与新增外部来源仍是独立后续项。
...
| 当前版本协作元数据 P13-B/C 与 P13-D1 在途 actor 账本 | `api/projects.py`、`api/schemas.py`、`services/editor_state_revision_service.py`、`services/task_service.py`、`core/database.py`、`tests/test_p13c_current_revision_source.py`、`tests/test_p13d1_revision_actor_ledger.py` | **齐（D1待最终验收）** | P13-B/C 已交付 UTC 时间与九类来源；P13-D1 顶注释保持四字段，当前 19 个生产文件已实现但未提交，专项 17 passed，禁止提前写成完成 |
...
| 优先级 | 项 | 现状 |
|--------|----|------|
| 当前 P0 | P13-D1 修订操作者可信账本 | Grok 已实现并完成一次 test-only 返修；19 个生产文件、1 个既有测试和 1 个新专项仍未暂存。专项 **17 passed**、schema **1 passed**；Codex 尚未最终验收。唯一顺序失败已定位为既有测试未恢复 SQLite PRAGMA，详见 `docs/HANDOFF-p13d1-in-progress.md` |
| 紧随 P0 | P13-D2 当前版本操作者用户名展示 | 尚未冻结/实现；必须先完成 D1。只在最新修订与当前版本精确匹配时解析用户名，不公开内部用户 ID，不混入历史 actor 搜索、presence 或实时协作 |
...
**粗估**：技术标 ~93%；商务 ~80%；合规工具可用；内网多人 ~30%；公网 SaaS ~15%。该估算是产品覆盖率，不代表 P13-D1 已验收。
...
4. 当前先完成 P13-D1 在途审查：按 `docs/HANDOFF-p13d1-in-progress.md` 修复精确 PRAGMA 测试污染、完成分级验收、提交推送与文档闭环；随后独立冻结 P13-D2 当前操作者用户名展示。P12I/J/K/L、P12M/N 与 P13-A/B/C 均已完成。D2 之后优先选择不依赖外部制品的工作空间切换/成员可见性，再分别设计 presence 和事件协议；跨项目版本、完整时间线与实时协作不得合包。
...
| **docs/HANDOFF-p13d1-in-progress.md** | **P13-D1 未提交实现的操作级交接；含脏文件、消息 ID、测试、已知 PRAGMA 污染和精确续跑命令** |
...
- **当前最高优先级 P13-D1 未完成**：冻结=`3132684`；当前未暂存差异为 19 个生产文件、`test_editor_state_revisions.py` 与未跟踪 `test_p13d1_revision_actor_ledger.py`。Grok 最终 test-only review=`msg_de747706fcb64a188eef50d77e29d451`，专项 17 passed、schema 1 passed；Codex 尚未最终验收、提交或推送。完整在途状态见 `docs/HANDOFF-p13d1-in-progress.md`。
- **当前唯一已定位测试问题**：既有 `test_no_commit_rollback_refresh_project_lock` 污染连接级 `PRAGMA ignore_check_constraints=1`，导致其后 P13-C 用例结束守卫失败；P13-C 单独通过。下一会话应最小 test-only 恢复 PRAGMA，禁止删除 P13-C 守卫或修改生产代码掩盖污染。
...
## P13-D1 修订操作者可信账本（在途，未提交）

冻结=`3132684`，契约=`docs/p13d1-editor-state-revision-actor-ledger-contract.md`，计划=`docs/plans/2026-07-20-p13d1-editor-state-revision-actor-ledger-plan.md`，操作级交接=`docs/HANDOFF-p13d1-in-progress.md`。Grok 已实现两张表的可空 actor、SQLite 幂等迁移、request actor helper、九类写链传播、任务持久身份和 recorder 的 before-null/after-actor 语义；19 个生产文件当前冻结，必要扩围 `business_task_service.py` 已被 Codex 接受。

首轮 failure-first **16 failed / 0 passed**，首轮专项 **16 passed**。Codex 拒绝恒真泄漏断言、假 worker、signature-only 传播、缺空账本同状态与伪迁移回滚证据，严格退回两个测试文件；Grok 返修后专项 **17 passed**、精确 schema **1 passed**，最终 review=`msg_de747706fcb64a188eef50d77e29d451`。当前代码仍未暂存、提交或推送，不得写成已完成。

两文件顺序联跑唯一失败已精确定位为既有 `test_no_commit_rollback_refresh_project_lock` 未恢复连接级 `PRAGMA ignore_check_constraints`；P13-C 目标用例单独通过。下一会话只做最小 test-only 恢复，再由 Codex 独立审查 19 个生产文件与返修专项，串行验证迁移、真实 worker、代表性九路、泄漏和原子回滚。默认不跑后端全量、Playwright、整仓 E2E；只有定点证据显示共享回归才扩大。

P13-D1 验收后由 Codex 中文提交 `功能：完成P13D1修订操作者可信账本` 并推送协作分支，再做文档闭环。随后单独冻结 P13-D2 当前操作者用户名展示；历史 actor 搜索、presence、SSE/WebSocket、协同光标/锁和完整审计仍是后续独立包。
