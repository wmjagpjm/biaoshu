...
> **交接日期**：2026-07-20（P13-D2 已冻结，待 Grok 受限实现与 Codex 独立验收）
...
> **最新增量基线**：P12N 已交付当前已加载修订固定优先；冻结=`337b401`、实现=`394639a`。P13-B 已交付当前客户端接受版本的 UTC 更新时间；冻结=`040d644`、实现=`1d4fe0b`。P13-C 已交付当前版本九类修订流程来源；冻结=`e62ea27`、实现=`6eaa89f`。P13-D1 已交付修订操作者可信账本；冻结=`3132684`、实现=`a8982e3`。
...
> **本地状态**：只允许分支 `collab/grok-code-codex-review`；P13-D1 实现=`a8982e3` 与闭环=`d89b006` 已推送。P13-D2 契约/计划已冻结；冻结提交推送后由 Grok 在严格白名单内实现，Grok 禁止 Git 写操作。
> **验收基线**：P13-B Grok P13-B/真值 **6/46 passed**，Codex 独立 **6 passed**。P13-C Grok 后端/前端 **18/11 passed**；Codex 独立后端 P13-C+P12C/全状态 **32/19 passed**、P13-B/C E2E **11 passed**。P13-D1 Codex 独立专项+schema/PRAGMA 顺序/代表写链 **18/2/5 passed**，py_compile/diff-check 通过。P13-B/C/D1 均未跑后端全量或整仓 318 E2E。**所有 pytest 与 Playwright E2E 共用 SQLite 重置库，pytest 禁止 xdist/并发分组，Playwright 必须显式 `--workers=1 --retries=0` 逐条串行运行；按风险分级验收，避免 Grok 与 Codex 重复全量。**
...
当前进度：P12A、P12B-A/B/C/D、P12C-A/B/C、P12D-A/B、P12E-A/B/C、P12F-A/B/C/D/E-A/E-B/F-A/F-B/G-A/G-B/H/I/J-A/J-B/P12K/P12L/P12M/P12N、P13-A/P13-B/P13-C/P13-D1、P9D、P9C-R1、M3-A 至 M3-D、P8B/P8C/P8D/P8E、P9A/P9B/P9C、P10A 至 P10K、P11A/P11B/P11C 均已完成。P13-D1 实现=`a8982e3`；整仓前端 **318 passed** 仍仅作既有历史基线。
...
当前执行包：P13-D1 已完成并推送，契约=`docs/p13d1-editor-state-revision-actor-ledger-contract.md`，计划=`docs/plans/2026-07-20-p13d1-editor-state-revision-actor-ledger-plan.md`，实现=`a8982e3`。Codex 独立通过专项+schema 18、PRAGMA 顺序 2、代表性真实写链 5 项；未跑后端全量或前端 E2E。
下一步：执行已冻结的 P13-D2 当前操作者用户名展示。契约=`docs/p13d2-current-revision-actor-username-contract.md`，计划=`docs/plans/2026-07-20-p13d2-current-revision-actor-username-plan.md`；先 failure-first，再由 Grok 严格九个生产文件/三个测试文件实现，Codex 独立审查验收。presence/SSE/WebSocket、历史/按 actor 搜索、Word 整章版式、真实解析模型/语料与新增外部来源仍是独立后续项。
...
| 当前版本协作元数据 P13-B/C 与 P13-D1 actor 账本 | `api/projects.py`、`api/schemas.py`、`services/editor_state_revision_service.py`、`services/task_service.py`、`core/database.py`、`tests/test_p13c_current_revision_source.py`、`tests/test_p13d1_revision_actor_ledger.py` | **齐** | P13-B/C 已交付 UTC 时间与九类来源；P13-D1 可信可空 actor 账本已以 `a8982e3` 完成并推送，专项+schema 18、顺序 2、代表写链 5 passed |
...
| 优先级 | 项 | 现状 |
|--------|----|------|
| 当前 P0 | P13-D2 当前操作者用户名展示 | P13-D1 已以 `a8982e3` 完成并推送；先冻结用户停用/删除/改名与版本不匹配时的保守 null 语义，再复用 P13-B/C 接受门，不公开内部 user id，不加入历史 actor 搜索或 presence |
| 紧随 P0 | 工作空间切换与成员可见性 | D2 后优先做不依赖实时协议的活动工作空间切换 UI 与成员可见性；presence、心跳、事件广播、WebSocket、协同光标和锁继续拆包 |
...
**粗估**：技术标 ~93%；商务 ~80%；合规工具可用；内网多人 ~30%；公网 SaaS ~15%。该估算是产品覆盖率；P13-D1 已验收，但真正多人实时协作仍未实现。
...
4. P13-D1 已完成。当前应独立冻结 P13-D2 当前操作者用户名展示；P12I/J/K/L、P12M/N 与 P13-A/B/C/D1 均已完成。D2 之后优先选择不依赖外部制品的工作空间切换/成员可见性，再分别设计 presence 和事件协议；跨项目版本、完整时间线与实时协作不得合包。
...
Codex 已以 `功能：完成P13D1修订操作者可信账本` 提交并推送，实现=`a8982e3`。文档闭环后单独冻结 P13-D2 当前操作者用户名展示；历史 actor 搜索、presence、SSE/WebSocket、协同光标/锁和完整审计仍是后续独立包。
...
前端只复用 P13-B/C 合法版本接受门和技术/商务标题区，固定显示“当前版本操作者”；项目切换同拍清空，迟到/409/失败/非法版本沿用既有隔离或保值，禁止新增请求、轮询、定时器、订阅或持久化。严格九个生产文件、三个测试文件；无模型/迁移/身份 API/history API 扩围。Grok 不得暂存、提交、推送，Codex 负责反假绿审查、独立验收与中文文档/Git 闭环。
