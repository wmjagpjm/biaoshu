...
> **交接日期**：2026-07-15（P11A/P11B/P11C 真实数据收口与 P8D 本机 MinerU 助手均已完成）
...
| 技术标 | 通用版本、响应矩阵与解析增强 | M3-D 已交付服务端原子确认和最近 20 批一次性漂移安全恢复；响应矩阵已完成来源分页、字段级三方合并和冲突保护；P8B/P8C/P8D 已接策略、单次票据与本机 MinerU 助手。仍未接：通用版本库、任意历史浏览/回滚、多人协作、MinerU 自动部署/模型打包、Docling 与其他交付增强 |
...
- **已验证基线**：后端 M3-D 专项 34 passed、受影响回归 71 passed、串行全量 487 passed（只有 1 条既有 Starlette/httpx 弃用警告）。P8D 新增助手单测 54 passed，P8C/P8B/解析后端受影响回归 35 passed。前端 P11C 18、P11B 11、P11A 10、认证/RBAC 11、解析策略 6、响应矩阵 8、融合确认 6、持久恢复 5、模板 1 passed，单 worker 串行全量 E2E 184 passed；本轮另复跑 P8C 9、P8B 6 passed；lint 零问题，build 通过（仅既有大 chunk 警告），实现暂存区 whitespace 检查通过。解析策略组合回归首跑曾有一次刷新时序波动，隔离重跑 6/6 且全量 184/184 覆盖通过。P8D 未改后端/前端，故 487/184 是沿用基线而非本包重跑。**E2E 共用 SQLite 重置库，所有 Playwright 命令必须串行。**
...
- **其余未实现主线**：所有 editor-state 的通用版本历史/任意历史浏览回滚/多人协作；MinerU 自动安装/模型打包/常驻服务与完整孙进程治理、Docling；P9C 固定模型运行时门和后续真实语义调优；Word `structure`/整章布局；除国能 e 招外的合法外部标讯来源；人力附件/真实证件核验；财务税务/审批/导出/预算/回款/版本、失败尝试与完整身份审计；投标人矩阵明细/版本/结果跟踪；Alembic、PostgreSQL、HTTPS、Key 加密、Docker 和公网 SaaS 能力。任何一项都须另立契约，不得扩大既有角色与生产路径。
