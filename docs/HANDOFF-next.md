...
> **交接日期**：2026-07-18（P12F-E-A/B 已闭环并推送；P12F-F-A 修订可见内容搜索后端已完成只读审计，当前文档提交即冻结）
...
> **协作分支功能基线**：P12F-E-B 冻结=`a31e50e`、实现=`f9127ec`、文档闭环见当前 HEAD；P12F-E-A 冻结=`af3798a`、实现=`c66b69d`、文档=`e1345ff`；P12F-D 冻结=`a2acdf3`、实现=`587df9a`；P13-A 冻结=`e8dfa61`、实现=`1509aa2`；P12F-C 冻结=`bb1ae3e`、实现=`fe99f5a`；P12F-B 冻结=`4ddd896`、实现=`c84a94d`；P12F-A 冻结=`e713fb3`、实现=`24f4cf2`；P12E-A 冻结=`5aa205c`、实现=`f9f067e`；P12E-B 冻结=`00ef081`、实现=`5a5b08a`；P12E-C 冻结=`8b40bf4`、实现=`b6a4375`；P12D-B 冻结=`fc19d93`、实现=`35ab377`、验收闭环=`c7cf67f`；P12D-A 冻结=`2cc6ee3`、实现=`9445fcc`；P9C-R1 冻结=`cd70ef0`、实现=`b53dcce`；其余既有功能基线见本文 §11。新会话必须以 `git rev-parse HEAD` 与远端分支一致为准。
> **最新增量基线**：P12F-E-A/B 已交付严格 UTC `createdFrom/createdBefore`、绑定时间/来源/位置的 `esrc3`，以及共用修订面板中的本地时间草稿、显式应用/清除和稳定分页；P12F-D 九类来源单选、P13-A 任务 SSE 工作空间鉴权及 P12F-A/B/C 的 20 条/20 MiB、有界游标页和手动加载更多继续保留。
> **参考 `origin/main`**：`4847a9d` — docs: 重写换会话交接并强制注释规范专章（非当前工作 HEAD）
> **本地状态**：P12F-E-B 冻结=`a31e50e`、实现=`f9127ec`、文档=`13dee82` 已推送；当前执行包 P12F-F-A 契约/计划已完成只读审计，冻结提交以当前 HEAD 为准。
> **验收基线**：P12F-E-A 后端专项/受影响回归/全量 **87/116/1073 passed**；P12F-E-B 前端聚焦/history/技术/商务/checkpoint/全量 **3/40/28/18/51/303 passed**，lint/build 通过。P13-A 独立 **13/72/918 passed**。**所有 Playwright E2E 共用 SQLite 重置库，必须逐条串行运行，禁止并行。**
...
当前进度：P12A、P12B-A/B/C/D、P12C-A/B/C、P12D-A/B、P12E-A/B/C、P12F-A/B/C/D/E-A/E-B、P13-A、P9D、P9C-R1、M3-A 至 M3-D、P8B/P8C/P8D/P8E、P9A/P9B/P9C、P10A 至 P10K、P11A/P11B/P11C 均已完成。P12F-E-B 冻结=`a31e50e`、实现=`f9127ec`；后端/前端全量基线为 1073/303 passed。
当前状态：修订页已支持来源与 UTC 时间范围组合筛选；共用前端以浏览器本地 `datetime-local` 草稿显式应用/清除，刷新、恢复、折叠和 `esrc3` 第二页均绑定已应用条件，项目切换重置。
当前执行包：P12F-F-A 修订可见内容搜索后端。新增独立 POST 请求体搜索，只扫描来源/时间条件下最新 20 条候选，只搜索严格用户可见字段并只回五键元数据；不改旧 GET、数据库或前端。
下一步：提交并推送 P12F-F-A 冻结契约/计划，通过既有 Grok 消息箱下发严格四文件 failure-first 实现；Codex 独立审查并串行跑专项、受影响回归和后端全量。P12F-F-B 前端、分页/片段、来源多选、日期预设、命名/固定/删除、跨项目历史、多人协作及 SSE 扩展不得混入。
...
4. P12B-A/B/C/D、P12C-A/B/C、P12D-A/B、P12E-A/B/C 与 P12F-A/B/C/D/E-A/E-B 已完成；P12F-E-B 冻结=`a31e50e`、实现=`f9127ec`，当前后端/前端全量基线 **1073/303 passed**。P12F-F-A 后端可见内容搜索已审计并由当前文档冻结；P12F-F-B 前端、删除、多来源筛选、跨项目历史或多人协作仍需另行规划。
...
- **Grok 当前状态**：P12F-E-B 已验收闭环，Codex 验收=`msg_489249aa6c264cc8a7125f07179b2d36`。新账号认证可用；Grok 仍只实现/自测、不提交推送，当前等待 P12F-F-A 冻结提交后的精确四文件任务。
- 当前分支仍为 `collab/grok-code-codex-review`；P12F-E-B 冻结=`a31e50e`、实现=`f9127ec` 已提交并推送，文档闭环以当前 HEAD 为准。全部既有基线保留。新会话第一步必须用 `git status -sb`、`git rev-parse HEAD`、`git rev-parse origin/collab/grok-code-codex-review` 重新核验，不可只信本文静态 SHA。
...
- **其余未实现主线**：修订自动批量比较、完整时间线、删除、正文/标题搜索、来源多选、日期预设、命名/固定、跨项目历史与多人协作；MinerU/Docling 自动安装、模型打包、常驻服务、真实模型样本验收与完整孙进程治理；P9C 真实用户语料调优、其他模型/GPU/在线 embedding/自动更新；Word `structure`/整章布局；除国能 e 招外的合法外部标讯来源；人力附件/真实证件核验；财务税务/审批/导出/预算/回款/版本、失败尝试与完整身份审计；投标人矩阵明细/版本/结果跟踪；Alembic、PostgreSQL、HTTPS、Key 加密、Docker 和公网 SaaS。
...
**换会话可直接：核验分支、HEAD/远端与工作区干净 → 读本文 §0～§3.1、§4.23、§5、§6、§11、P12F-A/B/C/D/E-A/E-B/F-A 契约/计划及路线图 → 确认 P12F-E-B 冻结=`a31e50e`、实现=`f9127ec`、文档=`13dee82` 已推送且后端/前端全量基线 1073/303 passed → 按 P12F-F-A 当前冻结提交向 Grok 下发严格四文件后端红测实现。禁止清理或重做已交付包、让 Grok commit/push、未经契约加入前端/游标/高亮/数据库索引/删除/跨项目历史/多人协作，或由 Codex 冒充 Grok 完成主实现。**
...
严格四文件：路由、Schema、history service、新专项测试。冻结生产 SHA-256：路由=`A5B6A9CE4DA528021C88E8A50E6D507B35BDE3AC26D220BA6863EDED69C789FC`，Schema=`852E0D691B004DCE754A4E90F034E970B3302B78710691C212CA738EED04AA65`，service=`89F4254D11E03E5C3E5F3D4F62CA75C8AAE22FC9FCA6CDCB93C03E3C1D8FB1AA`。Grok 必须先只新建测试形成真实 POST 路由红测，再实现三个生产文件；不得暂存、提交或推送。

本包不改前端、旧 GET、游标/分页、数据库/索引/迁移或依赖。P12F-F-B 前端入口必须在 A 包独立验收并文档闭环后重新审计冻结；命中高亮/片段、自动搜索、缓存、跨项目搜索、来源多选、日期预设、命名/固定/删除、多人协作和 SSE 仍未实现。
