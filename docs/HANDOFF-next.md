...
> **交接日期**：2026-07-17（P12D-A 修订与当前状态差异摘要 API 已完成、独立验收并推送）
...
| 技术标 | 通用版本、响应矩阵与解析增强 | M3-D 已交付最近 20 批恢复；P12B 已完成全状态版本、围栏、检查点恢复；P12C-A/B/C 已完成最近 10 条九来源修订、列表、按需摘要和双工作区恢复；P12D-A 已完成目标修订相对当前状态的后端只读字段差异与摘要。响应矩阵来源分页、字段级三方合并和冲突保护已完成；P8B/P8C/P8D/P8E 已接策略、票据与本机助手。仍未接：前端当前状态对比入口、正文 diff、任意历史两两比较、删除、搜索、跨项目历史、超出最近 10 条的完整历史/保留策略、多人协作、解析器自动部署/模型打包、真实模型验收与其他交付增强 |
...
4. P12B-A/B/C/D、P12C-A/B/C 与 P12D-A 已完成；P12D-A 冻结=`2cc6ee3`、实现=`9445fcc`，后端/前端全量 **831/284 passed**。下一步只读审计 P12D-B 前端当前状态对比入口；禁止扩成正文 diff、任意历史两两比较、删除、搜索、跨项目历史或多人协作。
