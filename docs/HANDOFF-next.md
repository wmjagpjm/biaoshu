...
> **V1-B 当前基线**：离线恢复与回滚演练已完成只读审计和双方问题确认，契约/计划已冻结、尚未实现。新备份必须升级 v2 并带独立 `biaoshu-data-v1` 和六根四态；v1 禁止自动覆盖。A review=`msg_af63265fc30b48bb8e6473af1a2ba3c4`、确认=`msg_0423e8b43c5846948c4da28ce0192635`；B review=`msg_64563cdc769b420dbb2aa2710d1cd40b`、确认=`msg_058bfb59096643bf9831e91779424a46`。
...
当前执行包：V1-B“离线恢复与回滚演练”已完成只读审计、问题双确认及契约/计划冻结；尚未实现。契约=`docs/v1b-offline-restore-rollback-contract.md`，计划=`docs/plans/2026-07-21-v1b-offline-restore-rollback-plan.md`。
下一步：提交并推送 V1-B 冻结文档；从冻结提交建立全新 A/B worktree。Grok B 先升级两份测试并提供真实 failure-first，Grok A 独立实现四个生产文件；Codex 组合后独立审查，疑似问题双方确认后才返修。在线热备、WAL、数据根迁移以及 V2/V3 能力继续后置；V1 继续优先完成自动化解析、标书内容制作、导出交付和本机稳定运行。
