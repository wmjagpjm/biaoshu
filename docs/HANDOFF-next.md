...
> **交接日期**：2026-07-16（P12C-C2 修订受限恢复已完成并独立验收）
...
当前进度：P12A、P12B-A/B/C/D、P12C-A、P12C-B-A、P12C-B-B1、P12C-B-B2、P12C-B-C1、P12C-B-C2、P12C-B-D1、P12C-B-D2、P12C-B-D3、P12C-C1、P12C-C2、P9D、M3-A 至 M3-D、P8B/P8C/P8D/P8E、P9A/P9B/P9C、P10A 至 P10K、P11A/P11B/P11C 均已完成。C2 冻结=`54af600`、范围修订=`2276366`、实现=`0803250`；后端全量 800、前端全量 263 passed。
当前执行包：P12C-C2 已交付受限 revision restore；锁后 CAS、C1 目标重验、恢复前安全检查点、准确 revision_restore、双配额与旧 SQLite 迁移均已闭环。
下一步：只读审计并冻结 P12C-C3 前端入口；必须重新定义列表挂载时机、按需详情/恢复确认、执行时最新 expected、成功后唯一重读和项目切换/折叠/连点迟到隔离。不得合入删除、diff、搜索、跨项目历史或多人协作。
...
| 编辑态有限修订与受限恢复 P12C | `models/entities.py`、`services/editor_state_revision_service.py`、`services/editor_state_revision_history_service.py`、`services/editor_state_revision_restore_service.py`、`api/editor_state_revisions.py`、`api/schemas.py`、`tests/test_p12c_revision_history_read.py`、`tests/test_p12c_revision_restore.py` | **齐** | 最近 10 条、九类准确来源、列表五列无正文、详情三重作用域重验、expected CAS、安全检查点与原子恢复；C2 独立 23/121/800 passed |
...
| 技术标 | 通用版本、响应矩阵与解析增强 | M3-D 已交付服务端原子确认和最近 20 批一次性漂移安全恢复；P12B-A/B/C/D 已完成全状态版本、延迟写入围栏、检查点恢复与双工作区入口；P12C-A/B/C1/C2 已交付最近 10 条修订账本、九类准确来源、公开列表/详情和后端受限恢复。响应矩阵已完成来源分页、字段级三方合并和冲突保护；P8B/P8C/P8D/P8E 已接策略、单次票据与本机助手。仍未接：revision 前端入口、删除、diff、搜索、跨项目历史、多人协作、解析器自动部署/模型打包、真实模型验收与其他交付增强 |
...
4. P12B-A/B/C/D、P12C-A、P12C-B-A/B1/B2/C1/C2/D1/D2/D3、P12C-C1 与 P12C-C2 已完成。下一步只读审计并冻结 P12C-C3 前端入口；必须证明挂载时机、执行时最新 expected、二次确认、唯一重读和迟到隔离，禁止合入删除、diff、搜索、跨项目历史或多人协作。
...
- **Grok 当前状态**：额度可用；P12C-C2 初版/范围回执=`msg_0f267572271042ed9486c40e758a20ca`，迁移假证据红测回执=`msg_fb1f416fba8c489a8876891c70a306c0`，事务返修回执=`msg_b0389af3509b4e4d93076334060141d8`，Codex 确认=`msg_1dc76dd27beb48ba8c367da1432ea0c6`。协作方式仍是 Grok 只实现/自测、不提交推送，Codex 独立规划、审查、验收、中文提交与推送；Grok 当前等待下一份 C3 冻结任务，不得自行开工。
- 当前分支仍为 `collab/grok-code-codex-review`；P12C-C2 实现=`0803250` 已提交，本文档闭环提交位于其后；P12C-C1 及 P12C-B-D3/D2/D1/C2/C1/B2/B1/B-A、P12C-A、P12B-D/C/B/A、P12A 等既有基线均保留。推送后新会话第一步必须用 `git status -sb`、`git rev-parse HEAD`、`git rev-parse origin/collab/grok-code-codex-review` 重新核验，不可只信本文静态 SHA。
...
- **已验证基线**：P12C-C2 专项/四文件回归 23/121、后端串行全量 800 passed（只有 1 条既有 Starlette/httpx 弃用告警）；P12C-C1 历史基线为 13/201/777，P12C-B-D3 为 18/270/764，P12C-B-D2 为 25/299/746，P12C-B-D1 为 11/285/732，P12C-B-C2 为 20/272/721，P12C-B-C1 为 10/224/711，P12C-B-B2 为 11/147/701，P12C-B-B1 为 10/126/690，P12C-B-A 为 14/107/680，P12C-A 为 67/77/666；P12B-D2 专项/受影响回归 51/63、单 worker、零重试前端全量 E2E 263 passed；11 文件 `py_compile`、真实 SQLite 迁移失败回滚、白名单、工作树与暂存区 diff 检查通过。P12B-D1 历史基线为 58/81/599；P12B-C 历史基线为后端/前端全量 570/212；P8E 独立 Docling 46、MinerU 54 继续保留。**E2E 共用 SQLite 重置库，所有 Playwright 命令必须串行。**
...
- **P12C-C2 已完成**：冻结=`54af600`、范围修订=`2276366`、实现=`0803250`。严格 expected CAS、C1 目标重验、安全检查点、共享写回、准确 `revision_restore`、双配额和旧库迁移已闭环；真实迁移红测与两轮返修后 Codex 独立 23/121/800 验收通过。无前端、删除、diff、搜索或多人协作。
- **其余未实现主线**：P12C-C3 历史前端与多人协作；修订删除、diff、搜索、跨项目历史与自动定时历史；MinerU/Docling 自动安装、模型打包、常驻服务、真实模型样本验收与完整孙进程治理；P9C 后续真实语义调优；Word `structure`/整章布局；除国能 e 招外的合法外部标讯来源；人力附件/真实证件核验；财务税务/审批/导出/预算/回款/版本、失败尝试与完整身份审计；投标人矩阵明细/版本/结果跟踪；Alembic、PostgreSQL、HTTPS、Key 加密、Docker 和公网 SaaS 能力。下一步只读审计并冻结 P12C-C3 的最小前端边界；不得把后端 POST 存在误报成前端已交付。
...
**换会话可直接：核验分支、HEAD/远端与工作区 → 读本文 §0～§3.1、§4.23、§5、§6、§11、P12C 契约/计划及路线图 → 确认 P12C-C2 冻结 `54af600`、范围修订 `2276366`、实现 `0803250` 已推送且后端/前端全量 800/263 passed → 只读审计并冻结 P12C-C3 前端，明确挂载时机、按需读取、二次确认、执行时 expected、成功唯一重读和迟到隔离。禁止重新实现 P12C-C2/C1/B-D3/B-D2/B-D1/B-C2/B-C1/B-B2/B-B1/B-A/P12C-A/P12B-A/B/C/D/P12A、合入删除/diff/搜索/多人协作、让 Grok commit/push，或由 Codex 冒充 Grok 完成主实现。**
