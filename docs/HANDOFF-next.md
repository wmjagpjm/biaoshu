...
> **V1-K 增量基线**：静默启动诚实诊断已完成；冻结=`8f0c137`、测试=`8f0c366`/`1fdea4c`/`997b57c`、实现=`cd4102f`。五入口委托唯一真源，有限状态侧车使用单次原子替换，显式诊断只读；Codex 独立 V1-K/V1-A 回归=`67/65 passed`。
...
> **V1-M 当前状态**：M1 管理式本机 OCR runtime 预检已完成并推送：契约=`9ed3a06`，测试=`3be6d19`/`3b8e42e`/`61dbe38`，生产=`a7d640b`；Codex 独立 `29/56/26/46 passed`、真实 junction no-follow 与 TEMP=0 通过。M2 只读审计=`msg_cc5b585777684a5ebfa90cffcb62186c`，待冻结决策；M3/M4 未开始，真实 runtime/quality did-not-run。
...
> **本地状态**：只允许主工作分支 `collab/grok-code-codex-review`，严禁操作 `main`。V1-A 至 V1-L、V1-M M1 均已完成并推送；M2-M4 待完成。文档闭环提交后以本地/远端分支一致为准，V2/V3 继续后置。
...
完整阅读 docs/v1m-managed-local-ocr-runtime-contract.md、docs/plans/2026-07-22-v1m-managed-local-ocr-runtime-plan.md、docs/agent-collaboration.md、docs/HANDOFF-next.md、docs/plans/2026-07-12-bid-writer-roadmap.md、docs/integration-checklist.md；V1-M M1 已闭环，下一步先冻结 M2 八项决策与精确白名单。
...
当前执行包：V1-M M2“后端 managed 调度、多文件 ASC 聚合、单事务写回与错误脱敏”决策冻结；B 只读审计已完成，尚未授权测试或生产。
下一步：Codex 批量核验并冻结 M2 八项决策、文件分隔字面量和严格 test/production/docs 白名单，再由 Grok 一次完成确认、failure-first 与限定生产；Codex 只做一次最终相关回归。真实安装/模型下载必须等 M4 取得用户明确授权；最终版式、在线热备、WAL、数据根迁移和 V2/V3 继续后置。
...
4. P13-E 至 P13-I4、V1-A 至 V1-L 已完成。V1-L 测试=`ea01c48`/`7c9266e`/`b0f197e`、实现=`10b5f3e`，已关闭默认回环系统无法显式安全提供可信内网单入口的缺口；下一包继续处理 V1 实际使用阻断，V2/V3 协作能力后置。
...
V1-K 后续缺口已由 V1-L 关闭：默认仍固定 loopback，仅显式可信内网 profile 绑定指定 RFC1918 IPv4 的 Vite 5173；仍禁止脱离契约直接改 `0.0.0.0` 或暴露后端 8000。

## V1-L 可信内网访问（已完成并推送）

契约=`docs/v1l-trusted-lan-access-contract.md`，计划=`docs/plans/2026-07-22-v1l-trusted-lan-access-plan.md`，基线=`ca7223a`，冻结=`2d7dd55`，测试=`ea01c48`/`7c9266e`/`b0f197e`，实现=`10b5f3e`。A/B 审计 review=`msg_092d74fd43f2402bb6f8a783d65274f3`/`msg_7d4576573bd049e68d828ed1e3ea5c74`；Q1 双确认 YES=`msg_33d84617361f434e93d6913c2968e15c`/`msg_0a67657dd9df443ba1e9eeddfb95409a`。

生产拓扑为“内网浏览器 → 显式 RFC1918 IPv4 上的 Vite 5173 → 回环 FastAPI 8000”。默认仍 loopback；LAN 必须显式 profile/host、强制 `AUTH_MODE=required` 并先证明 `authRequired=true` 与 `bootstrapped=true`；前端保持相对 `/api`，不扩 CORS、不改 Strict Cookie、不跨源直连后端。Vite Host 只允许有限精确值；IPv6、通配监听、自动防火墙、WAL/PostgreSQL、HTTPS/公网和 Docker 后置。

V1-K 状态继续固定七键与单次原子替换。防火墙只在文档给出 Private + LocalSubnet + TCP 5173 的管理员手工创建/删除步骤，生产代码不写系统策略。Codex 最终 Q8 定点 **5 passed**、V1-L **56 passed / 68 subtests passed**、V1-K **67 passed / 19 subtests passed**；lint/build、编译、Parse/BOM、diff、白名单、Stop 哈希与 TEMP 清理通过。因此 V1 产品覆盖率粗估更新为约 **94%**。

未运行真实 LAN 服务、防火墙、隔离数据库/uploads、真实管理员登录和第二台内网设备可达性验证。这些是下一步发布验收项，不得用自动化绿测或本机 loopback 冒充完成。
