...
| 技术标 | 通用版本、响应矩阵与解析增强 | M3-D 已交付最近 20 批恢复；P12B 已完成全状态版本、围栏、检查点恢复；P12C-A/B/C 已完成九来源修订、默认最近 10 条列表、按需摘要和双工作区恢复；P12D-A/B 已完成字段摘要与“与当前对比”；P12E-A/B/C 已完成单修订对当前及双历史修订的有界正文差异；P12F-A 已完成最多 20 条/20 MiB 的磁盘有界保留。仍未接：超过默认 10 条的游标分页、自动批量比较、完整时间线、删除、搜索、跨项目历史、多人协作、解析器自动部署/模型打包、真实模型验收与其他交付增强 |
...
4. P12B-A/B/C/D、P12C-A/B/C、P12D-A/B、P12E-A/B/C 与 P12F-A 已完成；P12F-A 冻结=`e713fb3`、实现=`24f4cf2`，当前后端/前端全量基线 **871/293 passed**。游标分页、删除、搜索、跨项目历史或多人协作仍需另行冻结。
...
- **P12C-C3 已完成**：冻结=`6b9143a`、实现=`5e4f9f6`。默认折叠、严格列表/按需摘要、共享令牌/保存链、执行时 expected、唯一重读和 list/detail/restore 迟到隔离已闭环；多轮反假绿后 Codex 独立 21/51/46/284、lint/build 验收通过。后续 P12D/P12E 已补齐对比，P12F-A 已补齐最多 20 条/20 MiB 有限保留；超过默认 10 条的游标分页、删除、搜索、跨项目历史或多人协作仍未实现。
