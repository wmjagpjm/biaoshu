...
> **本地状态**：只允许分支 `collab/grok-code-codex-review`。P13-H1 已完成并推送（实现=`4255823`、文档=`7e5e02e`）；当前只读审计并冻结 P13-H2，等待 Grok failure-first。详细边界见 `docs/p13h2-editor-state-event-sse-replay-contract.md`。
...
当前执行包：P13-H2 已只读审计并冻结；严格三文件，只在 H1 路由/服务增加项目级 SSE 与新专项测试。首次已有历史发送公开 tip 的 cursor 锚点而不回放旧变化；Last-Event-ID 只重放仍保留的后续事件。
下一步：让 Grok 先只写 P13-H2 新专项做真实 failure-first，再受限实现并串行自测；Codex 独立审查，疑似问题双方只读确认后才返修。P13-H3 前端版本提示、WebSocket、评论审批、协同光标、多任务总线和强制锁继续独立拆包。
...
4. P13-E、P13-F1/F2、P13-G1/G2、P13-H1 已完成，P13-H2 已冻结待 Grok failure-first；跨项目版本、完整时间线、前端提示、协同光标与多任务总线不得合包。
...
契约=`docs/p13h2-editor-state-event-sse-replay-contract.md`，计划=`docs/plans/2026-07-20-p13h2-editor-state-event-sse-replay-plan.md`，审计基线=`7e5e02e`。严格三文件，只修改 H1 路由/服务并新增 H2 专项；不改任务 SSE、main/schema/实体/transition、认证公共层或前端。

无 Last-Event-ID 且已有历史时不回放旧 editor-state，而是先发送公开 tip 的 cursor 锚点建立浏览器重连水位；有 Last-Event-ID 时仅正序重放保留窗口内的后续事件。空表首事件不能被 bootstrap 吸收；连接前 stale 固定 HTTP 409，连接中 stale 固定控制帧后关闭。等待 Grok 提交真实 failure-first 数字，Codex 不得代写或冒充结果。
