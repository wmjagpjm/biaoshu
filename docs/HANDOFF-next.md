...
> **交接日期**：2026-07-18（P12F-G-A/B 已闭环；P12F-H 单条修订命名完成只读审计，当前文档提交即冻结）
...
> **本地状态**：P12F-G-A 冻结=`c176cb5`、实现=`d2555d4`、闭环=`14298fd`；P12F-G-B 冻结=`89b5728`、实现=`bb7c4f4`、闭环=`4680f3c` 均已推送。P12F-H 契约/计划和十文件边界以当前文档提交冻结。
...
当前执行包：P12F-H 单条修订命名，契约=`docs/p12f-revision-display-name-contract.md`、计划=`docs/plans/2026-07-18-p12f-revision-display-name-plan.md`。严格十文件同时更新 nullable 名称列、独立写服务、六键元数据、共用前端与 history E2E。
下一步：提交并推送当前冻结文档，通过 Grok 消息箱下发十文件 failure-first；Codex 独立审查并串行跑后端专项/回归/全量与前端聚焦/history/checkpoint/truth/全量。固定/置顶、裁剪保护、名称搜索、批量、检查点命名、跨项目历史、多人协作及 SSE 不得混入。
...
| docs/plans/2026-07-18-p12f-revision-display-name-plan.md | P12F-H 十文件 failure-first、迁移、前后端与串行验收计划 |
...
- **Grok 当前状态**：新账号认证已由用户确认恢复。P12F-G-B 已经原任务、两轮受限返修完成，最终审查=`msg_ac681239aff641ab9cac8265bf0fb2bc`、验收=`msg_f51d6e2c60bf450eb2c3a7e3a24d3551`；Grok 仍只实现/自测、不提交推送。P12F-H 必须等待当前十文件冻结提交推送后通过本文消息箱脚本派发，不得沿用 G-B 三文件授权。
...
**换会话可直接：核验分支、HEAD/远端与工作区干净 → 读本文 §0～§3.1、§4.23、§5、§6、§11、P12F-G-A/B 与 P12F-H 契约/计划及路线图 → 确认 P12F-G-A 冻结=`c176cb5`/实现=`d2555d4`、P12F-G-B 冻结=`89b5728`/实现=`bb7c4f4` 均已推送且后端/前端全量基线 1110/310 passed → 以当前冻结提交通过消息箱下发 P12F-H 严格十文件 failure-first。禁止清理或重做已交付包、让 Grok commit/push、未经契约加入固定/置顶/裁剪保护/名称搜索/批量/软删除/回收站/检查点命名/跨项目历史/多人协作，或由 Codex 冒充 Grok 完成主实现。**
...
严格十文件，八个既有生产文件哈希及两个冻结时不存在的新文件见契约第 8 节。Grok 必须先只写新后端专项和既有 history E2E 形成真实 ASGI/UI 红测，再实现生产；不得提交推送。固定/置顶和裁剪保护明确留后，名称不会保护修订免于 P12F-A 裁剪。
