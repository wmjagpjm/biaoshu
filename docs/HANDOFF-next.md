...
- **P12C-C3 已完成**：冻结=`6b9143a`、实现=`5e4f9f6`。默认折叠、严格列表/按需摘要、共享令牌/保存链、执行时 expected、唯一重读和 list/detail/restore 迟到隔离已闭环；多轮反假绿后 Codex 独立 21/51/46/284、lint/build 验收通过。后续 P12D/P12E 已补齐对比，P12F-A 已补齐最多 20 条/20 MiB 有限保留，P12F-B 已补齐后端游标页；前端加载更多、删除、搜索、跨项目历史或多人协作仍未实现。
...
- **其余未实现主线**：超过默认最近 10 条的修订游标分页、自动批量比较、完整时间线、删除、搜索、跨项目历史与多人协作；MinerU/Docling 自动安装、模型打包、常驻服务、真实模型样本验收与完整孙进程治理；P9C 真实用户语料调优、其他模型/GPU/在线 embedding/自动更新；Word `structure`/整章布局；除国能 e 招外的合法外部标讯来源；人力附件/真实证件核验；财务税务/审批/导出/预算/回款/版本、失败尝试与完整身份审计；投标人矩阵明细/版本/结果跟踪；Alembic、PostgreSQL、HTTPS、Key 加密、Docker 和公网 SaaS。
...
**换会话可直接：核验分支、HEAD/远端与工作区干净 → 读本文 §0～§3.1、§4.23、§5、§6、§11、P12D-A/B、P12E-A/B/C 契约/计划及路线图 → 确认 P12E-C 实现=`b6a4375` 已推送且前端全量 293 passed → 继续审计剩余主线并重新冻结下一包。禁止清理或重做已交付包、让 Grok commit/push、未经契约扩成分页/搜索/恢复/多人协作，或由 Codex 冒充 Grok 完成主实现。**
