...
> **交接日期**：2026-07-22（V1-L 可信内网访问已完成审计与契约冻结，尚未实现）
...
> **V1-A 增量基线**：受控停机与离线备份基础已交付；实现=`5b4ad39`。两个根 bat、两个 UTF-8 BOM PS1、标准库备份核心和专项测试严格六文件；Codex 独立专项 `60 passed`，未真实停机或备份主仓数据。
> **V1-B 增量基线**：离线恢复与回滚演练已完成；代码=`20a4a60`。严格 v2、`biaoshu-data-v1`、六根四态、恢复前备份、同卷 staging+journal、提交前自动回滚、崩溃重入和 cleanup-pending 均已落地。初始 failure-first 为备份 `56/9`、恢复 `1/41`；Codex 最终独立备份/恢复 `65/81 passed`，A13-A15 手工物理态探针通过。
> **V1-C 增量基线**：本机解析运行时诚实预检已完成；冻结=`6e7aafb`、实现=`21d3213`。严格三文件新增零回调 `dry-run`、显式合成 DOCX 真值门、固定六键 JSON/诊断码和默认 MinerU/可选 Docling 运维说明。Codex 独立新专项/MinerU/Docling `26/54/46 passed`；本机无 MinerU 时真实 dry-run 为 `cli_missing`/2，未运行真实合成门或安装模型。
...
> **V1-H1 增量基线**：章节生成空白正文质量门已完成；冻结=`a1cee86`、实现=`d550dd9`。模型空白输出固定失败且零空章写入，多章保留既有成功前缀与逐章 CAS，合法短章和有效 Markdown 原样保留；Codex 独立专项/回归 `6/66 passed`。
> **V1-H2 增量基线**：技术标导出正文完整性提醒已完成；冻结=`73a76c6`、实现=`0047a46`。export result 始终返回独立 `contentWarnings`，空章或无有效章节时给出固定有限脱敏提醒，商务标固定空数组；前端纯文本收敛并受项目/导出代次隔离。Codex 独立后端 `6/35 passed`、前端 `4/4/18/14 passed`，lint/build/编译/diff/边界门通过。
> **V1-I 增量基线**：创建页真实文件摄入已完成；冻结=`c1cde54`、测试=`35b0f6b`/`b8c9776`、生产=`4e7a3c3`。技术类入口持有真实 `File`，一次 create 后串行 multipart，部分失败复用同一 projectId 只补剩余项；工作区只认服务端文件。Codex 独立 V1-I/P11A/技术 truth=`8/10/28 passed`，lint/build/diff/端口门通过。
...
> **本地状态**：只允许主工作分支 `collab/grok-code-codex-review`，严禁操作 `main`。V1-A 至 V1-K 均已完成并推送；V1-L 已冻结、待 failure-first 与实现，V2/V3 继续后置。
...
完整阅读 docs/v1l-trusted-lan-access-contract.md、docs/plans/2026-07-22-v1l-trusted-lan-access-plan.md、docs/HANDOFF-next.md、docs/plans/2026-07-12-bid-writer-roadmap.md、docs/integration-checklist.md；V1-K 已闭环，V1-L 已完成只读审计和双确认冻结，下一步只能先写 failure-first。
...
当前进度：P12A、P12B-A/B/C/D、P12C-A/B/C、P12D-A/B、P12E-A/B/C、P12F-A/B/C/D/E-A/E-B/F-A/F-B/G-A/G-B/H/I/J-A/J-B/P12K/P12L/P12M/P12N、P13-A 至 P13-I4、V1-A 至 V1-K、P9D、P9C-R1、M3-A 至 M3-D、P8B/P8C/P8D/P8E、P9A/P9B/P9C、P10A 至 P10K、P11A/P11B/P11C 均已完成；整仓前端 **318 passed** 仍仅作既有历史基线。
...
当前执行包：V1-L“可信内网访问”failure-first；A/B 审计与 Q1 双确认已完成，契约=`docs/v1l-trusted-lan-access-contract.md`，计划=`docs/plans/2026-07-22-v1l-trusted-lan-access-plan.md`。
下一步：Grok B 严格 test-only 形成真实业务红，Codex 排除假红并提交测试后，才授权 Grok A production-only；OCR/自动安装、复杂跨页 `structure`、在线热备、WAL、数据根迁移和 V2/V3 继续后置。
...
| 当前版本协作元数据 P13-B/C/D2 与 P13-D1 actor 账本 | `api/projects.py`、`api/schemas.py`、`services/editor_state_revision_service.py`、`services/task_service.py`、`core/database.py`、`tests/test_p13c_current_revision_source.py`、`tests/test_p13d1_revision_actor_ledger.py`、`tests/test_p13d2_current_revision_actor_username.py`；前端两 Hook/页面与共享 freshness 组件 | **齐** | P13-B/C 已交付 UTC 时间与九类来源；P13-D1 可信可空 actor 账本=`a8982e3`；P13-D2 当前用户名展示=`44c9196`，Codex 独立 44/15/17/4 passed |
...
| 本地解析与 V1-C 预检 | `local-parser`；`tools/local-parser/runtime_preflight.py`、`test_runtime_preflight.py` | **齐** | 页面 `projectId` 仅预填；预检零票据/零回调，dry-run 零进程，真实合成门须用户授权 |
...
| 优先级 | 项 | 现状 |
|--------|----|------|
| 已完成 | P13-D2 当前操作者用户名展示 | 冻结=`4b95ab5`、实现=`44c9196`；当前用户名、保守 null、单查询与 P13-B/C 接受门均已验收，不公开内部 user id |
| 已完成 | P13-H1 事件账本与游标后端 | 冻结=`da2537a`；独立事件表、真实 after 同事务写入、200 条裁剪、bootstrap tip 与严格项目级增量 GET 已验收 |
| 协作主线 | 真正多人协作 | 账号/workspace/RBAC/CAS/任务 SSE 鉴权、活动空间切换、成员可见性、presence、章节意图、事件游标、SSE 重放与前端保守提示已有；协同光标、强制锁、WebSocket、多任务总线、评论/审批/通知仍未实现 |
...
| 技术标 | 通用版本、响应矩阵与解析增强 | M3-D、P12B/C/D/E、P12F、P12G-N 已交付项目内有界版本、比较、搜索、分页、命名、删除、固定、检查点与恢复；不要把这些已完成功能重复列为缺口。仍缺跨项目历史、完整时间线、按 actor 搜索/审计、搜索片段/高亮/评分/FTS、自动批量比较、真正多人协作、解析器自动部署/模型打包、真实用户语料验收与其它交付增强 |
...
**实时粗估**：本机/内网可用标书系统整体约 **93%**；技术标约 **98%**，商务标约 **84%**，内网多人协作约 **55%～60%**，公网 SaaS 约 **15%**。该估算是产品覆盖率；P13-E 至 P13-I4、V1-A 至 V1-K 已验收，账号/RBAC/CAS/presence/章节意图/SSE 重放、任务成功安全水合、真实创建页文件摄入、空文档解析拒绝、空白模型输出拒绝、导出前保存、稳健人读名下载、空章导出提醒、静默启动和诚实诊断已有；可信内网发布、扫描 PDF OCR/真实解析器部署、最终版式、协同光标、评论审批、强制锁和生产部署治理仍未完成。
...
4. P13-E 至 P13-I4、V1-A 至 V1-K 已完成。V1-K 冻结=`8f0c137`、测试=`8f0c366`/`1fdea4c`/`997b57c`、实现=`cd4102f`，已关闭静默启动无诚实诊断与状态非原子覆盖；下一包为 V1-L 可信内网访问只读审计，V2/V3 协作能力继续后置。
...
| **docs/HANDOFF-p13d1-in-progress.md** | **P13-D1 提交前操作级交接的历史记录；顶部已补最终提交、验收数字与 PRAGMA 根因纠正** |
| docs/p13d1-editor-state-revision-actor-ledger-contract.md | P13-D1 两列、九路传播、before/after 真值与隐私冻结契约 |
| docs/plans/2026-07-20-p13d1-editor-state-revision-actor-ledger-plan.md | P13-D1 failure-first、范围、分级验收与提交边界 |
...
| docs/plans/2026-07-22-v1f-robust-export-download-plan.md | V1-F failure-first、双确认返修、十文件实现与 Codex 验收记录 |
...
- **PRAGMA 根因已纠正**：失败不来自前序测试主动设置 PRAGMA，而是 P13-C 测试在 `PRAGMA=1` 后 `Session.commit()` 归还连接，恢复可能落到另一池连接；现用同一显式连接完成开启、写坏数据、提交、关闭和精确校验，未改生产或删除守卫。
...
- **V1-E 已完成并推送**：冻结=`2f3beb1`、实现=`2a1b1ec`。原始 failure-first **11 failed / 3 passed**，双方确认并加固后生产修复前为 **14 passed / 4 failed**；Codex 最终新专项/图片告警/truth **18/4/46 passed**，lint/build/diff-check 通过。最终严格六文件；P9D 跨项目迟到下载旧语义经双方确认后由更严格的零下载契约覆盖。
- **V1-F 已完成并推送**：冻结=`5df1114`、契约收紧=`ee97701`、实现=`65fe5e6`、文档闭环=`5448ed9`。首轮后端/新下载/P9D/V1-E 为 **16f/1p、13f/0p、3f/1p、3f/15p**；Codex 最终独立后端 **20/1 passed**、前端 **14/4/18/28/18 passed**，lint/build/diff-check 通过。最终严格十文件。
...
## P13-D1 修订操作者可信账本（已完成并推送）

冻结=`3132684`，实现=`a8982e3`，契约=`docs/p13d1-editor-state-revision-actor-ledger-contract.md`，计划=`docs/plans/2026-07-20-p13d1-editor-state-revision-actor-ledger-plan.md`。两张表的可空 actor、SQLite 幂等迁移、request actor helper、九类写链传播、任务持久身份和 recorder 的 before-null/after-actor 语义均已落地；必要扩围 `business_task_service.py` 已验收。

首轮 failure-first **16 failed / 0 passed**，首轮专项 **16 passed**。Codex 拒绝恒真泄漏断言、假 worker、signature-only 传播、缺空账本同状态与伪迁移回滚证据，严格退回两个测试文件；Grok 返修后专项 **17 passed**，最终 review=`msg_de747706fcb64a188eef50d77e29d451`。

Codex 复现后纠正了交接根因：P13-C 测试在开启 PRAGMA 后 `Session.commit()` 归还连接，后续恢复可能换连接。改为同一显式连接闭环后，顺序回归 **2 passed**。独立专项+schema **18 passed**，融合 apply/consume、检查点恢复、修订恢复、本地票据回调代表路径 **5 passed**；py_compile、diff-check、19 个生产哈希与 22 文件实现白名单通过。未跑后端全量、Playwright、lint/build 或整仓 E2E。

Codex 已以 `功能：完成P13D1修订操作者可信账本` 提交并推送，实现=`a8982e3`。P13-D2 也已完成；历史 actor 搜索、presence、SSE/WebSocket、协同光标/锁和完整审计仍是后续独立包。

## P13-D2 当前已载入版本操作者用户名展示（已完成并推送）

契约=`docs/p13d2-current-revision-actor-username-contract.md`，计划=`docs/plans/2026-07-20-p13d2-current-revision-actor-username-plan.md`。产品语义已冻结：只看当前项目最新修订且版本必须与响应 `stateVersion` 精确匹配；actor 还必须关联启用用户与同工作区启用成员，用户名通过严格安全文本门，才返回 `currentRevisionActorUsername`，否则 `null`。

用户/成员删除或停用、跨工作区、disabled、旧修订和补账 before 统一未知；活动成员角色变化仍显示，未来改名展示当前名而非历史快照。来源和用户名由一次最新修订左联表查询独立校验，不加载 snapshot、口令、会话或内部 actor ID。

前端只复用 P13-B/C 合法版本接受门和技术/商务标题区，固定显示“当前版本操作者”；项目切换同拍清空，迟到/409/失败/非法版本沿用既有隔离或保值，禁止新增请求、轮询、定时器、订阅或持久化。最终严格九个生产文件、四个测试文件；无模型/迁移/身份 API/history API 扩围。

冻结=`4b95ab5`、实现=`44c9196`。后端真实 failure-first **26 failed / 0 passed**；前端无合规 E2E-only red，未补造。Grok 最终后端/freshness **44/17 passed**；Codex 独立后端核心/受影响回归/freshness **44/15/17 passed**，外部写唯一 GET 代表路径 **4 passed**，lint、py_compile、diff-check、哈希、白名单与泄漏门通过。未运行后端全量或整仓 318 E2E。

消息：初始 task/review=`msg_440d7e3e83fa4be0a2a835f8c22aacfc`/`msg_847dca8b61064346bad66b3cb99c6450`；返修 task/review=`msg_33342a63342c40399ab6f19501b6f5fc`/`msg_375ba4a732bf42b8986d21ce2602ba9c`。Grok 全程未执行 Git 写操作。下一包 P13-E 已冻结，仍禁止与 presence/事件协议合包。

## P13-E 活动工作空间切换与成员只读可见性（已完成并推送）

契约=`docs/p13e-active-workspace-switch-member-visibility-contract.md`，计划=`docs/plans/2026-07-20-p13e-active-workspace-switch-member-visibility-plan.md`。后端 P10A 已真实提供活动空间切换和 owner-only 成员列表，因此本包严格只改前端六个生产文件与 `frontend/e2e/auth-rbac.spec.ts`，不扩大成员权限、不修改后端。

全局选择器只在 required 已认证态显示；合法目标只取当前 `/auth/me` 工作空间，精确一次 PUT `{workspaceId}`。成功按新角色整页重载；失败必须 `/auth/me` 对账，确认服务端是否已经切换，防止旧空间 UI 继续向新空间会话发业务请求。disabled、同值、非法值均零请求，不新增 `X-Workspace-Id`、存储或外网。

设置页 required 工作空间名称/ID/角色/所有者改用真实 `activeMembership`，所有者显式点击后才单次 GET 成员。仅显示用户名、中文角色、所有者和启用状态；不展示 userId，不把 `isActive` 叫作在线，不向非所有者开放，不做成员写操作、自动加载、轮询或定时器。

开工基线=`986ace2`，文档冻结=`19f0bfe`，功能实现=`5685441`。代码提交严格六生产加一 E2E，无后端、router、统一 HTTP 客户端、依赖或配置扩围；Grok 全程未暂存、提交、推送或写文档，Codex 已完成独立审查、验收和 Git。

从本包第二轮审查起执行“双确认返修门”：Codex 发现疑似问题后先下发只读确认，Grok 独立确认或否认；只有双方明确确认存在，Codex 才另发修复授权。确认消息本身不授权改文件，分歧时保持代码原状补证据，仍不能统一则交用户裁定。双确认前若误触文件，必须立即中止并冻结，不继续、不清理、不提交，待新授权接管。

本轮 Codex 发现两项：非空空间允许 `activeWorkspaceId=null` 后回退首空间导致认证/授权真值分叉，以及 `expectCountStable` 在初值已满足时不能证明稳定窗口。只读确认请求=`msg_c1e71b76f13c418f99d6f73fbf778b77`，Grok 确认=`msg_e6f7094596fc4d3db79661611b217f10`，两项均由双方确认存在。此前在规则明确前提前启动的返修=`msg_c5e4959b6e9b4276b6ed0f15334ef865` 已中止；新授权=`msg_f3914a680ccf4b9fbf3b3a099fb3f3cb`，review_request=`msg_1bfe78d7492e476d9b7187ad847dbdbd`，Codex 验收=`msg_1ab08b68c9e74278ad7b17e537633321`。

真实 failure-first 为 **14 failed / 2 passed**，生产哈希仍冻结，首个业务失败为选择器缺失。Grok 最终 P13-E/完整认证 **25/36 passed**，lint/build/diff-check 通过；Codex 独立 **25 passed（40.5 秒）/36 passed（50.7 秒）**，lint/diff-check 通过，首轮三个既有后端定点 **3 passed**。最终七文件哈希见契约；测试产物已清理。未运行后端全量或整仓 E2E。

完整认证 E2E 仍会偶发打印 disabled 设置页 `workspace_settings.workspace_id` 并发 `UNIQUE` 异常，但本轮 36 项全部通过。该问题位于既有 Settings `get_or_create` 后端路径，不在 P13-E 白名单；后续若处理，仍须先由 Codex 与 Grok 双确认，再单独授权修复。

## P13-F1 项目在线租约后端基础（已完成并推送）

契约=`docs/p13f1-project-presence-lease-backend-contract.md`，计划=`docs/plans/2026-07-20-p13f1-project-presence-lease-backend-plan.md`，基线=`70dfb39`。只新增项目级 `POST presence/heartbeat|leave`；required 当前活动 workspace 的 strict bid_writer 才可用，任何 `X-Workspace-Id` 拒绝，项目继续按 workspace 隔离。

租约固定 45 秒、建议 15 秒续租；clientId 只接受 22..64 位安全 base64url/UUID 文本，数据库只存 SHA-256 摘要。同 client 原子续租，每用户每项目最多 8 个活动 client；快照同用户聚合，最多 50 个安全用户名，只返回 `username/isSelf`，不返回任何用户/成员/租约/client/会话内部 ID。

严格白名单为 `entities.py`、模型导出、`schemas.py`、新 presence service、新 presence API、`main.py` 与一个新专项测试。P13-F1 不修改公共依赖、认证、项目/editor-state/task 服务或前端；任务 SSE heartbeat 仍只是连接保活。Grok 必须先真实 failure-first，再实现与串行自测；Codex 发现问题后先让 Grok 只读确认，双方确认后才可另发修复授权。

完成记录：冻结=`78302bc`、实现=`6164d8c`。初始红测 **30 failed / 4 passed**；Codex 两轮双确认关闭 clientId 422 原文泄漏、SQLite 并发/8-client 上限、rollback 与测试口径、锁前 UTC 时钟五类问题。Grok 两轮返修红测 **16 failed / 5 passed**、**2 failed**，最终专项/直接回归 **41/55 passed**；Codex 独立专项/代表回归 **41/3 passed**，七文件编译、差异、白名单、哈希门通过，result=`msg_5aae77e9c06b436aaa9f46c5747e4648`。未跑后端全量、前端或整仓 E2E；P13-F2 完成记录见下节。

## P13-F2 项目近期成员前端（已完成并推送）

契约=`docs/p13f2-project-presence-frontend-contract.md`，计划=`docs/plans/2026-07-20-p13f2-project-presence-frontend-plan.md`，操作级交接=`docs/HANDOFF-p13f2-in-progress.md`，开工基线=`66f4390`。

Codex 已完成只读审计：现有 `EditorStateVersionFreshness` 必须继续无副作用，两个大型 editor Hook 不应复制定时器，全局 AppShell 也不能代表当前项目。因此冻结为共享 `projectPresenceApi.ts` + `ProjectPresencePanel.tsx`，技术/商务页面各薄挂一次，再以独立 `project-presence.spec.ts` 验收，严格四生产加一测试。

产品语义为 visible-only“近期在此项目”：required strict bid_writer 的可见项目页才 heartbeat；文档内存 UUID 不落盘，所有 heartbeat/leave 串行，成功后 15 秒续租，hidden/项目切换/卸载/pagehide best-effort leave。StrictMode 不得产生稳定期重复首跳；A→B 同步隐藏 A 并隔离迟到。响应必须精确四键、成员精确两键、安全用户名、唯一 self，坏包固定不可用且敏感值零出口。

完成记录：冻结=`a5709ed`、实现=`dfa6bc0`。初始 failure-first `7 failed / 1 passed`；第一轮八项双确认返修红测 `2 failed / 0 passed`；第二轮 E2E-only 双确认补齐原始 console 真实 UUID 门与 hidden 延迟生成证据。Grok 最终专项 `11 passed`；Codex 独立专项/freshness `11/17 passed`，lint/diff-check 通过，build 沿用最终生产改动后的 Grok 成功结果；result=`msg_f19ceb09650a4f0584e2d4b1d1985fb4`。协同光标、章节锁、广播、SSE/WebSocket presence、在线历史、评论审批继续不在本包。

## P13-G1 项目章节编辑意图租约后端（已完成并推送）

契约=`docs/p13g1-project-chapter-edit-intent-lease-backend-contract.md`，计划=`docs/plans/2026-07-20-p13g1-project-chapter-edit-intent-lease-backend-plan.md`，操作级交接=`docs/HANDOFF-p13g1-in-progress.md`，审计基线=`f0325d0`。

只读审计确认现有技术标章节仅位于整包 `chapters_json`，ID 可由前端或模型生成；现有 PUT 无 clientId、章节差异或租约令牌。因此本包冻结为单章节单持有者的短期“处理意图”，绝不称强制锁。heartbeat 锁后精确校验当前技术标章节唯一命中，同 user+client 续期，其它 client 固定冲突并只返回安全用户名；leave 五维精确幂等。TTL/refresh 为 45/15 秒，每用户项目最多 8 个活动章节。

严格七文件：entities、模型导出、schemas、新 lease service、新 lease API、main、新专项测试。无前端、editor-state PUT、P13-F1/F2、认证、配置、依赖或已有测试扩围。

完成记录：冻结=`a0b7c48`、实现=`015ab37`。有效 failure-first **42 failed / 3 passed**；首轮实现专项/代表回归 **45/41/8/1 passed**。Codex 首轮审查六项问题全部经 Grok 只读确认后才授权返修，最终关闭 4096 字节流式 body 上限、恒真/宽状态、真实锁等待 fresh-now、精确 FK/级联与 JSON 公开矩阵。Grok 聚焦/专项 **17/53 passed**；Codex 独立专项/P13-F1/认证/editor-state **53/41/8/1 passed**，result=`msg_18dc76c33b9f47d0a72d754e7578682c`。未跑后端全量、Playwright、前端或 xdist；P13-G2 已在后续独立包完成。

## P13-G2 项目章节编辑意图前端提示（已完成并推送）

契约=`docs/p13g2-project-chapter-edit-intent-frontend-contract.md`，计划=`docs/plans/2026-07-20-p13g2-project-chapter-edit-intent-frontend-plan.md`，操作级交接=`docs/HANDOFF-p13g2-in-progress.md`，审计基线=`6dac9da`。

严格四文件交付端点专用严格 API、独立串行生命周期面板、技术标 content 薄挂载与新 E2E。P13-F2 presence 与章节意图复用同一文档内存 UUID，但队列独立；200/409 严格解析，StrictMode、完成后 15 秒、A→B、hidden/visible/pagehide 和迟到隔离均有真实证据。冲突和不可用仅提示，不阻断正文、AI、autosave 或 editor-state PUT。

完成记录：冻结=`3a74fbb`、实现=`86abbbf`。failure-first=`msg_b20b7dbe314943ba806fcf62f37d95c9`，**8 failed / 1 passed**；两轮只读双确认=`msg_9fa0bb83f0f348f99eca175567b3983d`、`msg_24da16ad88c94f7585de0a34ef88095d`；Grok 最终 review=`msg_7a542b4e3d444c13800cc401141a0d90`，专项/聚焦关键序列 **13/7 passed**。Codex 独立专项/P13-F2/freshness **13/11/17 passed**，lint/build/diff/严格四文件与八文件哈希门通过。未跑整仓 318 E2E、后端 pytest、xdist 或并发 Playwright。

## P13-H1 editor-state 事件账本与游标后端（已完成并推送）

契约=`docs/p13h1-editor-state-event-cursor-backend-contract.md`，计划=`docs/plans/2026-07-20-p13h1-editor-state-event-cursor-backend-plan.md`，冻结=`da2537a`、实现=`4255823`、文档=`7e5e02e`。严格八文件新增独立 `editor_state_events` 表，并在统一 transition 原语中只为真实 after 修订同事务写入事件；项目级 GET 支持公开 bootstrap tip、增量读取、200 条裁剪和 stale 409。

本包只提供 required strict bid_writer 当前活动 workspace 项目的精确游标 GET，事件不含快照、正文、章节、actor/client、任务结果或内部异常；不做 SSE、Last-Event-ID、前端刷新、WebSocket、通知或强制锁。failure-first=`msg_ee84a231060941049177cce0f05f501a`，真实 **25 failed / 3 passed**；双方确认公开 API 缺 bootstrap tip 和宽状态断言两项问题后才返修。Grok 最终专项/回归 **28/90 passed**，Codex 独立专项/editor-state 与 P13-D1 回归 **28/90 passed**，`compileall` 与 diff-check 通过。

## P13-H2 editor-state 事件 SSE 与断线重放（已完成并推送）

契约=`docs/p13h2-editor-state-event-sse-replay-contract.md`，计划=`docs/plans/2026-07-20-p13h2-editor-state-event-sse-replay-plan.md`，审计基线=`7e5e02e`，功能=`c19bf94`。严格三文件，只修改 H1 路由/服务并新增 H2 专项；不改任务 SSE、main/schema/实体/transition、认证公共层或前端。

无 Last-Event-ID 且已有历史时不回放旧 editor-state，而是先发送公开 tip 的 cursor 锚点建立浏览器重连水位；有 Last-Event-ID 时仅正序重放保留窗口内的后续事件。空表首事件不能被 bootstrap 吸收；连接前 stale 固定 HTTP 409，连接中 stale 固定控制帧后关闭。failure-first **14 failed / 1 passed**；双方确认 request-scope Session 问题后最小返修，Codex 独立专项/代表回归 **15/46 passed**，compileall 与 diff-check 通过。

## P13-H3 编辑状态事件前端版本提示（已完成并推送）

契约=`docs/p13h3-editor-state-event-frontend-contract.md`，计划=`docs/plans/2026-07-21-p13h3-editor-state-event-frontend-plan.md`。严格六文件实现覆盖 required/authenticated/bid_writer 门控、精确 EventSource URL 与 `withCredentials`、cursor/editor-state 严格解析、重复键与 UTC 日历拒绝、默认 `message`/控制帧/网络错误固定不可用、同步刷新单飞、技术/商务页面失败提示及 A→B 在途刷新代次隔离；不自动覆盖正文、不 PUT、不写 storage/URL/日志。未注册命名事件受原生 EventSource 能力限制，已由真实 SSE 边界用例记录，后续协议扩展不得合入本包。

failure-first=`msg_1e4734a045024eed91aaf13a58ef705e`（1 passed / 1 failed / 5 did not run）；首轮返修=`msg_e9809e17435c494589e7cf1f13b8262a`；第二轮返修=`msg_898315bea44b4cfca1435744b0cd920f`。Codex 独立串行 H3 `15 passed`、freshness `17 passed`，lint/build/diff-check 通过；功能提交=`40aacc7`，未运行整仓 E2E、后端全量或并发测试。

## P13-I1 项目任务事件游标后端（已完成并推送）

契约=`docs/p13i1-project-task-event-cursor-backend-contract.md`，计划=`docs/plans/2026-07-21-p13i1-project-task-event-cursor-backend-plan.md`，功能提交=`f0d6d75`。严格十一文件新增独立 `project_task_events` 表、真实任务状态写链和两条直接终态 callback 事件，并提供 required 活动 workspace strict `bid_writer` 的项目级只读游标 GET；不提供 SSE、`Last-Event-ID`、WebSocket、通知、评论或任务正文结果。

真实 failure-first 为 **17 failed / 0 passed**。第一轮 Codex 只读问题=`msg_c60ffa9f89334940b6ab39eee85fb5c1`，Grok 确认=`msg_c4b13a10f3ec490f983a0c03f4f9d262`，双方确认两条直接终态写链漏事件后才扩至十文件；第二轮只读问题=`msg_85b188d2dabe4f12be6c62d3dde1850f`，Grok 确认=`msg_0e9da620244d4ead96a46f815131306c`，四项缺口确认后才扩至十一文件并返修。

Grok 最终回执=`msg_be5b0a6a444841969517152db8fac4f8`；Codex 独立验收回执=`msg_7e1d86e5d0b240a4a011bba4c5bce8bf`。Codex 串行通过专项 **25**、callback/票据 **20**、认证 **39**、单任务 SSE/P13-A **18**、P13-H1/H2 **43**，合计 **145 passed**；`compileall`、`git diff --check`、十一文件边界与哈希门通过。工作区已清空，本地 HEAD 与远端均为 `f0d6d75`。

未运行后端全量、xdist、前端或整仓 E2E。下一包为已在独立 worktree 在途的 P13-I3 前端任务事件提示；不得把任务通知、评论审批、协同光标、WebSocket 或强制锁并入 I3。

## P13-I2 项目任务事件 SSE 与断线重放（已完成）

契约=`docs/p13i2-project-task-event-sse-replay-contract.md`，计划=`docs/plans/2026-07-21-p13i2-project-task-event-sse-replay-plan.md`，冻结=`525d059`，功能=`03fb90e`。严格三文件在 I1 脱敏账本上增加项目级 `/task-events/stream`：无 header 时只发 tip cursor 锚点，有 `Last-Event-ID` 时正序重放保留事件，空表首事件不丢；SSE 只发六键任务事件、注释心跳与固定控制帧，不读取任务正文或结果。

真实 failure-first 为 **15 failed / 0 passed**。Codex 首轮 question=`msg_e3f6751a53c14bb8b08e4bb32c713f1e`，Grok 确认=`msg_63b808eadc244154afdca692874a27f8`；双方确认连接中 stale、unavailable、request-scope `get_db` 反假绿和跨 workspace 游标四项证据缺口后才授权 test-only 返修。控制帧唯一性又经 `msg_8830175702d24e99955a1a2d8824f6ba` / `msg_be93334deb8846d6ae6a999796223b85` 确认收紧，生产哈希全程不变。

Codex 独立串行专项 **17 passed**、I1/单任务 SSE/P13-A/H1/H2/认证代表回归 **125 passed**，合计 **142 passed**；`compileall`、`git diff --check`、严格三文件和哈希门通过。未运行后端全量、xdist、前端或整仓 E2E。

## P13-I3 项目任务事件前端提示（已完成）

契约=`docs/p13i3-project-task-event-frontend-contract.md`，计划=`docs/plans/2026-07-21-p13i3-project-task-event-frontend-plan.md`，冻结=`5c63890`，功能=`c6dbe2e`。严格四文件在 I2 项目级 SSE 上增加技术标/商务标共享的安全任务状态提示：仅在 required/authenticated/bid_writer 门控通过时建立原生 EventSource，严格解析 `cursor`、`task-event`、`cursor-stale`、`unavailable`，固定安全展示并隔离项目切换迟到事件；不请求任务详情、不刷新正文、不写 storage/URL/日志、不提供通知、评论审批、协同光标、WebSocket 或强制锁。

真实 failure-first 为 **1 failed / 1 passed / 3 did not run**。首轮问题包括非法日历时间、非法 ID/type/cursor 测试缺口、I3 文件顶注释缺失和控制帧 data 未严格解析；Codex question=`msg_6a19689c036540b09eac00d65bbb58a7`，Grok 确认=`msg_5ebe466f38f9404b8294f42c630c6f8a7`，双方确认后返修 task=`msg_98272242fe8741a086c96f460e2f90ed`。最终 review=`msg_bfe30b3e23574d6291f33b9a88baddde`，result=`msg_81187e032a1245d5b566f9238a7959ab`。

Grok B 独立 worktree `C:\Users\Administrator\biaoshu-p13i3-grok` 已完成并合并，不再处于在途。Codex 独立串行 I3/H3/freshness **5/15/17 passed**，lint、build、`git diff --check`、严格四文件边界通过；Playwright 单 worker、零重试，未运行整仓 E2E、后端全量或并发测试。下一包必须重新只读审计并冻结，不能沿用 I3 白名单。

## V1-H1 章节生成空白正文质量门（已完成）

契约=`docs/v1h1-writer-empty-body-quality-contract.md`，计划=`docs/plans/2026-07-22-v1h1-writer-empty-body-quality-plan.md`，冻结=`a1cee86`，实现=`d550dd9`。

1. `chapter/chapters` 共用生成函数只以 `strip()` 判空；空串或混合空白固定 failed，错误为“模型未返回有效章节正文，请重试”。
2. 单章空白零 editor-state 写入；多章首章空白零写且不调用后续章，中途空白只保留此前成功章并停止后续调用。
3. 合法短章“无。”继续 success；带首尾空白的有效 Markdown 原样保存，不增加最小字数、目标字数比例或状态门。
4. Grok B/Codex 独立 failure-first 均为 **4 failed / 2 passed**；Codex 独立串行专项/定向回归 **6/66 passed**，严格两文件、冻结测试哈希、diff-check 和空暂存区通过。

后续包 V1-H2 已完成：历史、手工或部分生成空章仍可导出草稿，但不再静默交付；不复用图片告警，不按字数硬拦。

## V1-H2 技术标导出正文完整性提醒（已完成）

契约=`docs/v1h2-export-content-warnings-contract.md`，计划=`docs/plans/2026-07-22-v1h2-export-content-warnings-plan.md`，冻结=`73a76c6`，实现=`0047a46`。严格六文件让技术标 export result 始终返回独立 `contentWarnings`；存在空章时按数量固定提醒，没有有效章节字典时固定提醒无正文，商务标固定空数组。空章仍保留 Word 占位，提醒不阻断草稿下载；前端最多 20 条、每条 240 Unicode 码点并只用 React 文本节点，项目切换和迟到 success 受既有 generation/准备令牌隔离。

原始 failure-first 后端/前端为 **5 failed / 0 passed**、**4 failed / 0 passed**；没有有效章节字典的 Word 空壳加固红测为 **1 failed / 5 passed**。全页链接假红、宽泛 `or` 与无有效字典边界均经双方确认后才返修。Codex 最终串行通过后端 H2/回归 **6/35 passed**，前端 H2/图片告警/最新编辑态/稳健下载 **4/4/18/14 passed**；lint、build、`py_compile`、diff-check、严格六文件、空暂存和端口清理通过。未运行后端全量或整仓 318 E2E。

## V1-I 创建页招标文件摄入真值（已完成）

契约=`docs/v1i-create-file-intake-truth-contract.md`，计划=`docs/plans/2026-07-22-v1i-create-file-intake-truth-plan.md`，冻结=`c1cde54`，test-only=`35b0f6b`/`b8c9776`，实现=`4e7a3c3`。

技术类 `/create` 现使用真实多选 input/drop；项目创建成功后按原序串行 multipart，全部成功才导航。创建失败零上传；部分上传失败停留原页，锁定能力和文件集合，重试不重建项目、不重传成功项。无文件允许诚实创建；技术工作区只展示 `pipeline.files`，历史 pending 键不读、不迁移、不删除。商务入口、后端、解析器、LLM、数据库与依赖未扩围。

生产未改时新专项/P11A 为 **8 failed / 0 passed**、**1 failed / 9 passed**；两轮测试反假绿和两轮生产/注释问题均先双方确认再返修。Codex 最终独立 V1-I/P11A/技术 editor-state truth **8/10/28 passed**，lint/build/diff-check、严格五文件、空暂存、端口和产物清理通过。完整消息链、五文件 blob/SHA-256 与未运行项见契约 §8。

## V1-J lightweight 无有效正文质量门（已完成并推送）

契约=`docs/v1j-lightweight-no-usable-text-quality-contract.md`，计划=`docs/plans/2026-07-22-v1j-lightweight-no-usable-text-quality-plan.md`，冻结=`1e33f1b`、测试=`3917c3a`、实现=`f9562a5`。A/B 已一致确认全空 PDF/DOCX/TXT/MD 是同一假成功缺口；质量门已落在 `parse_service` 结构提取层，没有在 task 层扫描最终 Markdown。仅空表视为无正文；混合 PDF 与正文+空表继续 success。

知识库共用解析器，只对新异常显示固定中文并物理删除旧 chunk。生产未改时新专项/V1-D 为 **6 failed / 4 passed**、**2 failed / 8 passed**；完整指纹和零下游钩子按 question/YES/返修闭环。Codex 最终独立 V1-J/V1-D/代表回归 **10/10/38 passed**，编译、diff-check、严格四文件、冻结 blob 和空暂存区通过。启动诊断随后已独立冻结为 V1-K；未运行后端全量、前端 E2E、真实数据、真实 CLI/模型或联网安装。

## V1-K 静默启动诚实诊断（已完成并推送）

契约=`docs/v1k-silent-startup-diagnostics-contract.md`，计划=`docs/plans/2026-07-22-v1k-silent-startup-diagnostics-plan.md`，冻结=`8f0c137`，测试=`8f0c366`/`1fdea4c`/`997b57c`，实现=`cd4102f`。A/B 两路审计一致确认缺 venv/npm、foreign 端口与 Hidden 子进程早退均缺稳定可见诊断；五入口现已委托唯一 UTF-8 BOM 真源，状态固定七键，显式 Diagnose 只读，Stop 未改。

生产未实现时 Codex 独立 failure-first 为 **44 failed / 14 passed / 19 subtests passed**。首轮生产专项 **58 passed / 19 subtests passed** 后，Q4/Q5/Q6 又关闭先删后移、无关 Replace 参数假证明和 PS5.1 双 Replace 异常回退；最终只有一处 `[System.IO.File]::Replace($tempPath, $StatusFinal, [NullString]::Value)`。Codex 最终独立 V1-K/V1-A 回归 **67 passed / 19 subtests passed**、**65 passed**，编译、Parse/BOM、diff、白名单、Stop 哈希和空暂存区通过。

当前 backend/Vite 仍固定 loopback，同一内网其它电脑不可直接访问的缺口真实，严格拆为 V1-L。只读审计和安全契约现已冻结；下一步只能先写 failure-first，仍禁止脱离契约直接改 `0.0.0.0`。

## V1-L 可信内网访问（已冻结，尚未实现）

契约=`docs/v1l-trusted-lan-access-contract.md`，计划=`docs/plans/2026-07-22-v1l-trusted-lan-access-plan.md`，基线=`ca7223a`。A/B 审计 review=`msg_092d74fd43f2402bb6f8a783d65274f3`/`msg_7d4576573bd049e68d828ed1e3ea5c74`；Q1 双确认 YES=`msg_33d84617361f434e93d6913c2968e15c`/`msg_0a67657dd9df443ba1e9eeddfb95409a`。

冻结拓扑为“内网浏览器 → 显式 RFC1918 IPv4 上的 Vite 5173 → 回环 FastAPI 8000”。默认仍 loopback；LAN 必须显式 profile/host、强制 `AUTH_MODE=required` 并先证明 `authRequired=true`；前端保持相对 `/api`，不扩 CORS、不改 Strict Cookie、不跨源直连后端。Vite Host 只允许有限精确值；IPv6、通配监听、自动防火墙、WAL/PostgreSQL、HTTPS/公网和 Docker 后置。

V1-K 状态继续固定七键与单次原子替换。防火墙只在文档给出 Private + LocalSubnet + TCP 5173 的管理员手工创建/删除步骤，生产代码不得写系统策略。当前只有文档冻结，failure-first、生产实现、串行验收和真实远端设备可达性均未完成，因此实时 V1 粗估继续为 **93%**，不得因审计或文档提交上调。
