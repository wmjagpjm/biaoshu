...
> **交接日期**：2026-07-15（P11A/P11B/P11C 真实数据收口与 P8D/P8E 本机解析助手均已完成）
...
- **已验证基线**：后端 M3-D 专项 34 passed、受影响回归 71 passed、串行全量 487 passed（只有 1 条既有 Starlette/httpx 弃用警告）。P8E 本轮独立通过 Docling 46、MinerU 54、P8E-A/P8C/P8B/解析后端受影响回归 37 passed。前端 P11C 18、P11B 11、P11A 10、认证/RBAC 11、解析策略 6、响应矩阵 8、融合确认 6、持久恢复 5、模板 1 passed，单 worker 串行全量 E2E 184 passed；P8E 本轮逐条串行复跑 P8C 9、P8B 6 passed；lint 零问题，build 通过（仅既有大 chunk 警告），实现暂存区 whitespace 检查通过。P8E 未重跑全量，故 487/184 是沿用基线而非本包结果。**E2E 共用 SQLite 重置库，所有 Playwright 命令必须串行。**
...
- **其余未实现主线**：所有 editor-state 的通用版本历史/任意历史浏览回滚/多人协作；MinerU/Docling 自动安装、模型打包、常驻服务、真实模型样本验收与完整孙进程治理；P9C 后续真实语义调优；Word `structure`/整章布局；除国能 e 招外的合法外部标讯来源；人力附件/真实证件核验；财务税务/审批/导出/预算/回款/版本、失败尝试与完整身份审计；投标人矩阵明细/版本/结果跟踪；Alembic、PostgreSQL、HTTPS、Key 加密、Docker 和公网 SaaS 能力。任何一项都须另立契约，不得扩大既有角色与生产路径。
