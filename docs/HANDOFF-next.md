...
> **交接日期**：2026-07-16（P12C-C1 修订历史只读列表/详情已完成并推送）
...
> **协作分支已推送功能基线**：P12C-C1 冻结=`26b504e`、实现=`7023ecd`；P12C-B-D3 冻结=`1d44484`、实现=`b91a7ff`；P12C-B-D2 冻结=`6b83fc1`、实现=`f256f5b`；P12C-B-D1 冻结=`e8ffaeb`、实现=`a6a28f6`；P12C-B-C2 冻结=`52bbabf`、实现=`82cc82e`；P12C-B-C1 冻结=`76834f5`、实现=`1d0ce0e`；P12C-B-B2 冻结=`3a30c03`、实现=`5149385`；P12C-B-B1 冻结=`05864f6`、实现=`5a0d1c0`；P12C-B-A 冻结=`fbf93c0`、实现=`acf3139`；P12C-A 冻结=`daa8c43`、实现=`226e1c1`；P12B-D 冻结=`613818f`、D1 后端=`551caba`、D2 前端=`0f81dd6`；P12B-C 冻结=`b5a9d90`、C1=`0c8fc77`、C2=`f3c05ae`、C3=`59fcd50`；P12B-B 契约/计划=`0636302`、实现=`473e823`；P12B-A 计划/契约=`0b55c30`、实现=`780cc82`；P12A 计划/契约=`bf8ccd6`、后端=`9f53d92`；其余既有功能基线见本文 §11。新会话必须以 `git rev-parse HEAD` 与远端分支一致为准。
> **最新增量基线**：P12C-C1 冻结=`26b504e`、实现=`7023ecd`；公开列表只读最近 10 条元数据，详情按需重验规范快照，尚无 revision restore 或前端入口。
> **参考 `origin/main`**：`4847a9d` — docs: 重写换会话交接并强制注释规范专章（非当前工作 HEAD）
> **本地状态**：P12C-C1 实现=`7023ecd` 已推送，本文档闭环提交位于其后。新会话仍须重新核对本地 HEAD、远端 SHA 与工作区。
> **验收基线**：P12C-C1 专项/扩大回归 **13/201 passed**，后端串行全量 **777 passed**（1 条既有 Starlette/httpx 弃用告警）；P12B-D2 前端专项/受影响回归 **51/63 passed**，单 worker、零重试前端全量 E2E **263 passed**；五文件 `py_compile`、真实坏时间 HTTP、白名单与 diff 检查通过。P12C-B-D3 历史基线为 18/270/764，P12C-B-D2 为 25/299/746，P12C-B-D1 为 11/285/732，P12C-B-C2 为 20/272/721，P12C-B-C1 为 10/224/711，P12C-B-B2 为 11/147/701，P12C-B-B1 为 10/126/690，P12C-B-A 为 14/107/680，P12C-A 为 67/77/666，P12B-D1 为 58/81/599，P12B-C 历史后端/前端全量为 570/212，Docling 46、MinerU 54 继续保留。**所有 Playwright E2E 共用 SQLite 重置库，必须逐条串行运行，禁止并行。**
...
当前进度：P12A、P12B-A/B/C/D、P12C-A、P12C-B-A、P12C-B-B1、P12C-B-B2、P12C-B-C1、P12C-B-C2、P12C-B-D1、P12C-B-D2、P12C-B-D3、P12C-C1、P9D、M3-A 至 M3-D、P8B/P8C/P8D/P8E、P9A/P9B/P9C、P10A 至 P10K、P11A/P11B/P11C 均已完成。C1 冻结=`26b504e`、实现=`7023ecd`；后端全量 777、前端全量 263 passed。
当前执行包：P12C-C1 已交付当前项目最近 10 条修订元数据列表与单条按需详情；列表不加载正文，详情三重作用域并严格重验规范 13 键，所有成功/业务错误 no-store。
下一步：只读审计并冻结 P12C-C2 受限 revision restore；必须重新定义 expectedStateVersion、恢复前安全检查点、目标快照重验、写回/新 revision 时间点、两个配额裁剪与失败全回滚。不得合入前端、删除、diff、搜索、跨项目历史或多人协作。
...
| 编辑态有限修订与只读历史 P12C | `models/entities.py`、`services/editor_state_revision_service.py`、`services/editor_state_revision_history_service.py`、`api/editor_state_revisions.py`、`api/schemas.py`、`tests/test_p12c_revision_history_read.py` | **齐** | 最近 10 条、八类原子来源、列表五列无正文、详情三重作用域与规范重验、损坏固定 no-store；C1 独立 13/201/777 passed |
...
### 4.23 P12C-A/B-A/B-B1/B-B2/B-C1/B-C2/B-D1 editor-state 有限自动修订账本
...
P12C-C1 已完成并推送：冻结=`26b504e`、实现=`7023ecd`。列表固定最近 10 条 `id/state_version/snapshot_bytes/source_kind/created_at` 投影，不读取 `snapshot_json`；详情以 revision/workspace/project 三重作用域读取并严格重验规范 JSON、字节和版本。Grok 首版 failure-first **12 failed / 0 passed**、专项 13；Codex 审查真实复现坏时间在 ORM 物化阶段裸 500，返修 failure-first **1 failed / 12 passed** 后关闭真实越界字节、非法来源、坏时间和正文损坏的固定 500/no-store 门。Codex 独立通过 **13/201/777 passed**，Grok 回执=`msg_1ea43d2ffaf94129aa18badfa1da9180`，Codex 确认=`msg_a61704c81ee24b98bda1d56f5a5f7cd9`。C2 revision restore 与前端仍未开始。
...
| 技术标 | 通用版本、响应矩阵与解析增强 | M3-D 已交付服务端原子确认和最近 20 批一次性漂移安全恢复；P12B-A/B/C/D 已完成全状态版本、浏览器 CAS、所有既有延迟写入围栏、恢复前安全检查点、原子 restore 与双工作区显式入口；P12C-A/B-A/B-B1/B-B2/B-C1/B-C2/B-D1/B-D2/B-D3 已交付独立最近 10 条修订账本和八类内部来源接入，P12C-C1 已交付公开元数据列表与按需详情。响应矩阵已完成来源分页、字段级三方合并和冲突保护；P8B/P8C/P8D/P8E 已接策略、单次票据与本机 MinerU/Docling 助手。仍未接：revision 受限恢复及前端入口、删除、diff、搜索、跨项目历史、多人协作、解析器自动部署/模型打包、真实模型验收与其他交付增强 |
...
4. P12B-A/B/C/D、P12C-A、P12C-B-A/B1/B2/C1/C2/D1/D2/D3 与 P12C-C1 已完成。下一步只读审计并冻结 P12C-C2 受限 revision restore；必须复用已校验详情边界并重新证明 CAS、安全检查点、新修订时间点和失败全回滚，禁止合入前端、删除、diff、搜索、跨项目历史或多人协作。
...
- **Grok 当前状态**：额度可用；P12C-C1 首版=`msg_67307de85a9d47528cdbc4987da252c8`、物化损坏返修任务=`msg_a002407f293a4c1b8f018c90a2a153c9`、返修回执=`msg_1ea43d2ffaf94129aa18badfa1da9180`，Codex 确认=`msg_a61704c81ee24b98bda1d56f5a5f7cd9`。协作方式仍是 Grok 只实现/自测、不提交推送，Codex 独立规划、审查、验收、中文提交与推送；Grok 当前等待下一份 C2 冻结任务，不得自行开工。
- 当前分支仍为 `collab/grok-code-codex-review`；P12C-C1 实现=`7023ecd` 及 P12C-B-D3/D2/D1/C2/C1/B2/B1/B-A、P12C-A、P12B-D/C/B/A、P12A、P8D、P11C/B/A、M3-D、P10K、P8C、P10J、P9D、M3-C、P10I/H/G/F/E 与 P8B 基线均已推送，本文档闭环提交位于其后。新会话第一步必须用 `git status -sb`、`git rev-parse HEAD`、`git rev-parse origin/collab/grok-code-codex-review` 重新核验，不可只信本文静态 SHA。
...
- **已验证基线**：P12C-C1 专项/扩大回归 13/201、后端串行全量 777 passed（只有 1 条既有 Starlette/httpx 弃用告警）；P12C-B-D3 历史基线为 18/270/764，P12C-B-D2 为 25/299/746，P12C-B-D1 为 11/285/732，P12C-B-C2 为 20/272/721，P12C-B-C1 为 10/224/711，P12C-B-B2 为 11/147/701，P12C-B-B1 为 10/126/690，P12C-B-A 为 14/107/680，P12C-A 为 67/77/666；P12B-D2 专项/受影响回归 51/63、单 worker、零重试前端全量 E2E 263 passed；五文件 `py_compile`、真实坏时间 HTTP、白名单、工作树与暂存区 diff 检查通过。P12B-D1 历史基线为 58/81/599；D2 全量首跑 239 passed 后单次纯白页，精确 1 项与完整 263 项重跑均通过。P12B-C 历史基线为后端/前端全量 570/212；P8E 独立 Docling 46、MinerU 54 继续保留。**E2E 共用 SQLite 重置库，所有 Playwright 命令必须串行。**
...
- **其余未实现主线**：P12C-C2 受限 revision restore、P12C 历史前端与多人协作；修订删除、diff、搜索、跨项目历史与自动定时历史；MinerU/Docling 自动安装、模型打包、常驻服务、真实模型样本验收与完整孙进程治理；P9C 后续真实语义调优；Word `structure`/整章布局；除国能 e 招外的合法外部标讯来源；人力附件/真实证件核验；财务税务/审批/导出/预算/回款/版本、失败尝试与完整身份审计；投标人矩阵明细/版本/结果跟踪；Alembic、PostgreSQL、HTTPS、Key 加密、Docker 和公网 SaaS 能力。下一步只读审计并冻结 P12C-C2 的最小恢复边界；不得把只读详情直接当作恢复已交付。
...
**换会话可直接：核验分支、HEAD/远端与工作区 → 读本文 §0～§3.1、§4.23、§5、§6、§11、P12C 契约/计划及路线图 → 确认 P12C-C1 冻结 `26b504e`、实现 `7023ecd` 已推送且后端/前端全量 777/263 passed → 只读审计并冻结 P12C-C2 受限 revision restore，明确 expectedStateVersion、恢复前安全检查点、目标重验、写回/新 revision、裁剪与失败全回滚。禁止重新实现 P12C-C1/B-D3/B-D2/B-D1/B-C2/B-C1/B-B2/B-B1/B-A/P12C-A/P12B-A/B/C/D/P12A、把只读详情直接当作恢复、合入前端/删除/diff/多人协作、让 Grok commit/push，或由 Codex 冒充 Grok 完成主实现。**
