...
当前执行包：P12E-A 已闭环。Grok 只实现七文件并发送 review_request，Codex 独立审查、验收、提交和推送；不得让 Grok 提交或推送。
下一步：先独立审计剩余版本历史需求与产品优先级，再冻结下一包并通过消息箱交 Grok；不得沿用 P12E-A 白名单顺手扩成任意历史比较、完整时间线、删除、搜索、分页或多人协作。
...
| 技术标 | 通用版本、响应矩阵与解析增强 | M3-D 已交付最近 20 批恢复；P12B 已完成全状态版本、围栏、检查点恢复；P12C-A/B/C 已完成最近 10 条九来源修订、列表、按需摘要和双工作区恢复；P12D-A/B 已完成后端字段摘要和双工作区前端“与当前对比”；P12E-A 已完成单条修订对当前状态的有界正文差异。仍未接：任意历史两两比较、删除、搜索、跨项目历史、完整历史/保留策略、多人协作、解析器自动部署/模型打包、真实模型验收与其他交付增强 |
...
4. P12B-A/B/C/D、P12C-A/B/C、P12D-A/B 与 P12E-A 已完成；P12D-B 冻结=`fc19d93`、实现=`35ab377`、闭环=`c7cf67f`；P12E-A 冻结=`5aa205c`、实现=`f9f067e`，P12E-A 后端/前端全量 **854/290 passed**。禁止把单条正文差异扩成任意历史两两比较、删除、搜索、跨项目历史或多人协作。
...
- **其余未实现主线**：任意历史两两比较、修订删除、搜索、跨项目历史、超出最近 10 条的完整历史/保留策略与多人协作；MinerU/Docling 自动安装、模型打包、常驻服务、真实模型样本验收与完整孙进程治理；P9C 真实用户语料调优、其他模型/GPU/在线 embedding/自动更新；Word `structure`/整章布局；除国能 e 招外的合法外部标讯来源；人力附件/真实证件核验；财务税务/审批/导出/预算/回款/版本、失败尝试与完整身份审计；投标人矩阵明细/版本/结果跟踪；Alembic、PostgreSQL、HTTPS、Key 加密、Docker 和公网 SaaS。
...
**换会话可直接：核验分支、HEAD/远端与工作区干净 → 读本文 §0～§3.1、§4.23、§5、§6、§11、P12D-A/B 与 P12E-A 契约/计划及路线图 → 确认 P12E-A 实现=`f9f067e` 已推送、后端/前端全量 854/290 passed → 先独立审计并冻结下一包，再通过消息箱交 Grok。禁止清理或重做已交付包、让 Grok commit/push、扩成任意历史比较/删除/分页/多人协作，或由 Codex 冒充 Grok 完成主实现。**
