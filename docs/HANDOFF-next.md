...
> **交接日期**：2026-07-19（P12F-J-B 已闭环，P12G 已完成只读审计并待冻结提交）
...
> **最新增量基线**：P12F-J-B 已交付七键/八键固定状态读取、严格前端 parser、技术/商务共用固定入口、全局单飞和迟到隔离，实现=`5ef7abd`、文档闭环=`dfb6b1e`。P12G 检查点展示名称已完成只读审计，契约/计划待本次提交冻结。
> **参考 `origin/main`**：`4847a9d` — docs: 重写换会话交接并强制注释规范专章（非当前工作 HEAD）
> **本地状态**：只允许分支 `collab/grok-code-codex-review`；P12F-J-B 实现与文档闭环均已推送，冻结 P12G 前 HEAD/跟踪远端=`dfb6b1eacb2e31c8d00d450ec57514caba674961` 且工作区干净。
> **验收基线**：P12F-J-B Codex 独立后端专项/全量 **297/1170 passed**；前端 P12F-J-B/history/checkpoint/技术 truth/商务 truth **6/61/51/28/18 passed**，整仓前端沿用已验收 **318 passed** 基线。仅保留既有 pytest 弃用警告与 build 大 chunk 提示。**所有 pytest 与 Playwright E2E 共用 SQLite 重置库，pytest 禁止 xdist/并发分组，Playwright 必须显式 `--workers=1 --retries=0` 逐条串行运行。**
...
当前进度：P12A、P12B-A/B/C/D、P12C-A/B/C、P12D-A/B、P12E-A/B/C、P12F-A/B/C/D/E-A/E-B/F-A/F-B/G-A/G-B/H/I/J-A/J-B、P13-A、P9D、P9C-R1、M3-A 至 M3-D、P8B/P8C/P8D/P8E、P9A/P9B/P9C、P10A 至 P10K、P11A/P11B/P11C 均已完成。P12F-J-B 实现=`5ef7abd`、文档闭环=`dfb6b1e`；后端/前端有效基线为 1170/318 passed。
当前状态：修订历史已闭合来源、UTC 时间、联合搜索、游标分页、单条删除、展示名称、固定与保护性裁剪；检查点已有创建、列表、详情和安全恢复，但尚无展示名称。
当前执行包：P12G 手动检查点展示名称已完成只读审计；契约=`docs/p12g-checkpoint-display-name-contract.md`，计划=`docs/plans/2026-07-19-p12g-checkpoint-display-name-plan.md`，本次中文文档提交即冻结。
下一步：冻结提交并推送后，通过消息箱让 Grok 严格十二文件先做 failure-first 再实现；Grok 不暂存、不提交、不推送。检查点搜索/固定/删除、跨项目历史和多人协作继续不得混入。
...
## P12G 手动检查点展示名称（已冻结待实现，2026-07-19）

契约=`docs/p12g-checkpoint-display-name-contract.md`，计划=`docs/plans/2026-07-19-p12g-checkpoint-display-name-plan.md`；本次中文文档提交即冻结。冻结前 HEAD/跟踪远端均为 `dfb6b1eacb2e31c8d00d450ec57514caba674961`，工作区干净；现有十文件 SHA-256 见契约第 9 节，另允许新增检查点名称服务和后端专项测试，总计严格十二文件。
...
Grok 必须先改测试形成真实 failure-first，再实现生产文件；pytest 与 Playwright 全部逐条串行，禁止 xdist、并发分组和 `--workers>1`。Grok 只通过消息箱发送任务进度/question/review_request，不暂存、不提交、不推送。Codex 负责十二文件审查、受限返修、独立验收、中文提交推送和文档闭环。
