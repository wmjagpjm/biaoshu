...
| 技术标 | 通用版本、响应矩阵与解析增强 | M3-D 已交付最近 20 批恢复；P12B 已完成全状态版本、围栏、检查点恢复；P12C-A/B/C 已完成最近 10 条九来源修订、列表、按需摘要和双工作区恢复；P12D-A/B 已完成后端字段摘要和双工作区前端“与当前对比”；P12E-A/B/C 已完成单修订对当前及双历史修订的有界正文差异。仍未接：自动批量比较、完整时间线、删除、搜索、跨项目历史、完整历史/保留策略、多人协作、解析器自动部署/模型打包、真实模型验收与其他交付增强 |
...
4. P12B-A/B/C/D、P12C-A/B/C、P12D-A/B 与 P12E-A/B/C 已完成；P12D-B 冻结=`fc19d93`、实现=`35ab377`、闭环=`c7cf67f`；P12E-C 冻结=`8b40bf4`、实现=`b6a4375`，后端/前端全量基线 **867/293 passed**。分页、删除、搜索、跨项目历史或多人协作仍需另行冻结。
...
- **其余未实现主线**：修订自动批量比较、完整时间线、删除、搜索、跨项目历史、超出最近 10 条的完整历史/保留策略与多人协作；MinerU/Docling 自动安装、模型打包、常驻服务、真实模型样本验收与完整孙进程治理；P9C 真实用户语料调优、其他模型/GPU/在线 embedding/自动更新；Word `structure`/整章布局；除国能 e 招外的合法外部标讯来源；人力附件/真实证件核验；财务税务/审批/导出/预算/回款/版本、失败尝试与完整身份审计；投标人矩阵明细/版本/结果跟踪；Alembic、PostgreSQL、HTTPS、Key 加密、Docker 和公网 SaaS。
