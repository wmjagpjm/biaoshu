...
> **交接日期**：2026-07-19（P12G 已闭环，P12H 单条检查点删除已完成只读审计并待冻结提交）
...
> **最新增量基线**：P12G 已交付检查点 nullable 展示名称、SQLite 幂等迁移、精确单列 PATCH、create/list/detail 七/七/八键、技术/商务共用原位命名入口、真实同步单飞和 A→B 迟到隔离；冻结=`9696ec1`、实现=`077e7d4`、闭环=`7a5345f`。P12H 单条检查点删除已完成只读审计，契约/计划待本次提交冻结。
> **参考 `origin/main`**：`4847a9d` — docs: 重写换会话交接并强制注释规范专章（非当前工作 HEAD）
> **本地状态**：只允许分支 `collab/grok-code-codex-review`；P12G 冻结/实现/闭环均已推送，冻结 P12H 前 HEAD/跟踪远端=`7a5345fea33fc219edc841dd4962e9cfe27c5005` 且工作区干净。
> **验收基线**：P12G Codex 独立后端聚焦/恢复回归/全量 **62/47/1203 passed**；前端 P12G/checkpoint/history/技术 truth/商务 truth **8/59/61/28/18 passed**，整仓前端沿用已验收 **318 passed** 基线且未冒充本包重跑结果。仅保留既有 pytest 弃用警告与 build 大 chunk 提示。**所有 pytest 与 Playwright E2E 共用 SQLite 重置库，pytest 禁止 xdist/并发分组，Playwright 必须显式 `--workers=1 --retries=0` 逐条串行运行。**
...
当前进度：P12A、P12B-A/B/C/D、P12C-A/B/C、P12D-A/B、P12E-A/B/C、P12F-A/B/C/D/E-A/E-B/F-A/F-B/G-A/G-B/H/I/J-A/J-B、P12G、P13-A、P9D、P9C-R1、M3-A 至 M3-D、P8B/P8C/P8D/P8E、P9A/P9B/P9C、P10A 至 P10K、P11A/P11B/P11C 均已完成。P12G 冻结=`9696ec1`、实现=`077e7d4`、闭环=`7a5345f`；后端/前端有效基线为 1203/318 passed。
当前状态：修订历史已闭合来源、UTC 时间、联合搜索、游标分页、单条删除、展示名称、固定与保护性裁剪；检查点已有创建、列表、详情、安全恢复和展示名称。
当前执行包：P12H 单条检查点删除已完成只读审计；契约=`docs/p12h-checkpoint-delete-contract.md`，计划=`docs/plans/2026-07-19-p12h-checkpoint-delete-plan.md`，本次中文文档提交即冻结。
下一步：冻结提交并推送后，通过消息箱让 Grok 严格七文件先做后端与前端 failure-first 再实现；Grok 不暂存、不提交、不推送。模型/迁移/Schema/核心恢复服务、批量/软删除、回收站、固定保护裁剪、跨项目历史和多人协作不得混入。
...
4. P12B-A/B/C/D、P12C-A/B/C、P12D-A/B、P12E-A/B/C 与 P12F-A/B/C/D/E-A/E-B/F-A/F-B/G-A/G-B/H/I/J-A/J-B 已完成；J-B 冻结=`f019a4b`、实现=`5ef7abd`、闭环=`dfb6b1e`，当前后端/前端有效全量基线 **1170/318 passed**。P12G 检查点展示名称已完成只读审计；片段/高亮、缓存、批量、跨项目历史或多人协作仍需另行规划。
...
- **其余未实现主线**：自动批量比较、完整时间线、来源多选、日期预设、名称排序/批量、跨项目历史与多人协作；P12G 检查点命名正在冻结，尚未实现；搜索片段/高亮/游标/缓存；MinerU/Docling 自动安装、模型打包、常驻服务、真实模型样本验收与完整孙进程治理；P9C 真实用户语料调优、其他模型/GPU/在线 embedding/自动更新；Word `structure`/整章布局；除国能 e 招外的合法外部标讯来源；人力附件/真实证件核验；财务税务/审批/导出/预算/回款/版本、失败尝试与完整身份审计；投标人矩阵明细/版本/结果跟踪；Alembic、PostgreSQL、HTTPS、Key 加密、Docker 和公网 SaaS。
...
**换会话可直接：核验分支、HEAD/远端与工作区状态 → 读本文 §0～§3.1、§4.23、§5、§6、§11、P12G 契约/计划及路线图 → 确认 P12F-J-B 冻结=`f019a4b`、实现=`5ef7abd`、闭环=`dfb6b1e` 已推送且后端/前端有效基线 1170/318 passed → 若 P12G 冻结提交已在远端，则通过消息箱让 Grok 严格十二文件 failure-first 实现。禁止让 Grok commit/push、未经契约加入创建时命名、检查点搜索/排序/固定/删除、跨项目历史/多人协作，或由 Codex 冒充 Grok 完成主实现。**
...
## P12H 单条检查点删除（已冻结待实现，2026-07-19）

契约=`docs/p12h-checkpoint-delete-contract.md`，计划=`docs/plans/2026-07-19-p12h-checkpoint-delete-plan.md`；冻结前 HEAD/跟踪远端均为 `7a5345fea33fc219edc841dd4962e9cfe27c5005`，工作区干净。严格七文件为后端检查点路由、既有 405 守卫、新删除服务、新专项测试，以及前端 checkpoint API、共用面板和既有 checkpoint E2E；五个现有文件冻结哈希见契约第 8 节。
...
Grok 必须先只改测试形成真实 failure-first，再实现生产四文件；pytest/Playwright 全部逐条串行，禁止 xdist、并发分组和 `--workers>1`。只通过消息箱发 status/question/review_request，不暂存、不提交、不推送；Codex 负责审查、受限返修、独立验收、中文提交推送和文档闭环。
