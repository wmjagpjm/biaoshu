# P13-F1 在途操作级交接：项目在线租约后端基础

> 交接日期：2026-07-20
> 当前状态：**Grok 已完成受限实现和自测，等待 Codex 独立审查、双确认返修判定与最终验收；不是已完成功能**
...
> Grok 最终审查请求：`msg_b176f13020d5470395f70792f811921b`

本文是当前现场的操作级真值。后续会话必须先读本文，再读主交接、路线图、契约、计划和联调清单。P13-F1 的七个实现文件仍在工作区中且全部未暂存；不得因为 Grok 自测全绿而跳过 Codex 独立审查，也不得在双确认前直接修复疑似问题。
...
```text
继续 biaoshu P13-F1 项目在线租约后端基础的独立审查与验收。仓库 C:\Users\Administrator\biaoshu，只能使用 collab/grok-code-codex-review，禁止操作 main。
...
当前 P13-F1 不是已完成功能：Grok 已在冻结基线 78302bc 上完成严格七文件实现，并发送 review_request=msg_b176f13020d5470395f70792f811921b；Codex 尚未独立完成逐文件审查、测试验收、结果回执、实现提交或文档闭环。

先执行 git status -sb、git rev-parse HEAD、git rev-parse origin/collab/grok-code-codex-review、git diff --cached --name-only。必须保留 4 个已跟踪修改和 3 个未跟踪新文件，不得 pull、reset、checkout、stash、rebase 或清理。

审查严格七文件：entities.py、models/__init__.py、schemas.py、新 project_presence_service.py、新 project_presence.py、main.py、新 test_p13f1_project_presence.py。重点审查作用域、可信 actor、摘要落库、UTC 租约、8-client 上限、同 client 并发、事务/回滚、50 人快照、安全用户名、级联、精确响应与测试反假绿。
...
1. Codex 对七文件生产与测试差异的完整逐段审查。
2. Codex 对新表约束、索引、级联和 main 唯一装配的独立确认。
3. Codex 对作用域、可信 actor、`X-Workspace-Id` 空头、角色矩阵和项目隔离的独立确认。
4. Codex 对 clientId 原文零落库/零响应/零日志的独立泄漏门。
5. Codex 对同 client 并发、8-client 极限并发和事务回滚的独立证据。
6. Codex 对专项测试反假绿的完整审查。
7. Codex 独立串行 pytest、py_compile 和差异门。
8. 若发现问题，Grok 的只读独立确认与双方确认后的新修复授权。
9. Codex 最终 `result`/ack 消息。
10. P13-F1 实现提交和 GitHub 推送。
11. 契约、计划、路线图、主交接、本文和联调清单的完成态闭环提交。
12. P13-F2 前端 heartbeat/leave/在线成员展示的冻结、实现和验收。

因此任何新会话都不得把 P13-F1 描述为“已完成”“已交付”或“已推送”。
...
本文和本轮状态更新应作为单独的中文文档提交推送，且只暂存 `docs/`。P13-F1 七个实现文件继续留在未暂存区等待验收。后续完成态还必须再有：

1. P13-F1 中文功能实现提交。
2. P13-F1 完成态中文文档闭环提交。
3. 本地 HEAD 与 GitHub 远端协作分支一致、工作区干净的最终确认。
