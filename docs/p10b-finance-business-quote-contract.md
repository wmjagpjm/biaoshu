...
浏览器页面只请求上述两个专用 `GET` 接口。它不以 `/projects`、`/editor-state`、`/settings` 或 `/files` 作为降级路径，也不把业务数据或认证信息写入 localStorage、sessionStorage 或 URL。
...
P10C 之前必须先独立冻结成本、利润、税率、审批、导出和审计查看事件的数据来源、精度、写入人、工作空间隔离与权限矩阵。人力团队推荐和投标人匿名预览/版本/合规数据域同样需要各自的契约；不得因为已经存在角色名称或前端路径而推断授权。
