...
> **状态**：只读审计、P12C-B-B1 九类任务接入与 P12C-B-B2 商务 revise 接入均已完成、独立验收并推送。
> **前置**：P12C-B-A 冻结=`fbf93c0`、实现=`acf3139`、闭环=`a45b2da`，后端/前端全量基线 **680/263 passed**。
> **固定拆包**：B1 九类任务来源 `task`（冻结=`05864f6`、实现=`5a0d1c0`）→ B2 五类商务 revise 来源 `revise`（冻结=`3a30c03`、实现=`5149385`）。两包已分别实现、验收、提交和闭环。
...
B1 闭环后以 `3a30c03` 冻结 B2。B2 只修改 `revise_service.py` 与独立新测试：两个真实写点传固定 `revise`，五类商务成功写回产生修订；结构解析失败/空正文的“只校验版本”200、普通技术 revise 和陈旧 409 均为零修订。B2 未修改 LLM 提示词、返回 Schema、历史 API 或前端。
...
冻结提交 `3a30c03`、实现提交 `5149385`。生产改动仅在 `revise_service.py` 的两个真实 upsert 写点增加服务端字面量 `revision_source_kind="revise"`；结构化四阶段和 `business_parse` 共覆盖五类商务 revise。无字段变化、普通技术 revise、陈旧 expected 与 LLM 期间漂移均不伪造 `revise` 修订。

Grok failure-first 为 **6 failed / 5 passed**，最终专项 **11 passed**、受影响回归 **122 passed**。Codex 独立审查确认真实 ASGI 脱敏 500、recorder/commit 失败双零、commit 前 flush、来源隔离及精确版本断言均成立；独立通过专项 **11 passed**、扩展受影响回归 **147 passed**、后端串行全量 **701 passed**，只有 1 条既有 Starlette/httpx 弃用警告。`py_compile`、精确双文件白名单和 diff 检查全部通过。Grok 最终回执=`msg_1f7714f97a9b4255985e55d3789ab5fd`，Codex 确认=`msg_10bf04a7dcde47428782ac75faac7389`。

B2 只完成商务 revise 自动留史；个人 callback、P8C 一次性本地解析 callback、content-fuse apply/consume 与 checkpoint restore 仍须按各自事务边界审计和拆包。
