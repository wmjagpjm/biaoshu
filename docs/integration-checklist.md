...
## V1-K 静默启动诚实诊断（已冻结，待实现）

契约=`docs/v1k-silent-startup-diagnostics-contract.md`，计划=`docs/plans/2026-07-22-v1k-silent-startup-diagnostics-plan.md`，基线=`94ff7bb`。
...
当前仅冻结，尚无 failure-first、实现或验收数字。测试必须使用 TEMP 假仓和严格注入快照，不得运行真实服务、端口、health、数据库、浏览器或联网安装。
