...
## V1-M 管理式本机 OCR 自动解析（M1 已完成，M2-M4 待完成）

契约=`docs/v1m-managed-local-ocr-runtime-contract.md`，计划=`docs/plans/2026-07-22-v1m-managed-local-ocr-runtime-plan.md`。契约=`9ed3a06`，测试=`3be6d19`/`3b8e42e`/`61dbe38`，生产=`a7d640b`。M1 只计 automated/fake-runtime 完成，不计真实 OCR 可用。
...
4. M2 必须关闭项目多文件只取最新 `files[0]`、parse 三次提交假失败和外部异常写入 task.error 三个问题；所有 source 文件确定性聚合，成功单事务。
5. M3 新增 `managed`，保持 `local` 人工回传原语义；M4 才允许管理员显式安装和真实 OCR 烟测。
6. 当前机器无 MinerU/Docling CLI/模型，Python 3.13 兼容、GPU、模型体积、中文 OCR 均未验证；real-runtime/quality 均 did-not-run，不得把文档、假 CLI 或 `cli_missing` 写成解析器就绪。V1 粗估仍约 94%。
