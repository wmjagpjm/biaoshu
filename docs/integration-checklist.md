...
## P12F-J-A 修订固定与裁剪保护后端基础（实现审查中）

契约=`docs/p12f-revision-pinning-backend-contract.md`、计划=`docs/plans/2026-07-19-p12f-revision-pinning-backend-plan.md`，冻结=`2f03b8c`。选择该包是因为固定/置顶会改变 P12F-A 的连续最新前缀，必须先独立冻结固定数量/容量、项目级锁、保护性裁剪和失败回滚；名称排序、检查点命名、前端固定入口与七键历史响应均不混入。

冻结生产边界不变：`is_pinned BOOLEAN NOT NULL DEFAULT 0` 与 SQLite 0/1 CHECK 迁移；固定最多 5 条且固定快照总和最多 10 MiB；新增单条 `PATCH /api/projects/{projectId}/editor-state-revisions/{revisionId}/pin`，请求精确 `{isPinned:boolean}`，成功一键 200/no-store，超限 409；裁剪先完整校验所有 `snapshot_bytes/is_pinned`，保留全部固定行与最新非固定前缀，总配额仍为 20 条/20 MiB。显式 DELETE 固定行仍允许，list/page/search/detail 继续精确六键。真实 failure-first **15 failed / 71 passed**；定向 **14/95 passed**；首轮全量 **2 failed / 1160 passed**，修复裸 INSERT 服务端默认后只剩旧删除测试字段清单未同步。现以冻结哈希 `E1CE8CBA925022EC6202146879557DC570DE87FB73ADE78A68705BAC7CD1529E` 将该测试加入第九文件，仅允许在一处 `ann_fields` 清单加入 `is_pinned`，随后必须重跑全量。

联调门：Grok 先只写两个后端测试文件形成真实 failure-first，再实现生产六文件；Codex 严格串行跑 pin 专项、核心修订/恢复回归、后端全量、py_compile、SQL/AST/错误脱敏/零写/精确八文件与空暂存区。J-A 完成后，J-B 才能扩展 `isPinned` 元数据、API parser、技术/商务 UI 和 E2E。
