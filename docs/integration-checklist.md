...
1. 单章模型返回空串或混合空白时，任务固定 failed、错误脱敏，目标章与 editor-state 版本完整不变。
2. 多章首章空白时零写入并停止后续生成；中途空白只保留此前成功章，空白章和后续章保持 pending。
3. “无。”等合法短章和带首尾空白的有效 Markdown 必须 success 且原样保存；禁止最小字数或 targetWords 比例门。
4. 逐章 CAS、取消、进度、导出、前端、数据库和依赖不变。

验收：真实 failure-first **4 failed / 2 passed**；Codex 独立专项/定向回归 **6/66 passed**，仅既有 Starlette/httpx warning。下一包为 V1-H2 技术标导出 `contentWarnings`，允许审阅草稿导出但不得静默忽略空章。
...
# P8B/V1-M M3：四策略接线（真实本机 API/任务；managed 使用假 runtime，禁止真实 CLI/模型）
...
3. 严格接受 `cursor`、`editor-state`、`cursor-stale`、`unavailable`；重复键、非法帧、默认 `message`、网络错误和控制帧固定不可用，禁止展示后端原文、ID、项目/空间/actor/client/正文/任务信息；未注册命名事件由原生 EventSource 不可观测，专项只记录真实边界。
...
6. 技术/商务均覆盖 A→B 在途刷新失败隔离；Playwright 固定 `--workers=1 --retries=0` 串行；Codex 独立 H3 `15 passed`、freshness `17 passed`，lint/build/diff-check 通过，不跑整仓 E2E 或并发测试。
...
下一包已完成为 V1-D lightweight DOCX 表格/标题结构及合成 PDF/DOCX 内容质量门；真实 CLI/模型安装、真实标书样本和 V2/V3 能力继续后置。

## V1-D lightweight 文档结构质量门（已完成并推送）

契约=`docs/v1d-lightweight-document-structure-quality-contract.md`，计划=`docs/plans/2026-07-22-v1d-lightweight-document-structure-quality-plan.md`，冻结=`3a1cadf`，实现=`821c136`。

联调验收项：

1. DOCX 只按 `document.element.body` 的 `w:p/w:tbl` 原顺序输出；禁止先段落后表格。
2. `Heading 1` 至 `Heading 6` 精确映射为对应 Markdown 标题；其它非空段落保持普通块。
3. 普通表格首行为表头，按最大列数补空并输出 GFM 分隔；连续空白折叠，`|` 转义，零行表固定输出「（空表）」。
4. TXT/MD/PDF/未知类型、解析引擎与任务协议不变；两页文字 PDF 和两页 blank PDF 的页内顺序与固定诊断均由合成测试锁定。
5. 只使用系统 TEMP 合成数据；不访问数据库、uploads、网络、服务、真实标书、CLI 或模型。

验收证据：Grok B 原始 failure-first **2 failed / 6 passed**，B1-B6 双确认收紧后 **3 failed / 6 passed**；最终测试 SHA-256=`842241C97AB4F570D090197C9CB932CC4F44B12B38B22FA08DB1F7186C8A6884`。Grok A 与 Codex 独立实现后均为专项/回归 **9/10 passed**，`py_compile` 与 `git diff --check` 通过。未运行后端全量或前端 E2E。

下一包已完成为 V1-E 导出前最新编辑态落盘；复杂跨页版式、OCR、真实解析器安装和 V2/V3 继续后置。

## V1-E 导出前最新编辑态落盘（已完成并推送）

契约=`docs/v1e-export-latest-editor-state-contract.md`，计划=`docs/plans/2026-07-22-v1e-export-latest-editor-state-plan.md`，冻结=`2f3beb1`，实现=`2a1b1ec`。

联调验收项：

1. 技术标/商务标存在防抖待保存编辑时，必须先完成既有 editor-state PUT，再创建 export POST；PUT 未完成时 export 精确为 0。
2. 无本地变化时导出不得额外 PUT；409、网络/HTTP 失败、非法成功版本和冲突时不得创建 export。
3. 快速双击只允许一个准备门和一个 export；不依赖 React disabled 的异步刷新冒充单飞。
4. 导出准备等待期间继续编辑时，保存 generation/epoch 必须稳定后才允许 export，顺序不得出现 `export_first`。
5. 项目切换同步作废旧 token；无论旧 export 是否已经 POST，迟到 success 都不得在新项目告警或下载。
6. 后端、export payload、任务正文、图片协议和数据库不变；P9D 图片告警仍保留当前项目成功导出的有限纯文本告警。

验收证据：原始 14 项 failure-first **11 failed / 3 passed**；A1-A3 经 Grok A/B 双确认，扩为 18 项后生产返修前真实 **14 passed / 4 failed**。Codex 最终串行、单 worker、零重试：新专项 **18/18 passed**、图片告警 **4/4 passed**、技术/商务 truth **46/46 passed**；lint/build/diff-check 通过。P9D 旧迟到下载断言经 `msg_fcd1dc83431d4a4b872d9cdcc60d4395`/`msg_6d53ab6d53b64ae19111fcb7c1f55dd5` 双确认后 test-only 修正；完整消息链与六文件 SHA-256 见契约 §8。

下一包已完成为 V1-F 稳健 Word 下载与人读文件名；V2/V3 继续后置。

## V1-F 稳健 Word 下载与人读文件名（已完成）

契约=`docs/v1f-robust-export-download-contract.md`，计划=`docs/plans/2026-07-22-v1f-robust-export-download-plan.md`，冻结=`5df1114`，契约收紧=`ee97701`，实现=`65fe5e6`。

联调验收项：

1. 技术标/商务标 export success 后必须走同源 Blob GET 与一次真实 download 事件；`window.open` 精确为 0，商务标不得消费任意 `downloadPath`。
2. 入口只接受当前项目、success、精确 `export_<8hex>.docx`；路径由当前 projectId/storedName 构造，401/403/404/500、网络失败、错误 MIME、空体和非法结果统一固定脱敏失败。
3. 响应头安全人读名优先，安全 task filename 次之，两者非法/缺失时回退 `标书.docx`；后端中文、空格、非法字符、重复扩展、保留名、100 码点、TAB/DEL/U+0085 与项目改名均按权威名称收敛。
4. 磁盘和 URL 继续使用随机 storedName；下载路由保持 workspace/project/basename/文件存在校验，不按人读名访问磁盘。
5. 下载 GET 挂起时 A→B，A 的 200 已交付后仍零 anchor/download/提示，B 后续导出成功；旧 finally 不得清 B token。
6. P9D 告警先于下载且下载失败不丢告警；V1-E 的保存门、generation/epoch、快速双击单飞和迟到 success 零下载保持。
7. 错误正文、detail、路径、storedName、Cookie、CSRF、正文、token 不进入 DOM/storage/query；Object URL 与 anchor 必须清理。

验收证据：首轮真实 failure-first 后端/新下载/P9D/V1-E 为 **16 failed / 1 passed、13 failed / 0 passed、3 failed / 1 passed、3 failed / 15 passed**。B1-B4、C1-C4、C5/C6 均先 question、双方确认后才返修；U+0085 生产修正前真实 **1 failed / 19 passed**。Grok 最终 review=`msg_b955c06cc8994560b801cddcec8250e7`；Codex 独立串行后端 **20/1 passed**、前端 **14/4/18/28/18 passed**，lint/build/diff-check、精确十文件、空暂存和 8010/5174 清理通过。未运行后端全量或整仓 318 E2E。

下一包已完成为 V1-G 任务成功后编辑态刷新围栏；多章内容质量门随后评估。

## V1-G 任务成功后编辑态刷新围栏（已完成）

契约=`docs/v1g-writer-task-success-refresh-fence-contract.md`，计划=`docs/plans/2026-07-22-v1g-writer-task-success-refresh-fence-plan.md`，冻结=`b9cacd1`，实现=`fb3b58f`。

联调验收项：

1. 技术 `parse/analyze/outline/chapters/chapter` 和商务 `parse/biz_qualify/biz_toc/biz_quote/biz_commit` 必须在 task success 后复核 `startedProjectId + taskGeneration`；旧项目、ABA 或同项目旧 run 均零 editor-state GET、零成功 tip、零项目/步进覆盖。
2. 技术五入口共用唯一 success-reload helper；商务 `runBizTask` 在 refresh、PATCH/GET 和 `setProject` 的每个 await 边界复核所有权，stale 时仍返回真实 task，不伪造 failed/cancelled。
3. 两个 editor-state Hook 在 writeEpoch、timer、loading 和 GET 前拒绝失效项目闭包；入口失效固定 false 且零副作用，初始水合、silent 恢复、P11 冲突/失败卡保持。
4. A running→B 完整就绪→释放 A success，技术五入口和商务资格均零额外 GET/零 sticky；A→B→A 也不得因 projectId 再次相同而接纳旧 success。
5. 同项目技术 analyze 与商务 biz_qualify success 仍分别精确 +1 editor-state GET 并水合；商务既有步进仍为 2。
6. `useProjectPipeline`、I4/SSE、后端任务协议、导出/下载、配置、依赖和数据库均不变。

验收证据：首轮 failure-first **6 failed / 2 passed**；ABA 与精确单 POST 经 `msg_55bcb5327e8d45ba8e588449850fd7ad`/`msg_c0fe6fb76a4e44e5b955f2f1d528d21c` 双确认后，生产前真实 **7 failed / 2 passed**。Grok A review=`msg_36a13fd6fcc046c3b1267f016aa1a829`；Codex 独立串行新专项/技术 truth/商务 truth/I4/I3/H3 **9/28/18/8/5/15 passed**，lint/build/diff-check、严格五文件、冻结测试哈希、空暂存和 8010/5174 清理通过。未运行后端 pytest、整仓 318 E2E 或并发测试。

下一包已完成为 V1-H1 章节生成空白正文质量门；V1-H2 也已完成技术标导出正文完整性有限告警，复杂跨页版式、OCR、真实解析器安装及 V2/V3 继续后置。

## V1-H2 技术标导出正文完整性提醒（已完成）

契约=`docs/v1h2-export-content-warnings-contract.md`，计划=`docs/plans/2026-07-22-v1h2-export-content-warnings-plan.md`，冻结=`73a76c6`，实现=`0047a46`。

联调验收项：

1. 技术标 export result 必须始终包含独立 `contentWarnings: string[]`；商务标固定空数组，不得复用 `imageWarnings`。
2. 有有效章节字典时只按 `body.strip()` 统计空章并返回一条固定数量提醒；没有有效章节字典时返回一条固定无正文提醒，且 Word 不出现“四、正文”空壳标题。
3. 空章仍写“（本章暂无正文）”并允许下载审阅草稿；合法短章和有正文的 pending 章零误报。
4. 前端独立显示“正文完整性提醒”，最多 20 条、每条 240 Unicode 码点，只作为 React 文本节点；非数组、非字符串和空白项丢弃。
5. 新导出同步清旧提醒；项目切换、A→B→A、同项目旧导出与迟到 success 均受现有 generation 和准备令牌隔离，下载失败不抹掉已接纳提醒。

验收证据：后端/前端原始 failure-first **5 failed / 0 passed**、**4 failed / 0 passed**，无有效章节字典的 Word 空壳加固红测 **1 failed / 5 passed**。测试强度和生产边界全部先 question、Grok 只读确认后才授权返修。Codex 最终串行后端 H2/回归 **6/35 passed**，前端 H2/图片告警/最新编辑态/稳健下载 **4/4/18/14 passed**；lint、build、`py_compile`、diff-check、严格六文件、哈希、空暂存和 8010/5174 清理通过。未运行后端全量或整仓 318 E2E。

## V1-I 创建页招标文件摄入真值（已完成）

契约=`docs/v1i-create-file-intake-truth-contract.md`，计划=`docs/plans/2026-07-22-v1i-create-file-intake-truth-plan.md`，冻结=`c1cde54`，测试=`35b0f6b`/`b8c9776`，实现=`4e7a3c3`。

联调验收项：

1. 点击/键盘上传区打开真实多选 input；drop 保留全部真实 `File`，显示真实 basename/大小，空 drop 零演示文件。
2. 技术类精确一次 create 后按选择顺序串行 multipart；字段恰好一个 `file` part，独立 filename/字节锚点正确，全部成功才导航。
3. create 失败零上传；部分上传失败停止后续，停留 `/create` 并锁定能力/文件集合；重试复用同一 projectId，不重传成功项。
4. 无文件允许一次 create、零 file POST；工作区显示服务端空态。历史 pending 键不读、不迁移、不删除，服务端 files 是唯一展示和解析门真值。
5. 项目 JSON 保持五键，不含文件名或内容；固定中文错误不得泄漏响应 detail、路径、项目 ID、字节锚点、Cookie 或 CSRF。
6. 商务入口保持原导航；后端、工作区手工上传、解析器、LLM、数据库、配置和依赖不变。

验收证据：生产未改时新专项 **8 failed / 0 passed**、P11A **1 failed / 9 passed**；反假绿与生产问题均按双方确认门返修。Codex 最终严格串行 V1-I/P11A/技术 editor-state truth **8/10/28 passed**，lint、build、diff-check、严格五文件、空暂存、8010/5174 和产物清理通过。未运行后端 pytest、整仓 318 E2E、真实数据库/uploads 或并发测试。

## V1-J lightweight 无有效正文质量门（已完成并推送）

契约=`docs/v1j-lightweight-no-usable-text-quality-contract.md`，计划=`docs/plans/2026-07-22-v1j-lightweight-no-usable-text-quality-plan.md`，冻结=`1e33f1b`、测试=`3917c3a`、实现=`f9562a5`。

1. 全空/空白 TXT、MD、markdown，空 DOCX、仅空表 DOCX，零页/全空 PDF 必须抛固定中文 `NoUsableTextError`。
2. 项目 parse 必须 failed，零 editor-state/project step 写入；不得放行 analyze/outline/LLM。
3. 混合 PDF 至少一页有文字时 success，空页提示数量/顺序保持；正文+空表 DOCX 继续 success。
4. 知识库空文档 failed、固定中文、旧 chunk 物理清除、计数 0，文档/磁盘保留；有正文仍 ready 可检索。
5. 只允许 `parse_service.py`、`knowledge_service.py`、新 V1-J 测试和 V1-D 合成测试；禁止 OCR、依赖、前端、数据库和启动脚本扩围。

验收证据：生产未改时新专项 **6 failed / 4 passed**、V1-D **2 failed / 8 passed**，Codex 独立复跑一致。完整状态指纹与零 analyze/outline 钩子经 question/YES=`msg_ad190c69b3c5485da46203af2390f29d`/`msg_f022456ba93948ee8e235c71fdb15fa2` 确认后返修；生产 review=`msg_adfaa73157f9476ea33a3a9b66703abb`。Codex 最终独立 V1-J/V1-D/解析引擎与导出及知识库代表回归 **10/10/38 passed**，`py_compile`、diff-check、严格四文件、冻结 blob 和空暂存区通过。未运行后端全量、前端 E2E、真实数据库/uploads、真实 CLI/模型或联网安装。启动失败诊断后置，未来方案必须保持默认后台静默、不弹终端/浏览器、不抢焦点。

## V1-K 静默启动诚实诊断（已完成）

契约=`docs/v1k-silent-startup-diagnostics-contract.md`，计划=`docs/plans/2026-07-22-v1k-silent-startup-diagnostics-plan.md`，冻结=`8f0c137`，测试=`8f0c366`/`1fdea4c`/`997b57c`，实现=`cd4102f`。

1. 五个既有启动入口只能薄委托唯一真源；默认 Hidden、无浏览器、无 `Read-Host`/pause。
2. 缺 venv/npm/node_modules、listener 无法枚举、foreign/mixed listener、owned 未就绪与派生超时均固定失败，不得把 LISTENING 或 Start-Process 成功冒充 ready。
3. backend 固定 `/api/health` 且要求 `status=ok/dbOk=true`；frontend 固定回环 `/create`；诊断/plan 零启动、零停止、零真实端口与数据。
4. `tmp/dev-start-status.json` 原子覆盖、固定七键与有限枚举；禁止 PID、路径、cmdline、异常原文、日志、密钥、Cookie、数据库/uploads 或正文。
5. Stop 脚本不改；本包不安装依赖、不改业务、host/CORS/Cookie 或端口。可信内网访问另立 V1-L。

验收证据：Codex 独立 failure-first **44 failed / 14 passed / 19 subtests passed**；首轮生产专项 **58 passed / 19 subtests passed**。Q4/Q5/Q6 双确认后关闭先删后移、无关 Replace 参数假证明和 PS5.1 双 Replace 异常回退；最终 Codex 独立 V1-K/V1-A 回归 **67 passed / 19 subtests passed**、**65 passed**。`py_compile`、两个 PS1 ParseFile/UTF-8 BOM、diff-check、严格七生产文件、Stop 哈希和空暂存区通过。测试只使用 TEMP 假仓和严格注入快照，未运行真实服务、端口、health、数据库、浏览器或联网安装。

## V1-L 可信内网访问（已完成并推送）

契约=`docs/v1l-trusted-lan-access-contract.md`，计划=`docs/plans/2026-07-22-v1l-trusted-lan-access-plan.md`。A/B 审计与决策均已双确认；测试冻结=`ea01c48`，夹具修正=`7c9266e`/`b0f197e`，生产实现=`10b5f3e`。

联调验收项：

1. 默认五入口仍只监听 `127.0.0.1`；LAN 只能显式 `-ListenProfile lan -LanHost <RFC1918 IPv4>`，缺参、通配、IPv6、非私有地址和未知组合全部 fail-closed。
2. LAN 只暴露 Vite 5173；FastAPI、health、OpenAPI/docs 与解析回调保持 `127.0.0.1:8000`，浏览器业务 URL 只用同源 `/api`。
3. LAN 必须 required；新启动或 owned 后端均须先经回环握手证明 `authRequired=true`，再允许前端监听。false、非法、超时、foreign/mixed 均不得出现 LAN 暴露窗口。
4. Vite bind 精确等于 LanHost，allowedHosts 仅精确 IP + loopback，proxy target 精确回环；禁止 `allowedHosts=true`、点前缀通配、自动信任网卡或环境改成外部后端。
5. 不扩 CORS、不改 Cookie Strict/Path/HttpOnly、不新增跨源 EventSource/下载/上传分支；现有 Cookie、CSRF、workspace 与 RBAC 继续作为唯一授权边界。
6. V1-K 状态保持顶层七键、有限 code 与单次原子替换；诊断、plan、测试零真实进程、端口、HTTP、防火墙、浏览器、数据库/uploads 与联网。
7. Windows 防火墙只文档化管理员手工 Private + LocalSubnet + TCP 5173 规则及精确删除回滚；生产代码零系统策略写入，8000 永不放行。
8. 最终真实烟测使用隔离数据库/uploads 与临时账号；验证 LAN URL 的登录、Cookie/CSRF、代表 GET/POST、SSE、上传和下载。无第二台设备时必须明确“远端可达性未验证”。
9. IPv6、WAL/PostgreSQL、在线热备、HTTPS/公网、Docker 与深度多人协作不进 V1-L。

验收证据：Codex 最终独立 Q8 定点 **5 passed**、V1-L **56 passed / 68 subtests passed**、V1-K **67 passed / 19 subtests passed**；前端 lint/build、`py_compile`、两个 PS1 ParseFile/UTF-8 BOM、diff-check、严格白名单、七键/单次 Replace、Stop 哈希与 TEMP 清理通过。未运行真实 LAN 服务、防火墙、隔离数据库/uploads、真实管理员登录或第二台设备可达性验证，均明确保留为部署未验证项。V1 产品覆盖率粗估更新为约 **94%**。

## V1-M 管理式本机 OCR 自动解析（M1-M3 已完成，M4 待执行）

契约=`docs/v1m-managed-local-ocr-runtime-contract.md`，计划=`docs/plans/2026-07-22-v1m-managed-local-ocr-runtime-plan.md`。M1 生产=`a7d640b`；M2 测试=`df85ac7`、生产=`86d5206`；M3 初始 test-only=`ea2ba31`/`d3748bd`/`c723636`、后续返修 test-only=`3521e12`/`b521d5f`/`cc58960`/`abe8afc`/`3fbd400`、production=`eb64dc1`。M1-M3 只计 automated/fake-runtime 与前后端接线完成，不计真实 OCR 可用。

1. M1 已用两页 image-only PDF + pypdf 空文本层建立 OCR 真值门；ASCII 与 Windows 中文 profile、同根 symlink/junction no-follow 均有反假绿测试。
2. MinerU 全部可写 HOME/APPDATA/TEMP/cache 与 cwd 已绑定单次 TEMP/output；V1-C 零参兼容保持。
3. 仓外 manifest 精确五键，CLI/model marker/requiredFreeBytes 均解析在 runtime 根和目标卷内；不硬编码 D 盘或未经核验的 20 GB。Codex 独立 managed/MinerU/V1-C/Docling=`29/56/26/46 passed`，TEMP left=0。
4. M2 已关闭项目多文件只取最新 `files[0]`、parse 三次提交假失败和外部异常写入 task.error：全部 source 按 `created_at ASC,id ASC` 聚合，成功五域单事务；锁等待/文件/任务超时、取消 rollback、source/runtime no-follow、唯一 manifest env 和固定中文脱敏均已收口。
5. M2 Codex 独立 M2/managed/task-revision-security/settings/B1-B2=`47/34/65/3/2 passed`；八 Python 文件编译、九生产文件白名单、diff、环境探针与 TEMP left=0 通过。真实 Popen/CLI/模型未运行。
6. M3 已新增精确 `light|managed|local|ask`，保持 `local` 人工回传原语义；非法存量/显式 null、跨项目迟到、旧任务混显和 ask 取消恢复均有红门。Codex 独立后端/P8B/V1-G=`31/25/10 passed`，lint/build/编译/diff/白名单通过。
7. 当前机器无 MinerU/Docling CLI/模型，Python 3.13 兼容、GPU、模型体积、中文 OCR 均未验证；real-runtime/quality 均 did-not-run，不得把文档、假 CLI 或 `cli_missing` 写成解析器就绪。开发工作量粗估约 96%，严格可交付口径仍约 94%。
8. V1-N 远程 MinerU 是独立后续包，不回写 M4 完成状态；当前仅有 B 工作树未提交的测试/契约，生产与真实外网烟测均未授权。
