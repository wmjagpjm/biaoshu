...
Grok 严格三文件：路由、history service、新专项测试；必须 failure-first，固定时间格式/范围错误、V3 条件错配 cursor-invalid、五列投影、source/from/before/keyset 谓词、`LIMIT 11`、lookahead 完整性、跨域、no-store 和五域零写。P12F-D/B/C1 为强制合并回归；Codex 独立后端全量验收并负责提交推送。
