...
正文/标题搜索、来源多选、日期预设、自动加载、命名/固定/删除、跨项目历史、多人协作和后端变更均不在本包。其中 P12F-F-A 后端搜索随后已在下一节独立审计冻结，但尚未实现。

## P12F-F-A 修订可见内容搜索后端（已审计，当前文档冻结）
...
Grok 必须先只新建专项测试形成真实 POST 路由红测，再实现三个生产文件；随后串行运行新专项、P12F-E/D/B、P12C-C1 及后端全量，完成编译/diff/白名单/空暂存区/AST 禁区检查并发 review_request。P12F-F-B 前端、游标/片段、数据库/索引/迁移、来源多选、日期预设、命名/固定/删除、跨项目搜索和多人协作不在 A 包。
