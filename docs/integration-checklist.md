...
## V1-M 管理式本机 OCR 自动解析（已冻结，尚未实现）

契约=`docs/v1m-managed-local-ocr-runtime-contract.md`，计划=`docs/plans/2026-07-22-v1m-managed-local-ocr-runtime-plan.md`，基线=`4631d68`。A/B 两路审计与 Q1 十项均已双确认；当前只允许 M1 failure-first，不计作完成功能。

1. M1 必须用两页 image-only PDF + pypdf 空文本层证明建立真实 OCR 门；fake runtime 只证明包装安全，真实/质量 did-not-run 单列。
2. MinerU 全部可写 HOME/APPDATA/TEMP/cache 与 cwd 必须绑定单次 TEMP；先红测后生产。
3. 仓外 manifest 精确五键，CLI/model marker/requiredFreeBytes 均解析在 runtime 根和目标卷内；不硬编码 D 盘或未经核验的 20 GB。
...
6. 当前机器无 MinerU/Docling CLI/模型，Python 3.13 兼容、GPU、模型体积、中文 OCR 均未验证；不得把文档、假 CLI 或 `cli_missing` 写成解析器就绪。
