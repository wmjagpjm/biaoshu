...
## V1-M 管理式本机 OCR 自动解析（M1-M2 已完成，M3-M4 待完成）

契约=`docs/v1m-managed-local-ocr-runtime-contract.md`，计划=`docs/plans/2026-07-22-v1m-managed-local-ocr-runtime-plan.md`。M1 生产=`a7d640b`；M2 测试=`df85ac7`、生产=`86d5206`。M1-M2 只计 automated/fake-runtime 与后端接线完成，不计真实 OCR 可用。
...
6. M3 新增 `managed`，保持 `local` 人工回传原语义；M4 才允许管理员显式安装和真实 OCR 烟测。
7. 当前机器无 MinerU/Docling CLI/模型，Python 3.13 兼容、GPU、模型体积、中文 OCR 均未验证；real-runtime/quality 均 did-not-run，不得把文档、假 CLI 或 `cli_missing` 写成解析器就绪。V1 粗估仍约 94%。
