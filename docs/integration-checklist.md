...
契约=`docs/v1j-lightweight-no-usable-text-quality-contract.md`，计划=`docs/plans/2026-07-22-v1j-lightweight-no-usable-text-quality-plan.md`，基线=`54eb128`。
...
当前仅冻结，尚无 failure-first、实现或验收数字。启动失败诊断后置，未来方案必须保持默认后台静默、不弹终端/浏览器、不抢焦点。
