...
## P12F-G-A 单条修订删除后端（已审计，当前文档冻结）

契约=`docs/p12f-revision-delete-backend-contract.md`、计划=`docs/plans/2026-07-18-p12f-revision-delete-backend-plan.md`。唯一入口为无 query/body 的 `DELETE /api/projects/{projectId}/editor-state-revisions/{revisionId}`；required 模式必须有效 bid_writer Session + 当前工作空间成员 + CSRF，disabled 保持本机兼容。
