...
契约=`docs/p13f1-project-presence-lease-backend-contract.md`，计划=`docs/plans/2026-07-20-p13f1-project-presence-lease-backend-plan.md`，在途交接=`docs/HANDOFF-p13f1-in-progress.md`。完成后的联调验收项：
...
9. P13-F1 无前端在线 UI、SSE/WebSocket、广播、光标、章节锁、历史或后台清理线程；P13-F2 必须重新冻结。

当前证据仅为 Grok 自测，不能冒充 Codex 验收：task/status/review=`msg_31bba4d10d154daca2acab7d3f6ea1e5`/`msg_aad9a00220a44195965981cfe82dae22`/`msg_b176f13020d5470395f70792f811921b`；failure-first **30 failed / 4 passed**，最终专项/直接回归 **34/55 passed**，py_compile/diff-check 通过。七文件仍未暂存；Codex 尚未完成逐文件审查、独立串行测试、双确认返修判定、结果回执、实现提交与完成态文档闭环。
