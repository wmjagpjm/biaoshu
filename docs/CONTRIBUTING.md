...
1. **本轮新增或大改的文件**必须先有完整文件顶注释，再合入交接清单。  
2. 公开函数至少「用途」+（有外部依赖时）「对接」；纯私有小工具可只写用途一行。  
3. **禁止**大段英文说明；API 路径、标识符、异常类名可保留英文。  
4. 注释描述**当前真实行为**，勿写「待接/TODO 假装已完成」。  
5. 新能力写进 `docs/HANDOFF-next.md` 时，对应模块路径与注释字段保持一致，方便下一会话定位。  

### 交接文档强制条款（极重要）

凡新建或重写 `docs/HANDOFF-*.md` / 换会话交接文，**必须**：

1. 单独设立 **「代码注释规范」** 章节，复述四字段（模块 / 用途 / 对接 / 二次开发）；  
2. 提供 **按功能域的注释齐备表**（齐 / 部分 / 弱），列出关键路径；  
3. 新会话入口提示词中写明注释强制句；  
4. 缺以上三条视为交接不合格——下一会话应先补文档再写功能。  

详见当前有效交接：`docs/HANDOFF-next.md` §2、§9。
