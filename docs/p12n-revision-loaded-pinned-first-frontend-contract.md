...
状态：2026-07-20 已在干净 HEAD `6081a41` 完成只读审计并冻结待实现。该包是快速前端版，不冒充服务端游标级固定优先。
...
2. 点击“固定”成功后，该项立即移动到已加载列表固定组末尾；点击“取消固定”成功后，该项立即回到普通组中按原始服务端顺序对应的位置。PATCH 失败完全保留原位置与状态。
