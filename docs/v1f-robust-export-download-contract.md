...
- 入口只接受当前项目、`status=success`、合法 `storedName` 和可选安全 `filename`；路径必须由当前 `projectId + storedName` 本地构造并逐段编码。
...
4. Windows 保留名 `CON/PRN/AUX/NUL/COM1..9/LPT1..9` 必须安全改写；
5. 空结果回退“标书”，最后只追加一次 `.docx`；
6. 磁盘文件仍为 `export_<8hex>.docx`，下载路由仍只接受 `export_*.docx` basename，绝不按人读名访问磁盘。
...
7. P9D：当前项目告警先显示后下载；跨项目迟到 success 仍零告警、零下载。
