...
> **状态：已完成两路只读审计与 Q1 双确认，契约冻结；尚未授权 failure-first 或生产实现。**
> **基线：** `4631d68`；只允许 `collab/grok-code-codex-review`，严禁操作 `main`。
...
系统已有两条真实链，但尚无“上传后自动 OCR”链：

1. `light`：浏览器创建 `engine=lightweight` 的 parse 任务，后端从项目 uploads 读取文件，使用 `pypdf/python-docx` 解析并以 CAS 写入 editor-state。
2. `local`：浏览器只导航 `/local-parser`；用户在宿主机终端显式运行 MinerU/Docling 外置助手，用一次性票据或 disabled 兼容回调写回 Markdown。
3. 生产 `parse_engines` 注册表只有 `lightweight`；`local|ask` 不启动解析器，MinerU/Docling CLI 与模型均未安装。
4. 本机为 Windows 11、31.8 GB RAM、RTX 5060 8 GB；后端 Python 3.13.3，当前只有 CPU 版 torch。C 盘约 40 GB 可用，D 盘约 154 GB 可用，但任何盘符都不得写成产品常量。
...
`file_service.list_files()` 按 `created_at.desc()` 返回新到旧，`task_service._run_parse()` 却只解析 `files[0]`。创建页允许上传多个源文件，因此当前只解析最新一份，其余文件不进入 `parsedMarkdown` 或 task result。

这属于 V1 自动解析必须关闭的缺口，不得后置到 V2，也不得以单文件 OCR 绿测冒充项目级解析完成。
...
后两步失败时可能出现正文已变化但任务显示 failed 的假失败。M2 必须用 parse 专用 finalizer 把 `parsedMarkdown + revision(source=task) + project step/status + task success + task event` 放入一次提交；任一步失败整包回滚。

### 2.3 外部错误可能泄漏

通用任务异常会把 `str(exc)` 写入 `task.error`，完整任务 API/SSE 又会返回该字段。管理式 runtime 必须在 service 边界将所有外部异常映射为有限 code 和固定中文，禁止绝对路径、argv、第三方 stdout/stderr、异常类名、模型目录或正文进入任务响应。

### 2.4 MinerU 可写根未隔离

P8D MinerU 助手目前继承父进程 `USERPROFILE/HOME/APPDATA/LOCALAPPDATA/TEMP/TMP/TMPDIR`，且 `Popen` 未设置 `cwd`。相对缓存与临时产物可能写入用户固定目录或调用 cwd。M1 必须先以 test-only 证明该缺口，再把全部可写目录和 cwd 绑定单次 TEMP 根；不得直接先改生产。
...
1. `engine=managed` 进入独立 runtime service；`lightweight` 继续走 `parse_engines`，禁止在该注册表执行 subprocess。
2. 所有当前项目 source 文件按 `created_at ASC, id ASC` 确定性解析，禁止继续 `files[0]`。
3. V1 单次任务最多 10 个文件、合计最多 200 MiB；任一越界整任务固定失败，禁止静默截断或部分假成功。
4. 每个文件顺序处理，输出以有限文件分隔组合；合计 Markdown 必须同时满足 1,000,000 Unicode 码点与 2 MiB UTF-8 上限。
5. 管理式 runtime 全进程并发上限 1；每文件最多 30 分钟，任务总时限 120 分钟；取消检查间隔不高于 1 秒，并对直接进程及可观测子进程做尽力清理。
6. 成功使用一次事务写入 editor-state、`source_kind=task` 修订、项目状态/步骤、既有任务 success 与 task event；不新增 revision source 枚举或数据库表。
7. task result 只返回 `engine/fileCount/chars` 等有限元数据，不复制 Markdown、绝对路径或第三方错误。
...
M2/M3 必须在各自开工前重新冻结精确文件表；本契约只冻结行为，不预授权代码。任何新增表、revision source、源文件下载 API、任意路径 API、服务端安装器或公网解析均不允许。
