...
> **状态：M1-M3 automated/fake-runtime 与前端策略接线已完成、独立验收并提交；M4 未开始。M4 真实 runtime/quality 仍为 did-not-run。**
> **基线：** 契约=`9ed3a06`；M1 测试=`3be6d19`/`3b8e42e`/`61dbe38`、生产=`a7d640b`；M2 测试=`df85ac7`、生产=`86d5206`；M3 初始 test-only=`ea2ba31`/`d3748bd`/`c723636`、后续返修 test-only=`3521e12`/`b521d5f`/`cc58960`/`abe8afc`/`3fbd400`、production-only=`eb64dc1`；只允许 `collab/grok-code-codex-review`，严禁操作 `main`。
...
> **M3 最终确认：** 显式 `parseStrategy:null` 零污染、旧任务读取期混显及 ask 取消后安全提示恢复均经 Codex 举证与 Grok YES 后关闭；最终生产 review=`msg_d22681dbfca54a78a5351660881d0313`，恢复问题 YES/review=`msg_bc8f92e12f10476ebc111bec9bfb1eb4`/`msg_97987a0da8404315850708580bb8480a`。

## 1. 当前真值

系统已有三条代码链；代码接线均已完成，但 managed 的真实 runtime 仍未验收：
...
3. `managed`：后端已有独立 adapter、共享 pure core、受控多文件任务和五域单事务 finalizer；它不进入 `parse_engines` 注册表。M3 前端已通过既有任务 API 精确创建 `engine=managed` 任务，M4 CLI/模型与真实质量尚未验收。
...
4. 技术标 `managed` 必须复用 `runWriterTaskWithSuccessReload`，商务标必须复用 `runBizTask`；成功仍走 V1-G 的项目 ID、任务代次与编辑态水合围栏。经 A9/A10 跨项目上传与迟到任务双方确认后，`useProjectPipeline` 仅补项目 owner/generation 围栏；SSE、任务 API 与 M2 finalizer 保持冻结。
...
11. `frontend/src/features/technical-plan/hooks/useProjectPipeline.ts`（仅双方确认的 A9/A10 项目 owner/generation 围栏）

冻结不改：`schemas.py`、`useWorkspaceParseStrategy.ts`、`ParseStrategyChoiceDialog.css`、tasks API、`task_service.py`、`managed_parse_runtime_service.py`、`managed_ocr_runtime_core.py`、`parse_engines.py`、迁移、依赖、启动脚本、真实配置、数据库和 uploads。任何新增必要性必须先由 Codex 举证并重新冻结，不能由实现阶段自行扩围。
...
**M3 完成记录（2026-07-23）：** 初始 test-only=`ea2ba31`/`d3748bd`/`c723636`，后续返修 test-only=`3521e12`/`b521d5f`/`cc58960`/`abe8afc`/`3fbd400`，production-only=`eb64dc1`。Codex 独立串行通过后端双文件 **31 passed**、更新后的 P8B 完整 **25 passed**、V1-G M3 **10 passed**；Python 编译、前端 lint/build、diff-check 与精确白名单通过。`light|managed|local|ask`、非法存量 fail-closed、显式 null 零污染、跨项目迟到隔离、旧任务读取期隐藏和 ask 取消恢复均已闭环。真实 CLI、模型、Popen、real-runtime 与 quality 均 did-not-run，因此本记录只证明 M1-M3 自动接线代码完成，不证明本机真实 OCR 已就绪。
...
M3 最终白名单见 §4；唯一生产扩围为经双方确认的 `useProjectPipeline.ts` 项目 owner/generation 与上传迟到围栏，禁止继续扩写前端。
...
M1-M3 自动接线代码、独立验收和中文分层提交已完成；只有文档闭环推送且 M4 真实 runtime/双扫描 PDF/隔离业务链实际通过后，才能把“扫描 PDF 自动解析”计为真实可用并重新核算严格可交付完成度。M1 文档、假 CLI 或当前 `cli_missing` 均不得上调 94% 口径。
