...
import { HrCredentialExpiryPage } from "../features/hr-credential-expiry/pages/HrCredentialExpiryPage";
...
/**
 * 用途：要求严格 hr 才渲染人员资质/团队推荐/人员业绩/到期提示页。
...
function RequireHr({ children }: { children: ReactNode }) {
  ...
  if (!canAccessHr) {
    return (
      <RestrictedAccessPage reason="仅人力角色可管理本空间人员资质、团队推荐、人员业绩与到期提示。个人版兼容模式与所有者、标书制作者、财务、投标人均不可通过本入口访问。" />
    );
  }
  return <>{children}</>;
}
