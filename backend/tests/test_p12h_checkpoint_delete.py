"""
模块：P12H 单条检查点删除后端专项测试
用途：真实 HTTP+SQLite 验收 DELETE .../editor-state-checkpoints/{checkpointId}
...
对接：DELETE /api/projects/{projectId}/editor-state-checkpoints/{checkpointId}；
  editor_state_checkpoint_delete_service；api.editor_state_checkpoints。
二次开发：
...
# delete_svc 在故障/rowcount 用例内延迟导入，保证 failure-first 可收集

_WS = "ws_local"
_WS_OTHER = "ws_other_p12h"
_SECRET = "SECRET_P12H_BODY_MUST_NOT_LEAK"
_PATH_MARKER = "/api/projects/leaked/editor-state-checkpoints/delete"
_INJECT_EXECUTE = "p12h_injected_execute_failure"
_INJECT_FLUSH = "p12h_injected_flush_failure"
_INJECT_COMMIT = "p12h_injected_commit_failure"
...
_MSG_PROJECT = "项目不存在"
_CODE_DELETE_ERROR = "editor_state_checkpoint_delete_error"
_MSG_DELETE_ERROR = "删除检查点失败"
_CODE_ROLE_FORBIDDEN = "role_forbidden"
_CODE_CSRF_INVALID = "csrf_invalid"

_OWNER_USER = "admin_p12h_owner"
_OWNER_PASS = "TestPass-P12H-Owner-0001!"
_ROLE_PASSWORDS = {
    "bid_writer": "TestPass-P12H-Writer-0001!",
    "finance": "TestPass-P12H-Finance-0001!",
    "hr": "TestPass-P12H-Hr-0001!",
    "bidder": "TestPass-P12H-Bidder-0001!",
}
...
_DELETE_SERVICE_PATH = (
    ...
    / "app"
    / "services"
    / "editor_state_checkpoint_delete_service.py"
)
_API_PATH = (
    Path(__file__).resolve().parents[1]
    / "app"
...
def _page_url(project_id: str) -> str:
    return f"/api/projects/{project_id}/editor-state-checkpoints/page"
...
def _detail_url(project_id: str, checkpoint_id: str) -> str:
    return _delete_url(project_id, checkpoint_id)


def _comparison_url(project_id: str, checkpoint_id: str) -> str:
    return f"{_delete_url(project_id, checkpoint_id)}/comparison"


def _body_diff_url(project_id: str, checkpoint_id: str) -> str:
    return f"{_delete_url(project_id, checkpoint_id)}/body-diff"
...
def _restore_url(project_id: str, checkpoint_id: str) -> str:
    return f"{_delete_url(project_id, checkpoint_id)}/restore"
...
def _assert_fixed_error(
    ...
    *,
    ...
    assert "editor_state_checkpoint_delete_service" not in blob
...
def _login_role(client: TestClient, role: str, *, is_owner: bool = False) -> str:
    ...
    username = f"user_{role}_p12h{'_own' if is_owner else ''}_{secrets.token_hex(3)}"
    ...
    *,
    ...
    for i, tag in enumerate(tags):
        ...
        _insert_raw_checkpoint(
            ...
            display_name=(f"名称{tag}" if named else None),
            created_at=base + timedelta(seconds=i),
            workspace_id=workspace_id,
        )
    return _db_cp_primary_rows(project_id, workspace_id=workspace_id)


def _insert_corrupt_checkpoint(
    project_id: str,
    *,
...
def _ensure_workspace(ws_id: str, name: str = "其他空间P12H") -> None:
    db = SessionLocal()
    try:
...
def _db_cp_primary_rows(project_id: str, workspace_id: str | None = None) -> list[dict]:
    db = SessionLocal()
    try:
...
def _domain_snapshot(project_id: str, *, include_audits: bool = False) -> dict:
    """
    用途：五域快照——检查点/修订/当前态/项目/任务。
    二次开发：权限登录会写审计；断言删除零副作用时默认排除 audits。
    """
    snap = {
        "checkpoints": _db_cp_primary_rows(project_id),
...
def _seed_five_domain(project_id: str, tags: list[str], *, named: bool = False) -> list[dict]:
    """用途：写入当前态/修订/任务/多检查点，供五域不变性断言。"""
    _ensure_editor_state(project_id, "cur")
    _insert_revision(project_id, "rev_keep")
    _insert_task(project_id, tag="keep")
    return _seed_checkpoints(project_id, tags, named=named)
...
def _delete(
    ...
    *,
    ...
    return client.request(
        "DELETE",
        _delete_url(project_id, checkpoint_id),
        **kwargs,
    )
...
def test_delete_positions_and_corrupt_then_second_404(disabled_client, position):
    ...
    rows = _seed_checkpoints(pid, ["a", "b", "c"])
    corrupt_id = _insert_corrupt_checkpoint(
        pid,
        created_at=datetime(2026, 7, 3, tzinfo=timezone.utc),
    )
    all_rows = _db_cp_primary_rows(pid)
    ...
    _assert_success_204(res)
    assert target_id not in {r["id"] for r in _db_cp_primary_rows(pid)}
...
def test_delete_project_missing_and_cross_scope_404(disabled_client):
    ...
    res_fp = _delete(client, foreign_pid, foreign_cp)
    _assert_fixed_error(res_fp, 404, _CODE_PROJECT, message=_MSG_PROJECT)

    # 跨 project：B 项目的 checkpoint 用 A 项目路径
    ...
    # 任意格式 ID
    for bad_id in ("not-a-checkpoint", "escp_short", "ESCP_" + ("f" * 32), target.upper()):
        res_bad = _delete(client, pid, bad_id)
        _assert_fixed_error(
            res_bad, 404, _CODE_NOT_FOUND, message=_MSG_NOT_FOUND, forbid_echo=bad_id
        )

    # 外域 checkpoint 行必须仍在；本域五域精确不变
    assert any(
        r["id"] == foreign_cp
        for r in _db_cp_primary_rows(foreign_pid, workspace_id=_WS_OTHER)
    )
    assert _domain_snapshot(pid) == before
    assert _db_cp_primary_rows(pid_b)[0]["id"] == rows_b[0]["id"]
...
def test_delete_query_and_body_fixed_422_zero_write(disabled_client):
    ...
    cases = [
        ...
        {"params": {"checkpointId": rid}, "content": None, "json_body": None, "echo": rid},
        {
            "params": None,
            "content": b'{"BODYMARKER_P12H":true}',
            "json_body": None,
            "echo": "BODYMARKER_P12H",
        },
        ...
        },
        {
            "params": None,
            "content": None,
            "json_body": {"x": 1, "marker": "JSONBODY_P12H"},
            "echo": "JSONBODY_P12H",
        },
        {
            "params": None,
    ...
    for case in cases:
        ...
        try:
            ...
            assert captured == [], f"422 不得执行 checkpoint DELETE: {captured}"
...
def test_delete_required_roles_csrf_and_owner_no_bypass(required_client):
    ...
    after = _domain_snapshot(pid)
    assert rid_ok not in {r["id"] for r in after["checkpoints"]}
    assert rid_deny in {r["id"] for r in after["checkpoints"]}
    assert after["revisions"] == before["revisions"]
    assert after["editor_state"] == before["editor_state"]
    assert after["project"] == before["project"]
    assert after["tasks"] == before["tasks"]
...
def test_delete_execute_flush_commit_failures_rollback(disabled_client):
    from app.services import editor_state_checkpoint_delete_service as delete_svc
    """
    ...
    def _run_fault(kind: str, checkpoint_id: str) -> dict:
        ...
        try:
            ...
            with pytest.raises(delete_svc.EditorStateCheckpointDeleteError) as ei:
                delete_svc.delete_editor_state_checkpoint(db, _WS, pid, checkpoint_id)
            exc = ei.value
            assert exc.status_code == 500
            assert exc.code == _CODE_DELETE_ERROR
            assert exc.message == _MSG_DELETE_ERROR
    ...
    try:
        ...
        delete_svc.delete_editor_state_checkpoint(db_ok, _WS, pid, targets[0])
        assert counters == {
            "execute": 2,
            "flush": 1,
            "commit": 1,
    ...
    assert targets[0] not in {r["id"] for r in _db_cp_primary_rows(pid)}
    assert targets[1] in {r["id"] for r in _db_cp_primary_rows(pid)}
    assert targets[2] in {r["id"] for r in _db_cp_primary_rows(pid)}
...
def test_delete_rowcount_none_negative_multi_fixed_500(disabled_client):
    ...
    def _run_with_rowcount(forced, checkpoint_id: str):
        db = SessionLocal()
        try:
            real_execute = db.execute
            real_rollback = db.rollback
            ...
            def _exec(*a, **k):
                ...
                if "editor_state_checkpoints" in sql and "delete" in sql:
    ...
    # 0 → checkpoint 404 + rollback，五域恢复
    ...
    after = _domain_snapshot(pid)
    assert targets[4] not in {r["id"] for r in after["checkpoints"]}
    assert after["revisions"] == before["revisions"]
    assert after["editor_state"] == before["editor_state"]
    assert after["project"] == before["project"]
    assert after["tasks"] == before["tasks"]
    for kept in targets[:4]:
        assert kept in {r["id"] for r in after["checkpoints"]}
...
def test_delete_read_chain_list_detail_restore_name(disabled_client):
    client = disabled_client
    pid = _create_project(client, name="读取链")
    rows = _seed_five_domain(pid, ["r0", "r1", "r2"])
    target = rows[1]
    expected_ids = [r["id"] for r in rows if r["id"] != target["id"]]
    before_other = {
        "revisions": _db_rev_rows(pid),
        "editor_state": _db_editor_state_row(pid),
        "project": _db_project_row(pid),
    ...
    assert _db_rev_rows(pid) == before_other["revisions"]
    ...
    _assert_fixed_error(d, 404, _CODE_NOT_FOUND)
    ...
    _assert_fixed_error(rst, 404, _CODE_NOT_FOUND)

    name = client.patch(
        f"{_delete_url(pid, target['id'])}/display-name",
        json={"displayName": "不应成功"},
    )
    _assert_fixed_error(name, 404, _CODE_NOT_FOUND)
...
def test_delete_ast_and_source_guards():
    ...
    assert "EditorStateRevisionRow" not in svc_src
    ...
    assert "delete_editor_state_checkpoint" in svc_src
    assert "editor_state_checkpoint_delete_error" in svc_src
    ...
    for node in api_tree.body:
        if not isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            continue
        if node.name != "delete_editor_state_checkpoint":
            continue
    ...
    assert "editor_state_checkpoint_delete_request_invalid" in api_src
