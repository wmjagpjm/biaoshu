...
class EditorStateCheckpointPinUpdate(BaseModel):
    """
    模块：P12J-A 单条检查点固定请求
    用途：仅 camelCase isPinned；值以 Any 原样承接后由 service 校验原生 bool。
    对接：PATCH .../editor-state-checkpoints/{checkpointId}/pin。
    二次开发：extra=forbid；禁止 populate_by_name；snake_case/缺失/额外键 422。
    """
...
class EditorStateCheckpointPinOut(BaseModel):
    """
    模块：P12J-A 单条检查点固定响应
    用途：成功仅回 isPinned（原生 boolean）。
    对接：PATCH .../editor-state-checkpoints/{checkpointId}/pin。
    二次开发：禁止回显 checkpointId/版本/正文/路径/计数/配额。
    """
