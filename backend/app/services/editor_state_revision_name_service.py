"""
模块：P12F-H editor-state 修订单条展示名称服务
用途：在当前工作空间/项目三重作用域内更新恰好一条修订的 display_name；成功唯一 commit。
对接：api.editor_state_revisions PATCH .../display-name；
  EditorStateRevisionRow；Project。
二次开发：
  - 禁止加载 snapshot_json / ORM 整实体 / 当前态 / 检查点；
...
CODE_NAME_INVALID = "editor_state_revision_display_name_invalid"
MSG_NAME_INVALID = "修订名称无效"
CODE_NAME_ERROR = "editor_state_revision_display_name_error"
MSG_NAME_ERROR = "保存修订名称失败"
...
class EditorStateRevisionNameError(Exception):
    """
...
def _name_failed() -> EditorStateRevisionNameError:
    """用途：统一构造脱敏内部失败，禁止附带异常原文。"""
    return EditorStateRevisionNameError(500, CODE_NAME_ERROR, MSG_NAME_ERROR)


def _name_invalid() -> EditorStateRevisionNameError:
    """用途：统一构造脱敏名称无效，禁止反射输入。"""
    return EditorStateRevisionNameError(422, CODE_NAME_INVALID, MSG_NAME_INVALID)
...
def set_editor_state_revision_display_name(
    db: Session,
    workspace_id: str,
    project_id: str,
    revision_id: str,
    ...
    """
    用途：确认项目后按三重作用域更新恰好一行修订的 display_name。
    对接：PATCH /api/projects/{projectId}/editor-state-revisions/{revisionId}/display-name。
    规则：0 行 → revision 404；精确 1 → 成功；None/负数/非 1 → 500；成功唯一 commit。
    """
    ...
    try:
        ...
        if project_row is None:
            raise EditorStateRevisionNameError(
                404, CODE_PROJECT_NOT_FOUND, MSG_PROJECT_NOT_FOUND
            )

        # 2) 三谓词单列 UPDATE；禁止按裸 ID 加载 ORM / 读取正文
        result = db.execute(
        ...
        db.commit()
        return normalized
    except EditorStateRevisionNameError:
        db.rollback()
        raise
    except Exception:
        db.rollback()
